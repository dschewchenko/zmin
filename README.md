...
| Written behavior rows | `960` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `760/960` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/960` | written rows that still do not match stock Git |
| Invalid input rows | `200/960` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `760/960` as Git compatibility. It only means `760` of the `960`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `200/960` additional rows are
...
| Plumbing Commands | `20` | `0` | `644` | `0` | `190` | `149` | `0` | `41` |
| Other Git `2.47` commands | `71` | `0` | `1075` | `0` | `6` | `2` | `0` | `4` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`960`** | **`760`** | **`0`** | **`200`** |
...
| `ls-files` | `42` | `0` | `27` | `71` | `55` | `0` | `0` | `16` | no |
