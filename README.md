...
| Documented option spellings represented by rows | `607/4632` | option spellings that have at least one behavior row; this is not support |
| Written behavior rows | `2649` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2272/2649` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2649` | written rows with incomplete parity |
| Open written rows | `1/2649` | written rows that still do not match stock Git |
| Invalid input rows | `376/2649` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2272/2649` as Git compatibility. It only means `2272` of the `2649`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2649` rows are partially matching,
`1/2649` rows are known open mismatches, and `376/2649` additional rows are
stock-compatible invalid inputs. Do not read `607/4632` as option support
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2649`** | **`2272`** | **`0`** | **`1`** | **`376`** |
...
| `multi-pack-index` | `10` | `0` | `1` | `6` | `3` | `0` | `0` | `3` | no |
