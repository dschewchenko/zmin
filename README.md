...
| Written behavior rows | `630` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `554/630` | exact written rows with focused parity evidence |
| Open written rows | `8/630` | written rows that still do not match stock Git |
| Invalid input rows | `68/630` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `554/630` as Git compatibility. It only means `554` of the `630`
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `120` | `119` | `0` | `1` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`630`** | **`554`** | **`8`** | **`68`** |
