...
| Written behavior rows | `2265` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1949/2265` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2265` | written rows with incomplete parity |
| Open written rows | `1/2265` | written rows that still do not match stock Git |
| Invalid input rows | `315/2265` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1949/2265` as Git compatibility. It only means `1949` of the `2265`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2265` rows are partially matching,
`1/2265` rows are known open mismatches, and `315/2265` additional rows are
...
| Branching and Merging | `9` | `0` | `581` | `0` | `208` | `176` | `0` | `0` | `32` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2265`** | **`1949`** | **`0`** | **`1`** | **`315`** |
