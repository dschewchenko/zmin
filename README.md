...
| Written behavior rows | `1087` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `819/1087` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1087` | written rows with incomplete parity |
| Open written rows | `1/1087` | written rows that still do not match stock Git |
| Invalid input rows | `267/1087` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `819/1087` as Git compatibility. It only means `819` of the `1087`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1087` rows are partially matching,
`1/1087` rows are known open mismatches, and `267/1087` additional rows are
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `309` | `291` | `0` | `0` | `18` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1087`** | **`819`** | **`0`** | **`1`** | **`267`** |
