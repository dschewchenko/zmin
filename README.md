...
| Written behavior rows | `1093` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `822/1093` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1093` | written rows with incomplete parity |
| Open written rows | `1/1093` | written rows that still do not match stock Git |
| Invalid input rows | `270/1093` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `822/1093` as Git compatibility. It only means `822` of the `1093`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1093` rows are partially matching,
`1/1093` rows are known open mismatches, and `270/1093` additional rows are
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1093`** | **`822`** | **`0`** | **`1`** | **`270`** |
