...
| Written behavior rows | `218` | rows written in the current matrices |
| Written rows matching stock Git | `203/218` | exact rows with focused parity evidence |
| Open written rows | `9/218` | written rows that still do not match stock Git |
| Invalid input rows | `6/218` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests and real tool traces |

Do not read `203/218` as Git compatibility. It only means `203` of the `218`
...
| Sharing and Updating Projects | `5` | `309` | `158` | `147` | `9` | `2` |
...
| **Git `2.47.1` unique total** | **`151`** | **`4632`** | **`218`** | **`203`** | **`9`** | **`6`** |
