...
| Written behavior rows | `769` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `693/769` | exact written rows with focused parity evidence |
| Open written rows | `0/769` | written rows that still do not match stock Git |
| Invalid input rows | `76/769` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `693/769` as Git compatibility. It only means `693` of the `769`
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`769`** | **`693`** | **`0`** | **`76`** |
