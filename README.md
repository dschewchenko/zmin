...
| Commands with any matrix rows | `88/151` | commands that have started behavior matrices; this is not support |
...
| Written behavior rows | `1940` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1633/1940` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1940` | written rows with incomplete parity |
| Open written rows | `1/1940` | written rows that still do not match stock Git |
| Invalid input rows | `306/1940` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1633/1940` as Git compatibility. It only means `1633` of the `1940`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1940` rows are partially matching,
`1/1940` rows are known open mismatches, and `306/1940` additional rows are
...
| Plumbing Commands | `21` | `0` | `650` | `0` | `462` | `397` | `0` | `0` | `65` |
| Other Git 2.47 commands | `70` | `0` | `1069` | `0` | `106` | `72` | `0` | `0` | `34` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1940`** | **`1633`** | **`0`** | **`1`** | **`306`** |
