...
| Written behavior rows | `853` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `737/853` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/853` | written rows that still do not match stock Git |
| Invalid input rows | `116/853` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `737/853` as Git compatibility. It only means `737` of the `853`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `116/853` additional rows are
