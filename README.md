...
| Written behavior rows | `1767` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1466/1767` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1767` | written rows with incomplete parity |
| Open written rows | `1/1767` | written rows that still do not match stock Git |
| Invalid input rows | `300/1767` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1466/1767` as Git compatibility. It only means `1466` of the `1767`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1767` rows are partially matching,
`1/1767` rows are known open mismatches, and `300/1767` additional rows are
...
| Plumbing Commands | `21` | `0` | `650` | `0` | `382` | `319` | `0` | `0` | `63` |
| Other Git 2.47 commands | `70` | `0` | `1069` | `0` | `106` | `72` | `0` | `0` | `34` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1767`** | **`1466`** | **`0`** | **`1`** | **`300`** |
...
| `ls-files` | `42` | `0` | `28` | `126` | `108` | `0` | `0` | `18` | no |
