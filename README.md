...
| Commands with any matrix rows | `19/151` | `archive`, `branch`, `column`, `config`, `status`, `fetch`, `diff`, `log`, `for-each-ref`, `ls-files`, `ls-tree`, `merge-base`, `notes`, `rerere`, `rev-parse`, `show-ref`, `symbolic-ref`, `tag` and `version` have started behavior matrices |
...
| Written behavior rows | `811` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `727/811` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/811` | written rows that still do not match stock Git |
| Invalid input rows | `84/811` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `727/811` as Git compatibility. It only means `727` of the `811`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `84/811` additional rows are
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `71` | `66` | `0` | `5` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`811`** | **`727`** | **`0`** | **`84`** |
...
`ls-tree`, `merge-base`, `notes`, `rerere`, `rev-parse`, `show-ref`, `symbolic-ref`,
`tag` or `version`
