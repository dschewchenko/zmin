...
| Written behavior rows | `2494` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2142/2494` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2494` | written rows with incomplete parity |
| Open written rows | `1/2494` | written rows that still do not match stock Git |
| Invalid input rows | `351/2494` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2142/2494` as Git compatibility. It only means `2142` of the `2494`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2494` rows are partially matching,
`1/2494` rows are known open mismatches, and `351/2494` additional rows are
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `288` | `259` | `0` | `0` | `29` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2494`** | **`2142`** | **`0`** | **`1`** | **`351`** |
