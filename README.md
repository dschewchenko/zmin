...
| Written behavior rows | `802` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `726/802` | exact written rows with focused parity evidence |
| Open written rows | `0/802` | written rows that still do not match stock Git |
| Invalid input rows | `76/802` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `726/802` as Git compatibility. It only means `726` of the `802`
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`802`** | **`726`** | **`0`** | **`76`** |
