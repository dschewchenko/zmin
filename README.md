...
| Written behavior rows | `783` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `707/783` | exact written rows with focused parity evidence |
| Open written rows | `0/783` | written rows that still do not match stock Git |
| Invalid input rows | `76/783` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `707/783` as Git compatibility. It only means `707` of the `783`
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`783`** | **`707`** | **`0`** | **`76`** |
