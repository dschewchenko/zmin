...
| Written behavior rows | `2396` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2054/2396` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2396` | written rows with incomplete parity |
| Open written rows | `1/2396` | written rows that still do not match stock Git |
| Invalid input rows | `341/2396` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2054/2396` as Git compatibility. It only means `2054` of the `2396`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2396` rows are partially matching,
`1/2396` rows are known open mismatches, and `341/2396` additional rows are
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `190` | `171` | `0` | `0` | `19` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2396`** | **`2054`** | **`0`** | **`1`** | **`341`** |
