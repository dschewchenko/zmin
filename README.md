...
| Written behavior rows | `784` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `708/784` | exact written rows with focused parity evidence |
| Open written rows | `0/784` | written rows that still do not match stock Git |
| Invalid input rows | `76/784` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `708/784` as Git compatibility. It only means `708` of the `784`
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `67` | `63` | `0` | `4` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`784`** | **`708`** | **`0`** | **`76`** |
