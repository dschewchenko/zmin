...
| Written behavior rows | `2476` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2126/2476` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2476` | written rows with incomplete parity |
| Open written rows | `1/2476` | written rows that still do not match stock Git |
| Invalid input rows | `349/2476` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2126/2476` as Git compatibility. It only means `2126` of the `2476`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2476` rows are partially matching,
`1/2476` rows are known open mismatches, and `349/2476` additional rows are
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `270` | `243` | `0` | `0` | `27` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2476`** | **`2126`** | **`0`** | **`1`** | **`349`** |
