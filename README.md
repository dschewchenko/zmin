...
| Written behavior rows | `910` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `747/910` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/910` | written rows that still do not match stock Git |
| Invalid input rows | `163/910` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `747/910` as Git compatibility. It only means `747` of the `910`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `163/910` additional rows are
...
| Debugging | `3` | `0` | `132` | `0` | `60` | `13` | `0` | `47` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`910`** | **`747`** | **`0`** | **`163`** |
