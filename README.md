...
| Written behavior rows | `731` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `655/731` | exact written rows with focused parity evidence |
| Open written rows | `0/731` | written rows that still do not match stock Git |
| Invalid input rows | `76/731` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `655/731` as Git compatibility. It only means `655` of the `731`
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `265` | `259` | `0` | `6` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`731`** | **`655`** | **`0`** | **`76`** |
...
| `fetch` | `73` | `0` | `30` | `265` | `259` | `0` | `0` | `6` | no |
