...
| Written behavior rows | `2528` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2154/2528` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2528` | written rows with incomplete parity |
| Open written rows | `1/2528` | written rows that still do not match stock Git |
| Invalid input rows | `373/2528` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2154/2528` as Git compatibility. It only means `2154` of the `2528`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2528` rows are partially matching,
`1/2528` rows are known open mismatches, and `373/2528` additional rows are
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `356` | `331` | `0` | `0` | `25` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2528`** | **`2154`** | **`0`** | **`1`** | **`373`** |
