...
| Written behavior rows | `1080` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `815/1080` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1080` | written rows with incomplete parity |
| Open written rows | `1/1080` | written rows that still do not match stock Git |
| Invalid input rows | `264/1080` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `815/1080` as Git compatibility. It only means `815` of the `1080`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1080` rows are partially matching,
`1/1080` rows are known open mismatches, and `264/1080` additional rows are
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `304` | `287` | `0` | `0` | `17` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1080`** | **`815`** | **`0`** | **`1`** | **`264`** |
...
| `fetch` | `73` | `0` | `30` | `295` | `287` | `0` | `0` | `8` | no |
