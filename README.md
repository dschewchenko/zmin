...
| Written behavior rows | `1050` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `788/1050` | supported-behavior rows with focused parity evidence |
| Partial written rows | `1/1050` | written rows with incomplete parity |
| Open written rows | `0/1050` | written rows that still do not match stock Git |
| Invalid input rows | `261/1050` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `788/1050` as Git compatibility. It only means `788` of the `1050`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `1/1050` row is only partially matching,
and `261/1050` additional rows are stock-compatible invalid inputs. Do
...
| Getting and Creating Projects | `2` | `0` | `66` | `0` | `2` | `1` | `0` | `0` | `1` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1050`** | **`788`** | **`1`** | **`0`** | **`261`** |
