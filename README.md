...
| Written behavior rows | `796` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `720/796` | exact written rows with focused parity evidence |
| Open written rows | `0/796` | written rows that still do not match stock Git |
| Invalid input rows | `76/796` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `720/796` as Git compatibility. It only means `720` of the `796`
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `146` | `143` | `0` | `3` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`796`** | **`720`** | **`0`** | **`76`** |
