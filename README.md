...
| Written behavior rows | `2646` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2269/2646` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2646` | written rows with incomplete parity |
| Open written rows | `1/2646` | written rows that still do not match stock Git |
| Invalid input rows | `376/2646` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2269/2646` as Git compatibility. It only means `2269` of the `2646`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2646` rows are partially matching,
`1/2646` rows are known open mismatches, and `376/2646` additional rows are
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `413` | `388` | `0` | `0` | `25` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2646`** | **`2269`** | **`0`** | **`1`** | **`376`** |
