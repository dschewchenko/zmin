...
| Written behavior rows | `855` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `739/855` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/855` | written rows that still do not match stock Git |
| Invalid input rows | `116/855` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `739/855` as Git compatibility. It only means `739` of the `855`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `116/855` additional rows are
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`855`** | **`739`** | **`0`** | **`116`** |
