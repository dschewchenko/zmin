...
| Written behavior rows | `226` | rows written in the current matrices |
| Written rows matching stock Git | `211/226` | exact rows with focused parity evidence |
| Open written rows | `9/226` | written rows that still do not match stock Git |
| Invalid input rows | `6/226` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests and real tool traces |

Do not read `211/226` as Git compatibility. It only means `211` of the `226`
...
| Sharing and Updating Projects | `5` | `309` | `166` | `155` | `9` | `2` |
...
| **Git `2.47.1` unique total** | **`151`** | **`4632`** | **`226`** | **`211`** | **`9`** | **`6`** |
