...
| Written behavior rows | `780` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `704/780` | exact written rows with focused parity evidence |
| Open written rows | `0/780` | written rows that still do not match stock Git |
| Invalid input rows | `76/780` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `704/780` as Git compatibility. It only means `704` of the `780`
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`780`** | **`704`** | **`0`** | **`76`** |
