...
| Documented option spellings represented by rows | `535/4632` | option spellings that have at least one behavior row; this is not support |
| Written behavior rows | `2115` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1804/2115` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2115` | written rows with incomplete parity |
| Open written rows | `1/2115` | written rows that still do not match stock Git |
| Invalid input rows | `310/2115` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1804/2115` as Git compatibility. It only means `1804` of the `2115`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2115` rows are partially matching,
`1/2115` rows are known open mismatches, and `310/2115` additional rows are
stock-compatible invalid inputs. Do not read `535/4632` as option support
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `391` | `383` | `0` | `0` | `8` |
...
| Other Git 2.47 commands | `70` | `0` | `1069` | `0` | `116` | `82` | `0` | `0` | `34` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2115`** | **`1804`** | **`0`** | **`1`** | **`310`** |
...
| `diff` | `133` | `0` | `69` | `234` | `231` | `0` | `0` | `3` | no |
| `diff-files` | `130` | `0` | `5` | `8` | `8` | `0` | `0` | `0` | no |
