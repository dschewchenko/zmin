...
| Commands with any matrix rows | `97/151` | commands that have started behavior matrices; this is not support |
...
| Written behavior rows | `2354` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2012/2354` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2354` | written rows with incomplete parity |
| Open written rows | `1/2354` | written rows that still do not match stock Git |
| Invalid input rows | `341/2354` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2012/2354` as Git compatibility. It only means `2012` of the `2354`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2354` rows are partially matching,
`1/2354` rows are known open mismatches, and `341/2354` additional rows are
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2354`** | **`2012`** | **`0`** | **`1`** | **`341`** |
