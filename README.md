...
| Written behavior rows | `865` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `739/865` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/865` | written rows that still do not match stock Git |
| Invalid input rows | `126/865` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `739/865` as Git compatibility. It only means `739` of the `865`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `126/865` additional rows are
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`865`** | **`739`** | **`0`** | **`126`** |
