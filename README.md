...
| Documented option spellings represented by rows | `231/4632` | option spellings that have at least one behavior row; this is not support |
| Written behavior rows | `834` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `732/834` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/834` | written rows that still do not match stock Git |
| Invalid input rows | `102/834` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `732/834` as Git compatibility. It only means `732` of the `834`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `102/834` additional rows are
stock-compatible invalid inputs. Do not read `231/4632` as option support
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `78` | `66` | `0` | `12` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`834`** | **`732`** | **`0`** | **`102`** |
