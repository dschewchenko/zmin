...
| Written behavior rows | `1091` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `821/1091` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1091` | written rows with incomplete parity |
| Open written rows | `1/1091` | written rows that still do not match stock Git |
| Invalid input rows | `269/1091` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `821/1091` as Git compatibility. It only means `821` of the `1091`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1091` rows are partially matching,
`1/1091` rows are known open mismatches, and `269/1091` additional rows are
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1091`** | **`821`** | **`0`** | **`1`** | **`269`** |
