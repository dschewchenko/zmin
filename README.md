...
| Written behavior rows | `778` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `702/778` | exact written rows with focused parity evidence |
| Open written rows | `0/778` | written rows that still do not match stock Git |
| Invalid input rows | `76/778` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `702/778` as Git compatibility. It only means `702` of the `778`
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `140` | `137` | `0` | `3` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`778`** | **`702`** | **`0`** | **`76`** |
