...
| Written behavior rows | `1082` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `817/1082` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1082` | written rows with incomplete parity |
| Open written rows | `1/1082` | written rows that still do not match stock Git |
| Invalid input rows | `264/1082` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `817/1082` as Git compatibility. It only means `817` of the `1082`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1082` rows are partially matching,
`1/1082` rows are known open mismatches, and `264/1082` additional rows are
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `306` | `289` | `0` | `0` | `17` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1082`** | **`817`** | **`0`** | **`1`** | **`264`** |
...
| `fetch` | `73` | `0` | `30` | `297` | `289` | `0` | `0` | `8` | no |
