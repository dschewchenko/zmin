...
| Documented option spellings represented by rows | `599/4632` | option spellings that have at least one behavior row; this is not support |
| Written behavior rows | `2622` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2245/2622` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2622` | written rows with incomplete parity |
| Open written rows | `1/2622` | written rows that still do not match stock Git |
| Invalid input rows | `376/2622` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2245/2622` as Git compatibility. It only means `2245` of the `2622`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2622` rows are partially matching,
`1/2622` rows are known open mismatches, and `376/2622` additional rows are
stock-compatible invalid inputs. Do not read `599/4632` as option support
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2622`** | **`2245`** | **`0`** | **`1`** | **`376`** |
