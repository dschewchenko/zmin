...
| Written behavior rows | `643` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `566/643` | exact written rows with focused parity evidence |
| Open written rows | `8/643` | written rows that still do not match stock Git |
| Invalid input rows | `69/643` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `566/643` as Git compatibility. It only means `566` of the `643`
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `133` | `131` | `0` | `2` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`643`** | **`566`** | **`8`** | **`69`** |
