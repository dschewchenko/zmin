...
| Written behavior rows | `2447` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2101/2447` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2447` | written rows with incomplete parity |
| Open written rows | `1/2447` | written rows that still do not match stock Git |
| Invalid input rows | `345/2447` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2101/2447` as Git compatibility. It only means `2101` of the `2447`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2447` rows are partially matching,
`1/2447` rows are known open mismatches, and `345/2447` additional rows are
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `241` | `218` | `0` | `0` | `23` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2447`** | **`2101`** | **`0`** | **`1`** | **`345`** |
