...
| Written behavior rows | `808` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `727/808` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/808` | written rows that still do not match stock Git |
| Invalid input rows | `81/808` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `727/808` as Git compatibility. It only means `727` of the `808`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `81/808` additional rows are
...
| Plumbing Commands | `20` | `0` | `644` | `0` | `175` | `144` | `0` | `31` |
| Other Git `2.47` commands | `71` | `0` | `1075` | `0` | `5` | `2` | `0` | `3` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`808`** | **`727`** | **`0`** | **`81`** |
...
| `rev-parse` | `72` | `0` | `24` | `48` | `43` | `0` | `0` | `5` | no |
