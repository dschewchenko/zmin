...
| Written behavior rows | `649` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `573/649` | exact written rows with focused parity evidence |
| Open written rows | `6/649` | written rows that still do not match stock Git |
| Invalid input rows | `70/649` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `573/649` as Git compatibility. It only means `573` of the `649`
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `198` | `189` | `6` | `3` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`649`** | **`573`** | **`6`** | **`70`** |
