...
| Written behavior rows | `2616` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2239/2616` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2616` | written rows with incomplete parity |
| Open written rows | `1/2616` | written rows that still do not match stock Git |
| Invalid input rows | `376/2616` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2239/2616` as Git compatibility. It only means `2239` of the `2616`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2616` rows are partially matching,
`1/2616` rows are known open mismatches, and `376/2616` additional rows are
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2616`** | **`2239`** | **`0`** | **`1`** | **`376`** |
