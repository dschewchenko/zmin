...
| Written behavior rows | `1015` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `776/1015` | supported-behavior rows with focused parity evidence |
| Partial written rows | `1/1015` | written rows with incomplete parity |
| Open written rows | `0/1015` | written rows that still do not match stock Git |
| Invalid input rows | `238/1015` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `776/1015` as Git compatibility. It only means `776` of the `1015`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `1/1015` row is only partially matching
stock Git and `238/1015` additional rows are stock-compatible invalid inputs. Do
...
| Branching and Merging | `9` | `0` | `581` | `0` | `62` | `37` | `0` | `0` | `25` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1015`** | **`776`** | **`1`** | **`0`** | **`238`** |
