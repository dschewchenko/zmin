...
| Written behavior rows | `1017` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `777/1017` | supported-behavior rows with focused parity evidence |
| Partial written rows | `1/1017` | written rows with incomplete parity |
| Open written rows | `0/1017` | written rows that still do not match stock Git |
| Invalid input rows | `239/1017` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `777/1017` as Git compatibility. It only means `777` of the `1017`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `1/1017` row is only partially matching
stock Git and `239/1017` additional rows are stock-compatible invalid inputs. Do
...
| Branching and Merging | `9` | `0` | `581` | `0` | `64` | `38` | `0` | `0` | `26` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1017`** | **`777`** | **`1`** | **`0`** | **`239`** |
