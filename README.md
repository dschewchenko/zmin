...
| Written behavior rows | `940` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `755/940` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/940` | written rows that still do not match stock Git |
| Invalid input rows | `185/940` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `755/940` as Git compatibility. It only means `755` of the `940`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `185/940` additional rows are
...
| Debugging | `3` | `0` | `132` | `0` | `90` | `21` | `0` | `69` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`940`** | **`755`** | **`0`** | **`185`** |
