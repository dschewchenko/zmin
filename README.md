...
| Written behavior rows | `644` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `567/644` | exact written rows with focused parity evidence |
| Open written rows | `8/644` | written rows that still do not match stock Git |
| Invalid input rows | `69/644` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `567/644` as Git compatibility. It only means `567` of the `644`
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `134` | `132` | `0` | `2` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`644`** | **`567`** | **`8`** | **`69`** |
