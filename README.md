...
| Written behavior rows | `2274` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1957/2274` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2274` | written rows with incomplete parity |
| Open written rows | `1/2274` | written rows that still do not match stock Git |
| Invalid input rows | `316/2274` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1957/2274` as Git compatibility. It only means `1957` of the `2274`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2274` rows are partially matching,
`1/2274` rows are known open mismatches, and `316/2274` additional rows are
...
| Branching and Merging | `9` | `0` | `581` | `0` | `217` | `184` | `0` | `0` | `33` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2274`** | **`1957`** | **`0`** | **`1`** | **`316`** |
