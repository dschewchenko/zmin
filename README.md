...
| Written behavior rows | `752` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `676/752` | exact written rows with focused parity evidence |
| Open written rows | `0/752` | written rows that still do not match stock Git |
| Invalid input rows | `76/752` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `676/752` as Git compatibility. It only means `676` of the `752`
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`752`** | **`676`** | **`0`** | **`76`** |
