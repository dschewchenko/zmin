...
| Documented option spellings represented by rows | `251/4632` | option spellings that have at least one behavior row; this is not support |
| Written behavior rows | `1074` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `811/1074` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1074` | written rows with incomplete parity |
| Open written rows | `1/1074` | written rows that still do not match stock Git |
| Invalid input rows | `262/1074` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `811/1074` as Git compatibility. It only means `811` of the `1074`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1074` rows are partially matching,
`1/1074` rows are known open mismatches, and `262/1074` additional rows are
stock-compatible invalid inputs. Do not read `251/4632` as option support
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `83` | `66` | `0` | `0` | `17` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1074`** | **`811`** | **`0`** | **`1`** | **`262`** |
