...
| Written behavior rows | `214` | rows written in the current matrices |
| Written rows matching stock Git | `199/214` | exact rows with focused parity evidence |
| Open written rows | `9/214` | written rows that still do not match stock Git |
| Invalid input rows | `6/214` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests and real tool traces |

Do not read `199/214` as Git compatibility. It only means `199` of the `214`
...
| Sharing and Updating Projects | `5` | `309` | `154` | `143` | `9` | `2` |
...
| **Git `2.47.1` unique total** | **`151`** | **`4632`** | **`214`** | **`199`** | **`9`** | **`6`** |
