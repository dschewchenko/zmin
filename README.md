...
| Written behavior rows | `842` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `732/842` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/842` | written rows that still do not match stock Git |
| Invalid input rows | `110/842` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `732/842` as Git compatibility. It only means `732` of the `842`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `110/842` additional rows are
...
| Plumbing Commands | `20` | `0` | `644` | `0` | `178` | `144` | `0` | `34` |
| Other Git `2.47` commands | `71` | `0` | `1075` | `0` | `6` | `2` | `0` | `4` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`842`** | **`732`** | **`0`** | **`110`** |
...
| `ls-files` | `42` | `0` | `27` | `60` | `51` | `0` | `0` | `9` | no |
