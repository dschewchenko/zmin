...
| Written behavior rows | `1044` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `786/1044` | supported-behavior rows with focused parity evidence |
| Partial written rows | `1/1044` | written rows with incomplete parity |
| Open written rows | `0/1044` | written rows that still do not match stock Git |
| Invalid input rows | `257/1044` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `786/1044` as Git compatibility. It only means `786` of the `1044`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `1/1044` row is only partially matching,
and `257/1044` additional rows are stock-compatible invalid inputs. Do
...
| Branching and Merging | `9` | `0` | `581` | `0` | `69` | `40` | `0` | `0` | `29` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1044`** | **`786`** | **`1`** | **`0`** | **`257`** |
