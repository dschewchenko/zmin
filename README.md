...
| Written behavior rows | `915` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `751/915` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/915` | written rows that still do not match stock Git |
| Invalid input rows | `164/915` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `751/915` as Git compatibility. It only means `751` of the `915`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `164/915` additional rows are
...
| Debugging | `3` | `0` | `132` | `0` | `65` | `17` | `0` | `48` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`915`** | **`751`** | **`0`** | **`164`** |
