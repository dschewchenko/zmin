...
| Written behavior rows | `730` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `654/730` | exact written rows with focused parity evidence |
| Open written rows | `0/730` | written rows that still do not match stock Git |
| Invalid input rows | `76/730` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `654/730` as Git compatibility. It only means `654` of the `730`
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `264` | `258` | `0` | `6` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`730`** | **`654`** | **`0`** | **`76`** |
...
| `fetch` | `73` | `0` | `30` | `264` | `258` | `0` | `0` | `6` | no |
