...
| Documented option spellings represented by rows | `558/4632` | option spellings that have at least one behavior row; this is not support |
| Written behavior rows | `2375` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2033/2375` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2375` | written rows with incomplete parity |
| Open written rows | `1/2375` | written rows that still do not match stock Git |
| Invalid input rows | `341/2375` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2033/2375` as Git compatibility. It only means `2033` of the `2375`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2375` rows are partially matching,
`1/2375` rows are known open mismatches, and `341/2375` additional rows are
stock-compatible invalid inputs. Do not read `558/4632` as option support
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `169` | `150` | `0` | `0` | `19` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2375`** | **`2033`** | **`0`** | **`1`** | **`341`** |
