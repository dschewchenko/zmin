...
| Written behavior rows | `2179` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1863/2179` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2179` | written rows with incomplete parity |
| Open written rows | `1/2179` | written rows that still do not match stock Git |
| Invalid input rows | `315/2179` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1863/2179` as Git compatibility. It only means `1863` of the `2179`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2179` rows are partially matching,
`1/2179` rows are known open mismatches, and `315/2179` additional rows are
...
| Branching and Merging | `9` | `0` | `581` | `0` | `122` | `90` | `0` | `0` | `32` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2179`** | **`1863`** | **`0`** | **`1`** | **`315`** |
