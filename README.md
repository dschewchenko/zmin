...
| Written behavior rows | `793` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `717/793` | exact written rows with focused parity evidence |
| Open written rows | `0/793` | written rows that still do not match stock Git |
| Invalid input rows | `76/793` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `717/793` as Git compatibility. It only means `717` of the `793`
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `143` | `140` | `0` | `3` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`793`** | **`717`** | **`0`** | **`76`** |
