...
| Written behavior rows | `800` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `724/800` | exact written rows with focused parity evidence |
| Open written rows | `0/800` | written rows that still do not match stock Git |
| Invalid input rows | `76/800` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `724/800` as Git compatibility. It only means `724` of the `800`
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`800`** | **`724`** | **`0`** | **`76`** |
