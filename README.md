...
| Written behavior rows | `2567` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2192/2567` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2567` | written rows with incomplete parity |
| Open written rows | `1/2567` | written rows that still do not match stock Git |
| Invalid input rows | `374/2567` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2192/2567` as Git compatibility. It only means `2192` of the `2567`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2567` rows are partially matching,
`1/2567` rows are known open mismatches, and `374/2567` additional rows are
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2567`** | **`2192`** | **`0`** | **`1`** | **`374`** |
...
| `fetch` | `73` | `0` | `30` | `321` | `311` | `0` | `0` | `10` | no |
