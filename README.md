...
| Written behavior rows | `1009` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `770/1009` | supported-behavior rows with focused parity evidence |
| Partial written rows | `1/1009` | written rows with incomplete parity |
| Open written rows | `0/1009` | written rows that still do not match stock Git |
| Invalid input rows | `238/1009` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `770/1009` as Git compatibility. It only means `770` of the `1009`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `1/1009` row is only partially matching
stock Git and `238/1009` additional rows are stock-compatible invalid inputs. Do
...
| Branching and Merging | `9` | `0` | `581` | `0` | `60` | `35` | `0` | `0` | `25` |
...
| Other Git `2.47` commands | `71` | `0` | `1075` | `0` | `33` | `7` | `1` | `0` | `25` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1009`** | **`770`** | **`1`** | **`0`** | **`238`** |
