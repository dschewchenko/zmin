...
| Written behavior rows | `785` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `709/785` | exact written rows with focused parity evidence |
| Open written rows | `0/785` | written rows that still do not match stock Git |
| Invalid input rows | `76/785` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `709/785` as Git compatibility. It only means `709` of the `785`
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`785`** | **`709`** | **`0`** | **`76`** |
