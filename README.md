...
| Written behavior rows | `801` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `725/801` | exact written rows with focused parity evidence |
| Open written rows | `0/801` | written rows that still do not match stock Git |
| Invalid input rows | `76/801` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `725/801` as Git compatibility. It only means `725` of the `801`
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `69` | `65` | `0` | `4` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`801`** | **`725`** | **`0`** | **`76`** |
