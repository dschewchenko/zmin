...
| Written behavior rows | `779` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `703/779` | exact written rows with focused parity evidence |
| Open written rows | `0/779` | written rows that still do not match stock Git |
| Invalid input rows | `76/779` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `703/779` as Git compatibility. It only means `703` of the `779`
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `141` | `138` | `0` | `3` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`779`** | **`703`** | **`0`** | **`76`** |
