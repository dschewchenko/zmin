...
| Documented option spellings represented by rows | `253/4632` | option spellings that have at least one behavior row; this is not support |
| Written behavior rows | `1094` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `823/1094` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1094` | written rows with incomplete parity |
| Open written rows | `1/1094` | written rows that still do not match stock Git |
| Invalid input rows | `270/1094` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `823/1094` as Git compatibility. It only means `823` of the `1094`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1094` rows are partially matching,
`1/1094` rows are known open mismatches, and `270/1094` additional rows are
stock-compatible invalid inputs. Do not read `253/4632` as option support
...
| Administration | `8` | `0` | `147` | `0` | `36` | `13` | `0` | `0` | `23` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1094`** | **`823`** | **`0`** | **`1`** | **`270`** |
...
| `filter-branch` | `37` | `0` | `2` | `2` | `2` | `0` | `0` | `0` | no |
