...
| Written behavior rows | `852` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `736/852` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/852` | written rows that still do not match stock Git |
| Invalid input rows | `116/852` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `736/852` as Git compatibility. It only means `736` of the `852`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `116/852` additional rows are
