...
| Written behavior rows | `2189` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1873/2189` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2189` | written rows with incomplete parity |
| Open written rows | `1/2189` | written rows that still do not match stock Git |
| Invalid input rows | `315/2189` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1873/2189` as Git compatibility. It only means `1873` of the `2189`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2189` rows are partially matching,
`1/2189` rows are known open mismatches, and `315/2189` additional rows are
...
| Branching and Merging | `9` | `0` | `581` | `0` | `132` | `100` | `0` | `0` | `32` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2189`** | **`1873`** | **`0`** | **`1`** | **`315`** |
