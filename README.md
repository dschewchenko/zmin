...
| Written behavior rows | `1092` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `822/1092` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1092` | written rows with incomplete parity |
| Open written rows | `1/1092` | written rows that still do not match stock Git |
| Invalid input rows | `269/1092` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `822/1092` as Git compatibility. It only means `822` of the `1092`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1092` rows are partially matching,
`1/1092` rows are known open mismatches, and `269/1092` additional rows are
...
| Getting and Creating Projects | `2` | `0` | `66` | `0` | `8` | `5` | `0` | `1` | `2` |
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `154` | `148` | `0` | `0` | `6` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1092`** | **`822`** | **`0`** | **`1`** | **`269`** |
