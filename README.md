...
| Written behavior rows | `837` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `732/837` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/837` | written rows that still do not match stock Git |
| Invalid input rows | `105/837` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `732/837` as Git compatibility. It only means `732` of the `837`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `105/837` additional rows are
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`837`** | **`732`** | **`0`** | **`105`** |
