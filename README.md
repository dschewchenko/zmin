...
| Written behavior rows | `926` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `755/926` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/926` | written rows that still do not match stock Git |
| Invalid input rows | `171/926` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `755/926` as Git compatibility. It only means `755` of the `926`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `171/926` additional rows are
...
| Debugging | `3` | `0` | `132` | `0` | `76` | `21` | `0` | `55` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`926`** | **`755`** | **`0`** | **`171`** |
