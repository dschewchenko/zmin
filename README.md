...
| Written behavior rows | `210` | rows written in the current matrices |
| Written rows matching stock Git | `195/210` | exact rows with focused parity evidence |
| Open written rows | `9/210` | written rows that still do not match stock Git |
| Invalid input rows | `6/210` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests and real tool traces |

Do not read `195/210` as Git compatibility. It only means `195` of the `210`
...
| Sharing and Updating Projects | `5` | `309` | `150` | `139` | `9` | `2` |
...
| **Git `2.47.1` unique total** | **`151`** | **`4632`** | **`210`** | **`195`** | **`9`** | **`6`** |
