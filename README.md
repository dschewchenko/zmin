...
| Written behavior rows | `222` | rows written in the current matrices |
| Written rows matching stock Git | `207/222` | exact rows with focused parity evidence |
| Open written rows | `9/222` | written rows that still do not match stock Git |
| Invalid input rows | `6/222` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests and real tool traces |

Do not read `207/222` as Git compatibility. It only means `207` of the `222`
...
| Sharing and Updating Projects | `5` | `309` | `162` | `151` | `9` | `2` |
...
| **Git `2.47.1` unique total** | **`151`** | **`4632`** | **`222`** | **`207`** | **`9`** | **`6`** |
