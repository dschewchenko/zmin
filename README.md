...
| Written behavior rows | `205` | rows written in the current matrices |
| Written rows matching stock Git | `190/205` | exact rows with focused parity evidence |
| Open written rows | `9/205` | written rows that still do not match stock Git |
| Invalid input rows | `6/205` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests and real tool traces |

Do not read `190/205` as Git compatibility. It only means `190` of the `205`
...
| Sharing and Updating Projects | `5` | `309` | `145` | `134` | `9` | `2` |
...
| **Git `2.47.1` unique total** | **`151`** | **`4632`** | **`205`** | **`190`** | **`9`** | **`6`** |
