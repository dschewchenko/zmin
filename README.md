...
| Written behavior rows | `792` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `716/792` | exact written rows with focused parity evidence |
| Open written rows | `0/792` | written rows that still do not match stock Git |
| Invalid input rows | `76/792` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `716/792` as Git compatibility. It only means `716` of the `792`
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`792`** | **`716`** | **`0`** | **`76`** |
