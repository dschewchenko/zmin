...
| Written behavior rows | `2290` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1968/2290` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2290` | written rows with incomplete parity |
| Open written rows | `1/2290` | written rows that still do not match stock Git |
| Invalid input rows | `321/2290` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1968/2290` as Git compatibility. It only means `1968` of the `2290`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2290` rows are partially matching,
`1/2290` rows are known open mismatches, and `321/2290` additional rows are
...
| Branching and Merging | `9` | `0` | `581` | `0` | `233` | `195` | `0` | `0` | `38` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2290`** | **`1968`** | **`0`** | **`1`** | **`321`** |
...
| `stash` | `30` | `0` | `0` | `172` | `160` | `0` | `0` | `12` | no |
