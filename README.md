...
| Written behavior rows | `1793` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1490/1793` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1793` | written rows with incomplete parity |
| Open written rows | `1/1793` | written rows that still do not match stock Git |
| Invalid input rows | `302/1793` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1490/1793` as Git compatibility. It only means `1490` of the `1793`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1793` rows are partially matching,
`1/1793` rows are known open mismatches, and `302/1793` additional rows are
...
| Plumbing Commands | `21` | `0` | `650` | `0` | `408` | `343` | `0` | `0` | `65` |
| Other Git 2.47 commands | `70` | `0` | `1069` | `0` | `106` | `72` | `0` | `0` | `34` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1793`** | **`1490`** | **`0`** | **`1`** | **`302`** |
...
| `ls-files` | `42` | `0` | `29` | `152` | `132` | `0` | `0` | `20` | no |
