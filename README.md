...
| Written behavior rows | `809` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `727/809` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/809` | written rows that still do not match stock Git |
| Invalid input rows | `82/809` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `727/809` as Git compatibility. It only means `727` of the `809`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `82/809` additional rows are
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`809`** | **`727`** | **`0`** | **`82`** |
