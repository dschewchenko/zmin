...
| Written behavior rows | `845` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `732/845` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/845` | written rows that still do not match stock Git |
| Invalid input rows | `113/845` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `732/845` as Git compatibility. It only means `732` of the `845`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `113/845` additional rows are
...
| Plumbing Commands | `20` | `0` | `644` | `0` | `181` | `144` | `0` | `37` |
| Other Git `2.47` commands | `71` | `0` | `1075` | `0` | `6` | `2` | `0` | `4` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`845`** | **`732`** | **`0`** | **`113`** |
...
| `ls-files` | `42` | `0` | `27` | `63` | `51` | `0` | `0` | `12` | no |
