...
| Documented option spellings represented by rows | `230/4632` | option spellings that have at least one behavior row; this is not support |
| Written behavior rows | `831` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `731/831` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/831` | written rows that still do not match stock Git |
| Invalid input rows | `100/831` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `731/831` as Git compatibility. It only means `731` of the `831`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `100/831` additional rows are
stock-compatible invalid inputs. Do not read `230/4632` as option support
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`831`** | **`731`** | **`0`** | **`100`** |
