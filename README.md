...
| Written behavior rows | `794` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `718/794` | exact written rows with focused parity evidence |
| Open written rows | `0/794` | written rows that still do not match stock Git |
| Invalid input rows | `76/794` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `718/794` as Git compatibility. It only means `718` of the `794`
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `144` | `141` | `0` | `3` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`794`** | **`718`** | **`0`** | **`76`** |
