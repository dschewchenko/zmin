...
| Written behavior rows | `946` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `755/946` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/946` | written rows that still do not match stock Git |
| Invalid input rows | `191/946` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `755/946` as Git compatibility. It only means `755` of the `946`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `191/946` additional rows are
...
| Debugging | `3` | `0` | `132` | `0` | `96` | `21` | `0` | `75` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`946`** | **`755`** | **`0`** | **`191`** |
