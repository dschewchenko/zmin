...
| Written behavior rows | `843` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `732/843` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/843` | written rows that still do not match stock Git |
| Invalid input rows | `111/843` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `732/843` as Git compatibility. It only means `732` of the `843`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `111/843` additional rows are
...
| Plumbing Commands | `20` | `0` | `644` | `0` | `179` | `144` | `0` | `35` |
| Other Git `2.47` commands | `71` | `0` | `1075` | `0` | `6` | `2` | `0` | `4` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`843`** | **`732`** | **`0`** | **`111`** |
...
| `ls-files` | `42` | `0` | `27` | `61` | `51` | `0` | `0` | `10` | no |
