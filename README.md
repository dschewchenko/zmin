...
| Documented option spellings represented by rows | `579/4632` | option spellings that have at least one behavior row; this is not support |
| Written behavior rows | `2480` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2129/2480` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2480` | written rows with incomplete parity |
| Open written rows | `1/2480` | written rows that still do not match stock Git |
| Invalid input rows | `350/2480` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2129/2480` as Git compatibility. It only means `2129` of the `2480`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2480` rows are partially matching,
`1/2480` rows are known open mismatches, and `350/2480` additional rows are
stock-compatible invalid inputs. Do not read `579/4632` as option support
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `274` | `246` | `0` | `0` | `28` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2480`** | **`2129`** | **`0`** | **`1`** | **`350`** |
