...
| Written behavior rows | `1089` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `820/1089` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1089` | written rows with incomplete parity |
| Open written rows | `1/1089` | written rows that still do not match stock Git |
| Invalid input rows | `268/1089` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `820/1089` as Git compatibility. It only means `820` of the `1089`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1089` rows are partially matching,
`1/1089` rows are known open mismatches, and `268/1089` additional rows are
...
| Branching and Merging | `9` | `0` | `581` | `0` | `85` | `55` | `0` | `0` | `30` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1089`** | **`820`** | **`0`** | **`1`** | **`268`** |
