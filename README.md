...
| Written behavior rows | `2209` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1893/2209` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2209` | written rows with incomplete parity |
| Open written rows | `1/2209` | written rows that still do not match stock Git |
| Invalid input rows | `315/2209` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1893/2209` as Git compatibility. It only means `1893` of the `2209`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2209` rows are partially matching,
`1/2209` rows are known open mismatches, and `315/2209` additional rows are
...
| Branching and Merging | `9` | `0` | `581` | `0` | `152` | `120` | `0` | `0` | `32` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2209`** | **`1893`** | **`0`** | **`1`** | **`315`** |
