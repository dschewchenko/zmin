...
| Written behavior rows | `201` | rows written in the current matrices |
| Written rows matching stock Git | `186/201` | exact rows with focused parity evidence |
| Open written rows | `9/201` | written rows that still do not match stock Git |
| Invalid input rows | `6/201` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests and real tool traces |

Do not read `186/201` as Git compatibility. It only means `186` of the `201`
...
| Sharing and Updating Projects | `5` | `309` | `141` | `130` | `9` | `2` |
...
| **Git `2.47.1` unique total** | **`151`** | **`4632`** | **`201`** | **`186`** | **`9`** | **`6`** |
