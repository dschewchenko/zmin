...
| Written behavior rows | `753` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `677/753` | exact written rows with focused parity evidence |
| Open written rows | `0/753` | written rows that still do not match stock Git |
| Invalid input rows | `76/753` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `677/753` as Git compatibility. It only means `677` of the `753`
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`753`** | **`677`** | **`0`** | **`76`** |
