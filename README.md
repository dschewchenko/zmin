...
| Documented option spellings represented by rows | `549/4632` | option spellings that have at least one behavior row; this is not support |
| Written behavior rows | `2149` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1837/2149` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2149` | written rows with incomplete parity |
| Open written rows | `1/2149` | written rows that still do not match stock Git |
| Invalid input rows | `311/2149` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1837/2149` as Git compatibility. It only means `1837` of the `2149`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2149` rows are partially matching,
`1/2149` rows are known open mismatches, and `311/2149` additional rows are
stock-compatible invalid inputs. Do not read `549/4632` as option support
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `400` | `391` | `0` | `0` | `9` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2149`** | **`1837`** | **`0`** | **`1`** | **`311`** |
