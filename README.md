...
| Written behavior rows | `2279` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1957/2279` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2279` | written rows with incomplete parity |
| Open written rows | `1/2279` | written rows that still do not match stock Git |
| Invalid input rows | `321/2279` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1957/2279` as Git compatibility. It only means `1957` of the `2279`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2279` rows are partially matching,
`1/2279` rows are known open mismatches, and `321/2279` additional rows are
...
| Branching and Merging | `9` | `0` | `581` | `0` | `222` | `184` | `0` | `0` | `38` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2279`** | **`1957`** | **`0`** | **`1`** | **`321`** |
...
| `stash` | `30` | `0` | `0` | `161` | `149` | `0` | `0` | `12` | no |
