...
| Written behavior rows | `949` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `755/949` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/949` | written rows that still do not match stock Git |
| Invalid input rows | `194/949` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `755/949` as Git compatibility. It only means `755` of the `949`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `194/949` additional rows are
...
| Debugging | `3` | `0` | `132` | `0` | `99` | `21` | `0` | `78` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`949`** | **`755`** | **`0`** | **`194`** |
