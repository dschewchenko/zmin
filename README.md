...
| Written behavior rows | `739` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `663/739` | exact written rows with focused parity evidence |
| Open written rows | `0/739` | written rows that still do not match stock Git |
| Invalid input rows | `76/739` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `663/739` as Git compatibility. It only means `663` of the `739`
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `273` | `267` | `0` | `6` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`739`** | **`663`** | **`0`** | **`76`** |
...
| `fetch` | `73` | `0` | `30` | `273` | `267` | `0` | `0` | `6` | no |
