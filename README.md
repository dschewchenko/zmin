...
| Written behavior rows | `846` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `732/846` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/846` | written rows that still do not match stock Git |
| Invalid input rows | `114/846` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `732/846` as Git compatibility. It only means `732` of the `846`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `114/846` additional rows are
...
| Plumbing Commands | `20` | `0` | `644` | `0` | `182` | `144` | `0` | `38` |
| Other Git `2.47` commands | `71` | `0` | `1075` | `0` | `6` | `2` | `0` | `4` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`846`** | **`732`** | **`0`** | **`114`** |
...
| `ls-files` | `42` | `0` | `27` | `64` | `51` | `0` | `0` | `13` | no |
