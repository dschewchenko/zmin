...
| Written behavior rows | `1055` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `790/1055` | supported-behavior rows with focused parity evidence |
| Partial written rows | `4/1055` | written rows with incomplete parity |
| Open written rows | `0/1055` | written rows that still do not match stock Git |
| Invalid input rows | `261/1055` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `790/1055` as Git compatibility. It only means `790` of the `1055`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `4/1055` rows are only partially matching,
and `261/1055` additional rows are stock-compatible invalid inputs. Do
...
| Other Git `2.47` commands | `71` | `0` | `1075` | `0` | `48` | `17` | `4` | `0` | `27` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1055`** | **`790`** | **`4`** | **`0`** | **`261`** |
...
| `fast-import` | `25` | `0` | `1` | `7` | `0` | `4` | `0` | `3` | no |
