...
| Written behavior rows | `795` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `719/795` | exact written rows with focused parity evidence |
| Open written rows | `0/795` | written rows that still do not match stock Git |
| Invalid input rows | `76/795` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `719/795` as Git compatibility. It only means `719` of the `795`
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `145` | `142` | `0` | `3` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`795`** | **`719`** | **`0`** | **`76`** |
