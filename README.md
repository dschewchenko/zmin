...
| Written behavior rows | `777` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `701/777` | exact written rows with focused parity evidence |
| Open written rows | `0/777` | written rows that still do not match stock Git |
| Invalid input rows | `76/777` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `701/777` as Git compatibility. It only means `701` of the `777`
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`777`** | **`701`** | **`0`** | **`76`** |
