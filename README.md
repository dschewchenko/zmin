...
| Documented option spellings represented by rows | `229/4632` | option spellings that have at least one behavior row; this is not support |
| Written behavior rows | `830` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `730/830` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/830` | written rows that still do not match stock Git |
| Invalid input rows | `100/830` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `730/830` as Git compatibility. It only means `730` of the `830`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `100/830` additional rows are
stock-compatible invalid inputs. Do not read `229/4632` as option support
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`830`** | **`730`** | **`0`** | **`100`** |
...
`archive`, `bisect`, `blame`, `branch`, `cat-file`, `column`, `config`, `status`, `fetch`, `diff`, `log`, `for-each-ref`, `ls-files`,
