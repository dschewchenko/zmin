...
| Written behavior rows | `913` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `750/913` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/913` | written rows that still do not match stock Git |
| Invalid input rows | `163/913` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `750/913` as Git compatibility. It only means `750` of the `913`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `163/913` additional rows are
...
| Debugging | `3` | `0` | `132` | `0` | `63` | `16` | `0` | `47` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`913`** | **`750`** | **`0`** | **`163`** |
