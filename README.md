...
| Written behavior rows | `829` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `729/829` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/829` | written rows that still do not match stock Git |
| Invalid input rows | `100/829` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `729/829` as Git compatibility. It only means `729` of the `829`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `100/829` additional rows are
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`829`** | **`729`** | **`0`** | **`100`** |
