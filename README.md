...
| Written behavior rows | `1075` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `811/1075` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1075` | written rows with incomplete parity |
| Open written rows | `1/1075` | written rows that still do not match stock Git |
| Invalid input rows | `263/1075` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `811/1075` as Git compatibility. It only means `811` of the `1075`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1075` rows are partially matching,
`1/1075` rows are known open mismatches, and `263/1075` additional rows are
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1075`** | **`811`** | **`0`** | **`1`** | **`263`** |
