...
| Written behavior rows | `1008` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `770/1008` | supported-behavior rows with focused parity evidence |
| Partial written rows | `1/1008` | written rows with incomplete parity |
| Open written rows | `0/1008` | written rows that still do not match stock Git |
| Invalid input rows | `237/1008` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `770/1008` as Git compatibility. It only means `770` of the `1008`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `1/1008` row is only partially matching
stock Git and `237/1008` additional rows are stock-compatible invalid inputs. Do
...
| Other Git `2.47` commands | `71` | `0` | `1075` | `0` | `32` | `7` | `1` | `0` | `24` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1008`** | **`770`** | **`1`** | **`0`** | **`237`** |
