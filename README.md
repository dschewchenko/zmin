...
| Written behavior rows | `999` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `770/999` | supported-behavior rows with focused parity evidence |
| Partial written rows | `1/999` | written rows with incomplete parity |
| Open written rows | `0/999` | written rows that still do not match stock Git |
| Invalid input rows | `228/999` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `770/999` as Git compatibility. It only means `770` of the `999`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `1/999` row is only partially matching
stock Git and `228/999` additional rows are stock-compatible invalid inputs. Do
...
| Other Git `2.47` commands | `71` | `0` | `1075` | `0` | `25` | `7` | `1` | `0` | `17` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`999`** | **`770`** | **`1`** | **`0`** | **`228`** |
...
| `fast-import` | `25` | `0` | `1` | `1` | `0` | `1` | `0` | `0` | no |
