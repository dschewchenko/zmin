...
| Written behavior rows | `2420` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2075/2420` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2420` | written rows with incomplete parity |
| Open written rows | `1/2420` | written rows that still do not match stock Git |
| Invalid input rows | `344/2420` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2075/2420` as Git compatibility. It only means `2075` of the `2420`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2420` rows are partially matching,
`1/2420` rows are known open mismatches, and `344/2420` additional rows are
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `214` | `192` | `0` | `0` | `22` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2420`** | **`2075`** | **`0`** | **`1`** | **`344`** |
