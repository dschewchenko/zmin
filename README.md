...
| Written behavior rows | `725` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `649/725` | exact written rows with focused parity evidence |
| Open written rows | `0/725` | written rows that still do not match stock Git |
| Invalid input rows | `76/725` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `649/725` as Git compatibility. It only means `649` of the `725`
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `259` | `253` | `0` | `6` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`725`** | **`649`** | **`0`** | **`76`** |
...
| `fetch` | `73` | `0` | `30` | `259` | `253` | `0` | `0` | `6` | no |
