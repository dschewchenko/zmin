...
| Written behavior rows | `2312` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1986/2312` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2312` | written rows with incomplete parity |
| Open written rows | `1/2312` | written rows that still do not match stock Git |
| Invalid input rows | `325/2312` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1986/2312` as Git compatibility. It only means `1986` of the `2312`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2312` rows are partially matching,
`1/2312` rows are known open mismatches, and `325/2312` additional rows are
...
| Branching and Merging | `9` | `0` | `581` | `0` | `255` | `213` | `0` | `0` | `42` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2312`** | **`1986`** | **`0`** | **`1`** | **`325`** |
