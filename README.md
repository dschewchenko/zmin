...
| Written behavior rows | `1084` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `818/1084` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1084` | written rows with incomplete parity |
| Open written rows | `1/1084` | written rows that still do not match stock Git |
| Invalid input rows | `265/1084` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `818/1084` as Git compatibility. It only means `818` of the `1084`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1084` rows are partially matching,
`1/1084` rows are known open mismatches, and `265/1084` additional rows are
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `308` | `290` | `0` | `0` | `18` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1084`** | **`818`** | **`0`** | **`1`** | **`265`** |
...
| `fetch` | `73` | `0` | `30` | `299` | `290` | `0` | `0` | `9` | no |
