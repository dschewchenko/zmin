...
| Written behavior rows | `761` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `685/761` | exact written rows with focused parity evidence |
| Open written rows | `0/761` | written rows that still do not match stock Git |
| Invalid input rows | `76/761` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `685/761` as Git compatibility. It only means `685` of the `761`
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `138` | `135` | `0` | `3` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`761`** | **`685`** | **`0`** | **`76`** |
