...
| Written behavior rows | `1761` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1460/1761` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1761` | written rows with incomplete parity |
| Open written rows | `1/1761` | written rows that still do not match stock Git |
| Invalid input rows | `300/1761` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1460/1761` as Git compatibility. It only means `1460` of the `1761`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1761` rows are partially matching,
`1/1761` rows are known open mismatches, and `300/1761` additional rows are
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1761`** | **`1460`** | **`0`** | **`1`** | **`300`** |
