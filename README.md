...
| Written behavior rows | `755` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `679/755` | exact written rows with focused parity evidence |
| Open written rows | `0/755` | written rows that still do not match stock Git |
| Invalid input rows | `76/755` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `679/755` as Git compatibility. It only means `679` of the `755`
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `136` | `133` | `0` | `3` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`755`** | **`679`** | **`0`** | **`76`** |
