...
| Documented option spellings represented by rows | `586/4632` | option spellings that have at least one behavior row; this is not support |
| Written behavior rows | `2498` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2144/2498` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2498` | written rows with incomplete parity |
| Open written rows | `1/2498` | written rows that still do not match stock Git |
| Invalid input rows | `353/2498` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2144/2498` as Git compatibility. It only means `2144` of the `2498`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2498` rows are partially matching,
`1/2498` rows are known open mismatches, and `353/2498` additional rows are
stock-compatible invalid inputs. Do not read `586/4632` as option support
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `403` | `391` | `0` | `0` | `9` |
...
| Administration | `8` | `0` | `147` | `0` | `105` | `65` | `0` | `0` | `40` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2498`** | **`2144`** | **`0`** | **`1`** | **`353`** |
