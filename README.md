...
| Written behavior rows | `2504` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `2144/2504` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2504` | written rows with incomplete parity |
| Open written rows | `1/2504` | written rows that still do not match stock Git |
| Invalid input rows | `359/2504` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `2144/2504` as Git compatibility. It only means `2144` of the `2504`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2504` rows are partially matching,
`1/2504` rows are known open mismatches, and `359/2504` additional rows are
...
| Administration | `8` | `0` | `147` | `0` | `111` | `65` | `0` | `0` | `46` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2504`** | **`2144`** | **`0`** | **`1`** | **`359`** |
