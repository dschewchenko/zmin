...
| Commands with any matrix rows | `66/151` | commands that have started behavior matrices; this is not support |
| Documented option spellings represented by rows | `403/4632` | option spellings that have at least one behavior row; this is not support |
| Written behavior rows | `1676` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1377/1676` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1676` | written rows with incomplete parity |
| Open written rows | `1/1676` | written rows that still do not match stock Git |
| Invalid input rows | `298/1676` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1377/1676` as Git compatibility. It only means `1377` of the `1676`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1676` rows are partially matching,
`1/1676` rows are known open mismatches, and `298/1676` additional rows are
stock-compatible invalid inputs. Do not read `403/4632` as option support
...
| Other Git 2.47 commands | `70` | `0` | `1069` | `0` | `71` | `38` | `0` | `0` | `33` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1676`** | **`1377`** | **`0`** | **`1`** | **`298`** |
