...
| Written behavior rows | `844` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `732/844` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/844` | written rows that still do not match stock Git |
| Invalid input rows | `112/844` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `732/844` as Git compatibility. It only means `732` of the `844`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `112/844` additional rows are
...
| Plumbing Commands | `20` | `0` | `644` | `0` | `180` | `144` | `0` | `36` |
| Other Git `2.47` commands | `71` | `0` | `1075` | `0` | `6` | `2` | `0` | `4` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`844`** | **`732`** | **`0`** | **`112`** |
...
| `ls-files` | `42` | `0` | `27` | `62` | `51` | `0` | `0` | `11` | no |
