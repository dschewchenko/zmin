...
| Written behavior rows | `918` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `753/918` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/918` | written rows that still do not match stock Git |
| Invalid input rows | `165/918` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `753/918` as Git compatibility. It only means `753` of the `918`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `165/918` additional rows are
...
| Debugging | `3` | `0` | `132` | `0` | `68` | `19` | `0` | `49` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`918`** | **`753`** | **`0`** | **`165`** |
