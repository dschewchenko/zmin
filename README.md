...
| Written behavior rows | `2004` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1693/2004` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/2004` | written rows with incomplete parity |
| Open written rows | `1/2004` | written rows that still do not match stock Git |
| Invalid input rows | `310/2004` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1693/2004` as Git compatibility. It only means `1693` of the `2004`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/2004` rows are partially matching,
`1/2004` rows are known open mismatches, and `310/2004` additional rows are
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2004`** | **`1693`** | **`0`** | **`1`** | **`310`** |
