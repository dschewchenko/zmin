...
| Written behavior rows | `1018` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `778/1018` | supported-behavior rows with focused parity evidence |
| Partial written rows | `1/1018` | written rows with incomplete parity |
| Open written rows | `0/1018` | written rows that still do not match stock Git |
| Invalid input rows | `239/1018` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `778/1018` as Git compatibility. It only means `778` of the `1018`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `1/1018` row is only partially matching
stock Git and `239/1018` additional rows are stock-compatible invalid inputs. Do
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1018`** | **`778`** | **`1`** | **`0`** | **`239`** |
