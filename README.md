...
| Written behavior rows | `1020` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `778/1020` | supported-behavior rows with focused parity evidence |
| Partial written rows | `1/1020` | written rows with incomplete parity |
| Open written rows | `0/1020` | written rows that still do not match stock Git |
| Invalid input rows | `241/1020` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `778/1020` as Git compatibility. It only means `778` of the `1020`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `1/1020` row is only partially matching
stock Git and `241/1020` additional rows are stock-compatible invalid inputs. Do
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `152` | `147` | `0` | `0` | `5` |
...
| Plumbing Commands | `20` | `0` | `644` | `0` | `197` | `151` | `0` | `0` | `46` |
| Other Git `2.47` commands | `71` | `0` | `1075` | `0` | `36` | `10` | `1` | `0` | `25` |
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1020`** | **`778`** | **`1`** | **`0`** | **`241`** |
