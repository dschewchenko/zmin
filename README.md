...
| Written behavior rows | `751` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `675/751` | exact written rows with focused parity evidence |
| Open written rows | `0/751` | written rows that still do not match stock Git |
| Invalid input rows | `76/751` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `675/751` as Git compatibility. It only means `675` of the `751`
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`751`** | **`675`** | **`0`** | **`76`** |
