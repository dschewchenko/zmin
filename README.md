...
| Commands with any matrix rows | `18/151` | `archive`, `branch`, `column`, `config`, `status`, `fetch`, `diff`, `log`, `for-each-ref`, `ls-files`, `ls-tree`, `merge-base`, `rerere`, `rev-parse`, `show-ref`, `symbolic-ref`, `tag` and `version` have started behavior matrices |
...
| Written behavior rows | `810` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `727/810` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/810` | written rows that still do not match stock Git |
| Invalid input rows | `83/810` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `727/810` as Git compatibility. It only means `727` of the `810`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `83/810` additional rows are
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `70` | `66` | `0` | `4` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`810`** | **`727`** | **`0`** | **`83`** |
...
`ls-tree`, `merge-base`, `rerere`, `rev-parse`, `show-ref`, `symbolic-ref`,
`tag` or `version`
