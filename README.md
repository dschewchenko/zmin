...
| Written behavior rows | `937` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `755/937` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/937` | written rows that still do not match stock Git |
| Invalid input rows | `182/937` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `755/937` as Git compatibility. It only means `755` of the `937`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `182/937` additional rows are
...
| Debugging | `3` | `0` | `132` | `0` | `87` | `21` | `0` | `66` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`937`** | **`755`** | **`0`** | **`182`** |
