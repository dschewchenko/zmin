...
| Written behavior rows | `797` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `721/797` | exact written rows with focused parity evidence |
| Open written rows | `0/797` | written rows that still do not match stock Git |
| Invalid input rows | `76/797` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `721/797` as Git compatibility. It only means `721` of the `797`
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `147` | `144` | `0` | `3` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`797`** | **`721`** | **`0`** | **`76`** |
