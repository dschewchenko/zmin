...
| Written behavior rows | `885` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `742/885` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/885` | written rows that still do not match stock Git |
| Invalid input rows | `143/885` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `742/885` as Git compatibility. It only means `742` of the `885`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `143/885` additional rows are
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`885`** | **`742`** | **`0`** | **`143`** |
