...
| Written behavior rows | `841` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `732/841` | supported-behavior rows with focused parity evidence |
| Open written rows | `0/841` | written rows that still do not match stock Git |
| Invalid input rows | `109/841` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `732/841` as Git compatibility. It only means `732` of the `841`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, and `109/841` additional rows are
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`841`** | **`732`** | **`0`** | **`109`** |
