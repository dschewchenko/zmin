...
| Documented option spellings represented by rows | `499/4632` | option spellings that have at least one behavior row; this is not support |
| Written behavior rows | `1975` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `1664/1975` | supported-behavior rows with focused parity evidence |
| Partial written rows | `0/1975` | written rows with incomplete parity |
| Open written rows | `1/1975` | written rows that still do not match stock Git |
| Invalid input rows | `310/1975` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `1664/1975` as Git compatibility. It only means `1664` of the `1975`
rows already written down are closed supported-behavior rows. The larger
unexpanded surface is not counted yet, `0/1975` rows are partially matching,
`1/1975` rows are known open mismatches, and `310/1975` additional rows are
stock-compatible invalid inputs. Do not read `499/4632` as option support
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `346` | `338` | `0` | `0` | `8` |
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1975`** | **`1664`** | **`0`** | **`1`** | **`310`** |
