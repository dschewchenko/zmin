...
| Written behavior rows | `756` | explicit rows currently written in command matrices |
| Written rows matching stock Git | `680/756` | exact written rows with focused parity evidence |
| Open written rows | `0/756` | written rows that still do not match stock Git |
| Invalid input rows | `76/756` | rows where stock Git rejects the input |
| Full Git behavior denominator | not known yet | still being expanded from docs, upstream tests, IDE traces and platform checks |

Do not read `680/756` as Git compatibility. It only means `680` of the `756`
...
| **Git `2.47.1` unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`756`** | **`680`** | **`0`** | **`76`** |
