...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 48/151 commands with matrix rows / 252/4632 represented doc-option pairs / 1081 written rows / 816 written rows matching stock Git / 0 partial written rows / 1 open written rows`
...
The latest completed slice is `fetch --tags` with a named local remote
branch fetch:

`git fetch --tags origin main`

Stock Git accepts this option combination, writes branch and tag rows to
`FETCH_HEAD`, follows annotated and lightweight tags into `refs/tags/*`, emits
the branch, tag and remote-tracking update lines on stderr, updates
`refs/remotes/origin/main` and exits successfully. Zmin now matches those side
effects for named local remote branch fetches. The row is recorded in
`docs/cli/matrices/fetch_v2_47.tsv` with focused evidence in
`git_transport_local_compat::fetch_tags_named_local_remote_branch_follows_tags_like_stock_git`.
...
This card is the exact handoff target after the current `1081` written-row
...
`0/1081`; the `1/1081` open row and the still incomplete command/doc-option
