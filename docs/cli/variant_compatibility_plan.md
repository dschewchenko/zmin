...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 15/151 commands with matrix rows / 223/4632 represented doc-option pairs / 768 written rows / 692 written rows matching stock Git / 0 open written rows`
...
`git rev-parse --show-toplevel` through the temporary `git` shim on a cloned
repository root.
...
1. Add a closed row to `docs/cli/matrices/rev_parse_v2_47.tsv` with evidence
   `tools/git-replacement-dogfood-smoke.sh::rev_parse_toplevel`.
...
Tracked closed blocks in this table: `436` verified variants.
