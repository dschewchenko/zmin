...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 46/151 commands with matrix rows / 247/4632 represented doc-option pairs / 1054 written rows / 790 written rows matching stock Git / 3 partial written rows / 0 open written rows`
...
The latest partial behavior row is top-level `fast-import` `progress`.
Stock Git accepts `progress <message>`, prints the progress line to stdout and
continues the import. Zmin now matches that stdout and imports matching tree
content, but the row stays `partial` because stock Git also prints
`fast-import statistics:` to stderr and Zmin does not yet match that output.

The adjacent partial behavior rows are top-level `fast-import` `checkpoint`
and `fast-import --date-format=rfc2822`. Zmin accepts/imports those shapes, but
they stay `partial` for the same fast-import statistics stderr gap.
...
This card is the exact handoff target after the current `1054` written-row
...
`0/1054`; the complete command matrices and complete doc-option matrices remain
`0/151` and `0/4632`.
