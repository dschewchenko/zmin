...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 95/151 commands with matrix rows / 532/4632 represented doc-option pairs / 2065 written rows / 1754/2065 written rows matching stock Git / 0 partial written rows / 1 open written rows`
...
`git blame --show-stats a.txt`

`blame_v2_47.tsv` now records ten more already-tested supported rows for local
date output, a one-line numeric range, whitespace ignore, root attribution,
display toggles `-b`, `-c`, `-s` and `-t`, stats output and intra-file move
detection. Existing `git_history_query_compat` evidence compares stock Git and
Zmin output for those simple linear-history cases. This is an evidence import
...
This card is the exact handoff target after the current `2065` written-row
...
`0/2065`; the `1/2065` open row and the still incomplete command/doc-option
