...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 96/151 commands with matrix rows / 550/4632 represented doc-option pairs / 2261 written rows / 1945/2261 written rows matching stock Git / 0 partial written rows / 1 open written rows`
...
The latest completed slice imports the `git_stash_compat.rs` stash reference
`--quiet --no-quiet` backlog rows from
`docs/cli/existing_oracle_test_inventory.tsv`.

`stash_v2_47.tsv` now records already-tested `stash apply`, `stash pop` and
`stash drop` rows where `--no-quiet` follows `--quiet`. The focused
`git_stash_compat.rs` evidence covers stdout, stderr, exit status and stash
stack/worktree side effects against stock Git. The oracle inventory reduced
total `missing_or_unclassified` to `520`. No Rust behavior changed.
...
This card is the exact handoff target after the current `2261` written-row
...
`0/2261`; the `1/2261` open row and the still incomplete command/doc-option
