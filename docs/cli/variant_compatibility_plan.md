...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 26/151 commands with matrix rows / 236/4632 represented doc-option pairs / 918 written rows / 753 written rows matching stock Git / 0 open written rows`
...
The latest closed guard classification is `git blame -L /x\(y\)/ a.txt`,
`git blame -L /x\(y\)*/ a.txt`, and `git blame -L /q\(r\)/ a.txt` for Git
basic-regex escaped grouping behavior.
...
`0/918`; the complete command matrices and complete doc-option matrices remain
`0/151` and `0/4632`.
