...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 55/151 commands with matrix rows / 350/4632 represented doc-option pairs / 1472 written rows / 1180 written rows matching stock Git / 0 partial written rows / 1 open written rows`
...
The latest completed slice extends the `config` command matrix with include
and onbranch rows from existing stock-oracle evidence:

`git config --show-scope --show-origin --get demo.conditional`

`docs/cli/matrices/config_v2_47.tsv` now records plain include lookup,
scoped/origin includeIf lookup and separate onbranch main and feature wildcard
rows from `git_admin_tools_compat::config_include_origin_and_scope_match_stock_git`.
...
covered by `git_admin_tools_compat::config_include_origin_and_scope_match_stock_git`:
plain include lookup, scoped/origin includeIf lookup and separate onbranch main
and feature wildcard rows. No Rust behavior changed.

### Current Slice Card

This card is the exact handoff target after the current `1472` written-row
...
`0/1472`; the `1/1472` open row and the still incomplete command/doc-option
