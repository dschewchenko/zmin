...
`82/151 complete command matrices / 1252/3175 complete doc-option matrices / 155/151 commands with matrix rows / 1386/3175 represented doc-option pairs / 5332 written rows / 4624/5332 written rows matching stock Git / 0 partial written rows / 26 open written rows`
...
After the latest zero-code `revert` doc-option promotion, `revert`
adds another `4` durable reviewed-complete documented option pairs without
changing Rust behavior and is up to `4/19` documented options complete with
`4/19` represented documented option pairs and `7/7` classified rows.
...
promotion cluster for the represented helper-free `revert` surface. The
existing exact stock-Git evidence already closed four `revert` option
families across local merge-mainline and no-commit lanes, so this slice
only promotes those
...
`tools/git-compat-command-summary.sh --tsv | rg '^(revert|summary)\t'`,
and `git diff --check`.
...
matrices, `1252/3175` complete documented option pairs, and `1949`
remaining checklist rows. `revert` now reads `19` documented option
...
Current durable census after the zero-code reviewed-complete `revert`
documented-option promotion cluster:

- complete command matrices: `82 / 151`
- complete documented command-option pairs: `1252 / 3175`
- matrix rows: `5332`
- verified rows: `4624`
...
- `revert` now has four represented helper-free local documented option
  families promoted into the reviewed-complete doc-option census list,
  bringing the command to `4/19` reviewed-complete documented option pairs.
- The slice adds no new behavior rows; it only makes already closed exact
  stock-Git local merge-mainline and no-commit evidence durable in the
  reviewed doc-option source list.
- Focused verification was
  `python3 tools/git-compat-census.py --root .`,
  `tools/git-cli-readiness-status.sh`,
  and `tools/git-compat-command-summary.sh --tsv | rg '^(revert|summary)\t'`.

Latest in-progress family follow-up:

- `revert` now has `4/19` documented option pairs reviewed complete with
  `4/19` represented documented option pairs, `7/7` classified rows, and
