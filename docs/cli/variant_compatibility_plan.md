...
`82/151 complete command matrices / 1248/3175 complete doc-option matrices / 155/151 commands with matrix rows / 1386/3175 represented doc-option pairs / 5332 written rows / 4624/5332 written rows matching stock Git / 0 partial written rows / 26 open written rows`
...
After the latest zero-code `send-pack` doc-option promotion, `send-pack`
adds another `7` durable reviewed-complete documented option pairs without
changing Rust behavior and is up to `8/12` documented options complete with
`8/12` represented documented option pairs and `16/16` classified rows.
...
promotion cluster for the represented helper-free `send-pack` surface. The
existing exact stock-Git evidence already closed seven `send-pack` option
families across local default push, mirror sync, receive-pack override, stdin
ref, atomic rejection, and dry-run/force/verbose/thin lanes, so this slice
only promotes those
...
`tools/git-compat-command-summary.sh --tsv | rg '^(send-pack|summary)\t'`,
and `git diff --check`.
...
matrices, `1248/3175` complete documented option pairs, and `1953`
remaining checklist rows. `send-pack` now reads `12` documented option
pairs with `8` reviewed complete, `8` represented documented option pairs,
and `16/16` classified written rows with `0` open.
...
Current durable census after the zero-code reviewed-complete `send-pack`
documented-option promotion cluster:

- complete command matrices: `82 / 151`
- complete documented command-option pairs: `1248 / 3175`
- matrix rows: `5332`
- verified rows: `4624`
...
- `send-pack` now has seven additional represented helper-free local
  documented option families promoted into the reviewed-complete doc-option
  census list, bringing the command to `8/12` reviewed-complete documented
  option pairs.
- The slice adds no new behavior rows; it only makes already closed exact
  stock-Git local default push, mirror sync, receive-pack override, stdin
  ref, atomic rejection, and dry-run/force/verbose/thin evidence durable in
  the reviewed doc-option source list.
- Focused verification was
  `python3 tools/git-compat-census.py --root .`,
  `tools/git-cli-readiness-status.sh`,
  and `tools/git-compat-command-summary.sh --tsv | rg '^(send-pack|summary)\t'`.

Latest in-progress family follow-up:

- `send-pack` now has `8/12` documented option pairs reviewed complete with
  `8/12` represented documented option pairs, `16/16` classified rows, and
