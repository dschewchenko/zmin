...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 26/151 commands with matrix rows / 236/4632 represented doc-option pairs / 921 written rows / 755 written rows matching stock Git / 0 open written rows`
...
The latest closed guard classification is `git blame -L /x\{2\}/ a.txt`,
`git blame -L /x\{2,3\}/ a.txt`, and `git blame -L /q\{2\}/ a.txt` for Git
basic-regex escaped interval behavior.
...
`0/921`; the complete command matrices and complete doc-option matrices remain
`0/151` and `0/4632`.
