...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 50/151 commands with matrix rows / 269/4632 represented doc-option pairs / 1114 written rows / 843 written rows matching stock Git / 0 partial written rows / 1 open written rows`
...
The latest matrix inventory slice records eight supported local `clone` rows
already covered by `git_clone_compat::clone_local_repo_matches_stock_git_state`:
default local path clone, `--quiet`, `--local`, `--no-local`,
`--no-hardlinks`, `--shared`, repeated `-c` config and `--template`. No Rust
behavior changed; the slice imports existing stock-oracle evidence into
`clone_v2_47.tsv`.

### Current Slice Card

This card is the exact handoff target after the current `1114` written-row
...
`0/1114`; the `1/1114` open row and the still incomplete command/doc-option
...
| `clone` local path options | `8` | `0` | default local clone, `--quiet`, `--local`, `--no-local`, `--no-hardlinks`, `--shared`, repeated `-c` config and `--template` forms already covered by `git_clone_compat::clone_local_repo_matches_stock_git_state` |
