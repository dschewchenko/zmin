...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 29/151 commands with matrix rows / 238/4632 represented doc-option pairs / 964 written rows / 760 written rows matching stock Git / 0 open written rows`
...
The latest closed guard classification is `git index-pack --verify` with an
unsupported pack reverse-index version. Stock Git rejects a checksum-valid
`.rev` with version `2` as `fatal: sha1 file '<rev>' validation error`, and
Zmin already matches that exit/stderr behavior through the reverse-index
validation mapper.
...
This card is the exact handoff target after the current `964` written-row
...
| Implementation area | start at `crates/zmin-git-core/src/pack.rs:5395` (`unsupported pack file version`) unless a new WebStorm replacement trace appears first |
...
`0/964`; the complete command matrices and complete doc-option matrices remain
`0/151` and `0/4632`.
...
Tracked closed blocks in this table: `613` verified variants.
