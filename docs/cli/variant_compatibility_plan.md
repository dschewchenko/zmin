...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 80/151 commands with matrix rows / 446/4632 represented doc-option pairs / 1793 written rows / 1490 written rows matching stock Git / 0 partial written rows / 1 open written rows`
...
The latest completed slice expands `ls-files` conflicted-index non-unmerged
matrix rows from
existing stock-oracle evidence:

`git ls-files -s -v`

`ls_files_v2_47.tsv` now records default, `-c`, `-t`, `-v`, `--stage`,
`-s -t` and `-s -v` forms in a conflicted index as separate rows, comparing
stdout, stderr and exit status with stock Git through
...
The latest matrix inventory slice expands `ls_files_v2_47.tsv` conflicted-index
coverage into separate default, cached, tag, lowercase tag, stage, stage-tag
and stage-lowercase-tag rows already covered by
`git_ls_files_compat::ls_files_unmerged_index_matches_stock_git`.
No Rust behavior changed.

### Current Slice Card

This card is the exact handoff target after the current `1793` written-row
...
`0/1793`; the `1/1793` open row and the still incomplete command/doc-option
...
Tracked closed blocks in this table: `781` verified variants.
