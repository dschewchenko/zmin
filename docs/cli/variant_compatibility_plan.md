...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 52/151 commands with matrix rows / 284/4632 represented doc-option pairs / 1249 written rows / 966 written rows matching stock Git / 0 partial written rows / 1 open written rows`
...
The latest matrix inventory slice expands `status --ignore-submodules` rows
already covered by
`git_status_compat::status_ignore_submodules_modes_match_stock_git`.
`status_v2_47.tsv` now splits dirty submodule, dirty+untracked submodule and
new submodule commit states across porcelain v1, porcelain v2, short and human
output modes. No Rust behavior changed.

### Current Slice Card

This card is the exact handoff target after the current `1249` written-row
...
`0/1249`; the `1/1249` open row and the still incomplete command/doc-option
