...
After the latest zero-code `ls-remote` doc-option promotion, `ls-remote`
adds another `2` durable reviewed-complete documented option pairs without
changing Rust behavior and is up to `3/14` documented options complete with
`3/14` represented documented option pairs and `37/37` classified rows.
...
promotion cluster for the represented helper-free `ls-remote` surface. The
existing exact stock-Git evidence already closed two additional `ls-remote`
option families across the represented `--refs` and `--tags` lanes, so this
...
matrices, `1323/3175` complete documented option pairs, and `1878`
remaining checklist rows. `ls-remote` now reads `14` documented option
...
Current durable census after the zero-code reviewed-complete `ls-remote`
documented-option promotion cluster:

- complete command matrices: `82 / 151`
- complete documented command-option pairs: `1323 / 3175`
- matrix rows: `5332`
- verified rows: `4624`
...
- `ls-remote` now has two additional represented helper-free documented option
  families promoted into the reviewed-complete doc-option census list,
  bringing the command to `3/14` reviewed-complete documented option pairs.
- The slice adds no new behavior rows; it only makes already closed exact
  stock-Git multi-transport refs and tags evidence durable in the reviewed
  doc-option source list.
- Focused verification was
  `python3 tools/git-compat-census.py --root .`,
  `tools/git-cli-readiness-status.sh`,
  and `tools/git-compat-command-summary.sh --tsv | rg '^(ls-remote|summary)\t'`.

Latest in-progress family follow-up:

- `ls-remote` now has `3/14` documented option pairs reviewed complete with
  `3/14` represented documented option pairs, `37/37` classified rows, and
