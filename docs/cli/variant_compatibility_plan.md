...
`82/151 complete command matrices / 1290/3175 complete doc-option matrices / 155/151 commands with matrix rows / 1386/3175 represented doc-option pairs / 5332 written rows / 4624/5332 written rows matching stock Git / 0 partial written rows / 26 open written rows`
...
After the latest zero-code `restore` doc-option promotion, `restore`
...
`7/25` represented documented option pairs and `81/81` classified rows.
...
promotion cluster for the represented helper-free `restore` surface. The
existing exact stock-Git evidence already closed seven `restore` option
families across source, staged, worktree, no-overlay, short-form alias,
repeated-form, and invalid-value lanes, so this slice only promotes those
...
`tools/git-compat-command-summary.sh --tsv | rg '^(restore|summary)\t'`,
and `git diff --check`.
...
matrices, `1290/3175` complete documented option pairs, and `1911`
remaining checklist rows. `restore` now reads `25` documented option
pairs with `7` reviewed complete, `7` represented documented option pairs,
and `81/81` classified written rows with `0` open.
...
Current durable census after the zero-code reviewed-complete `restore`
documented-option promotion cluster:

- complete command matrices: `82 / 151`
- complete documented command-option pairs: `1290 / 3175`
- matrix rows: `5332`
- verified rows: `4624`
...
- `restore` now has seven represented helper-free documented option families
...
- The slice adds no new behavior rows; it only makes already closed exact
  stock-Git source, staged, worktree, no-overlay, short-form alias,
  repeated-form, and invalid-value evidence durable in the reviewed doc-option
  source list.
- Focused verification was
  `python3 tools/git-compat-census.py --root .`,
  `tools/git-cli-readiness-status.sh`,
  and `tools/git-compat-command-summary.sh --tsv | rg '^(restore|summary)\t'`.

Latest in-progress family follow-up:

- `restore` now has `7/25` documented option pairs reviewed complete with
  `7/25` represented documented option pairs, `81/81` classified rows, and
