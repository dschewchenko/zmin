...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 96/151 commands with matrix rows / 550/4632 represented doc-option pairs / 2199 written rows / 1883/2199 written rows matching stock Git / 0 partial written rows / 1 open written rows`
...
and exit-code backlog rows from `docs/cli/existing_oracle_test_inventory.tsv`.

`stash_v2_47.tsv` now records already-tested `stash show` status/output rows
for `--exit-code`, `--exit-code --patch`, exit-code/no-exit-code ordering,
`--quiet`, quiet/no-quiet ordering, `--quiet --no-exit-code`,
`--quiet --no-quiet --exit-code`, and `--only-untracked --exit-code`. The
...
`missing_or_unclassified` to `530`. No Rust behavior changed.
...
This card is the exact handoff target after the current `2199` written-row
...
`0/2199`; the `1/2199` open row and the still incomplete command/doc-option
