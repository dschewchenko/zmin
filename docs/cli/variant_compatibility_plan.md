...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 96/151 commands with matrix rows / 550/4632 represented doc-option pairs / 2265 written rows / 1949/2265 written rows matching stock Git / 0 partial written rows / 1 open written rows`
...
The latest completed slice imports the `git_stash_compat.rs` stash branch
backlog rows from
`docs/cli/existing_oracle_test_inventory.tsv`.

`stash_v2_47.tsv` now records already-tested `stash branch` rows for selected
stash entries, raw `stash create` commits, numeric stash selectors and
apply-overwrite failure preservation. The focused `git_stash_compat.rs`
evidence covers stdout, stderr, exit status, branch/status output, worktree
content and stash stack side effects against stock Git. The oracle inventory
reduced total `missing_or_unclassified` to `516`. No Rust behavior changed.
...
This card is the exact handoff target after the current `2265` written-row
...
`0/2265`; the `1/2265` open row and the still incomplete command/doc-option
