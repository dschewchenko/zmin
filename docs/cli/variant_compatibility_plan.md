...
catalog is `151/151` command matrices, `3212/3212` documented pairs, `7758`
behavior rows, `6827` verified rows and `907` stock-compatible invalid-input
rows, but this is not a drop-in claim. A fresh all-nondeprecated manifest batch
at offset `50`, limit `50` initially passed `36/50`. Direct file-URL fetch with
a custom `--upload-pack` now reaches server-side safe-directory validation, so
`t0411-clone-from-partial.sh` is green. Atomic deletion of a ref whose loose
value shadows an older packed value now preserves state and emits stock exit 1
and stderr when `packed-refs.lock` exists, making
`t0600-reffiles-backend.sh` green. Pinned gettext shim generation, seven
v2.47 short-help synopses, update-index file-to-directory replacement, and the
sparse-checkout preservation warning then closed `t0201`, `t0450`, `t1001`,
`t1002`, `t1011`, and `t1060`. The current `t0610` closure makes reftable stack
reads consistent across compaction and is stable under 100 concurrent
`update-ref` writers. The `t1050` closure implements large-file threshold and
binary-diff semantics, stored ZIP output, retained clone packs, and direct
streaming of large blobs into pack/index/rev files. The effective batch
frontier is therefore `47/50`, with `t0613`, `t1013`, and `t1022` still
failing. The `t1060` closure validates corrupt
loose payloads for size queries, exposes the canonical empty tree as Git's
implicit object, and rejects pack-mode fetch collisions with corrupt loose
objects. Status also treats tree-derived index
entries with an all-zero stat cache as untrusted and hashes their content
instead of declaring every size-mismatched path modified; this restores clean
post-rebase status and continuation in the transport regression suite. The
current census has `7758` rows, `6827` verified and `907` invalid-input rows;
the oracle inventory has `1466` tests, `1248` represented and `218` missing or
unclassified. Next work should continue through the three remaining
upstream files, then the next manifest batch and platform/real-client gates.
...
option pairs, `7743` matrix rows, `6815` verified rows, `904` invalid-input
...
batch added two exact stock-Git rows, completed the final documented
`cvsexportcommit` option pairs `-P` and `-W`, and removed those rows from the
durable deferral inventory. The runtime closure stayed intentionally bounded:
Zmin now matches stock Git for explicit non-parent export bases under `-P` and
for `-W` same-worktree exports that quietly detach to the parent and target
commits, then restore the original branch-backed worktree with the stock
checkout-back stderr transcript. Focused gates were
`ZMIN_STOCK_GIT=/tmp/.../git cargo test -q -p zmin-cli --test git_foreign_scm_compat cvsexportcommit_ -- --nocapture`,
`cargo run -q -p zmin-cli --bin zmin -- compat --profile v2-47 --format json > /tmp/zmin-v2-47-schema.json`,
`python3 tools/git-compat-census.py --root . --zmin-schema-json /tmp/zmin-v2-47-schema.json`,
`tools/git-cli-readiness-status.sh`, and `git diff --check`. Durable counts are
now `151/151` complete command matrices, `3172/3212` complete documented
option pairs, `7694` matrix rows, `6775` verified rows, `896` invalid-input
rows, `0` implemented-but-unverified rows, and `0` remaining-to-fix-or-verify
rows. The active backlog still lives entirely in deferred helper-option tails,
now only `svn` (`25`), `cvsimport` (`14`), and `archimport` (`1`).
...
- Public docs may show current generated matrix counts, but must not present
  catalog closure as full drop-in support without the upstream, platform and
  real-client completion gates.
...
The durable target is not `151/151` command dispatch or a closed generated
catalog. It is verified behavior across commands, documented options, option
values, meaningful combinations, transport/local modes, edge cases, invalid
inputs, side effects, upstream tests, platforms and real clients. Public docs
must keep those layers separate and leave drop-in compatibility unclaimed while
any completion gate is open.
...
| ---: | --- | --- | --- |
| 1 | Burn down the three remaining failures in the current Git v2.47.1 upstream batch (`t0613`, `t1013`, `t1022`) | focused upstream rerun plus a local differential regression when practical | advance the effective `47/50` frontier without excluding live Git behavior |
| 2 | Continue the all-nondeprecated upstream manifest in bounded batches | immutable pinned manifest, per-file logs and honest pass/fail summary | every selected file passes or has an explicit user-approved out-of-scope decision |
| 3 | Re-run GUI replacement and paired performance/RSS gates after runtime changes | exact argv/stdin/stdout/stderr/exit/state replay and paired benchmark output | current release stays exact and faster/lower-memory on the supported corpus |
| 4 | Add Linux and Windows evidence for platform-sensitive rows | local platform runs for paths, process spawning, hooks, permissions and line endings | platform-dependent rows have matching stock-Git evidence |
...
Current census counts are `7758` matrix rows, `6827` verified rows, `907`
invalid-input rows, `0` exact-open rows, `0` implemented-but-unverified rows,
`151/151` complete command matrices, `3212/3212` complete documented option
pairs, and `0` remaining fix/verify rows in the generated Git `2.47.1`
checklist. This does not mean replace-git readiness is complete; it means the
baseline denominator is currently fully classified.
...
- Zmin-only snapshot-clone, hooks, and CMS workflow hardening
...
2026-07-15 observed GUI memory follow-up

- Common `show` and `ls-files` invocations now use narrow typed parsers that
  preserve the existing schema types and return to the full parser for any
  unfamiliar, missing-value, or order-sensitive form.
- Named `show --stdin` streams tree-diff records instead of retaining two full
  synthetic indexes and a diff vector. The exact `%H %P` lane reads commit
  links without decoding unused commit bodies, and its transient tree cache
  avoids retaining objects after their index data has been extracted.
- Normal packed-object lookup now matches stock Git by accepting an otherwise
  valid pack index whose trailer checksum alone is corrupt. Explicit index
  decoding and verification remain checksum-strict; the differential test is
  recorded in `show_v2_47.tsv`.
- The final 20-repeat GUI corpus matched exit/stdout/stderr in every pair.
  Median time ratios were below `0.85x` on every lane; p95 RSS ratios were
  `1.004x` for the formerly largest `.idea` `ls-files` gap and `1.051x` for
  stdin-fed named `show`.
- Verification passed `267/267` core tests, `110/110` history tests, `21/21`
  ls-files tests, `18/18` observed-client tests, the runtime-dependency audit,
  formatting, and exact paired benchmark output checks.
