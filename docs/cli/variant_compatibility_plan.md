...
`82/151 complete command matrices / 1254/3175 complete doc-option matrices / 155/151 commands with matrix rows / 1386/3175 represented doc-option pairs / 5332 written rows / 4624/5332 written rows matching stock Git / 0 partial written rows / 26 open written rows`
...
After the latest zero-code `send-email` doc-option promotion, `send-email`
adds another `2` durable reviewed-complete documented option pairs without
changing Rust behavior and is up to `2/64` documented options complete with
`2/64` represented documented option pairs and `18/18` classified rows.
...
promotion cluster for the represented helper-free `send-email` surface. The
existing exact stock-Git evidence already closed two `send-email` option
families across local alias dump and alias translation lanes, so this slice
only promotes those
...
`tools/git-compat-command-summary.sh --tsv | rg '^(send-email|summary)\t'`,
and `git diff --check`.
...
matrices, `1254/3175` complete documented option pairs, and `1947`
remaining checklist rows. `send-email` now reads `64` documented option
pairs with `2` reviewed complete, `2` represented documented option pairs,
and `18/18` classified written rows with `0` open.
...
Current durable census after the zero-code reviewed-complete `send-email`
documented-option promotion cluster:

- complete command matrices: `82 / 151`
- complete documented command-option pairs: `1254 / 3175`
- matrix rows: `5332`
- verified rows: `4624`
...
- `send-email` now has two represented helper-free local documented option
  families promoted into the reviewed-complete doc-option census list,
  bringing the command to `2/64` reviewed-complete documented option pairs.
- The slice adds no new behavior rows; it only makes already closed exact
  stock-Git local alias dump and alias translation evidence durable in the
  reviewed doc-option source list.
- Focused verification was
  `python3 tools/git-compat-census.py --root .`,
  `tools/git-cli-readiness-status.sh`,
  and `tools/git-compat-command-summary.sh --tsv | rg '^(send-email|summary)\t'`.

Latest in-progress family follow-up:

- `send-email` now has `2/64` documented option pairs reviewed complete with
  `2/64` represented documented option pairs, `18/18` classified rows, and
