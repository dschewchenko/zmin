...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 98/151 commands with matrix rows / 574/4632 represented doc-option pairs / 2451 written rows / 2105/2451 written rows matching stock Git / 0 partial written rows / 1 open written rows`
...
complete current backlog list to walk: `961` focused oracle functions, `535`
represented or classified and `426` `missing_or_unclassified`. If the generated
...
backlog snapshot at `961` focused oracle functions: `535` already represented
or classified and `426` still `missing_or_unclassified`. Treat that snapshot as
...
The latest completed slice extends `notes_v2_47.tsv` with four `notes`
reuse-message and reedit-message rows from existing focused stock-oracle
evidence in `git_notes_compat.rs`.

`notes_v2_47.tsv` now records closed rows for `notes add -C`, `notes append
--reuse-message`, `notes add -c` and `notes append --reedit-message`. Evidence
comes from `git_notes_compat::notes_reuse_message_matches_stock_git_for_add_and_append`
and `git_notes_compat::notes_reedit_message_matches_stock_git_for_add_and_append`,
comparing stock Git and Zmin command output plus resulting note contents. The
oracle inventory now lists `535` represented/classified functions and `426`
`missing_or_unclassified`. Current written rows are `2451`, with `2105/2451`
matching stock Git, `1/2451` open and `345/2451` invalid-input. No Rust
behavior changed.
