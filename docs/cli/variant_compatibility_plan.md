...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 52/151 commands with matrix rows / 282/4632 represented doc-option pairs / 1177 written rows / 895 written rows matching stock Git / 0 partial written rows / 1 open written rows`
...
The latest completed slice imports already-covered `stash list` limiting and
grep filtering rows from existing stock-oracle tests:

`git stash list --skip=1 --max-count=1`

`docs/cli/matrices/stash_v2_47.tsv` now records max-count aliases, skip,
grep, max-count plus grep, invert-grep and case-insensitive grep rows with
focused evidence in
...
This card is the exact handoff target after the current `1177` written-row
...
`0/1177`; the `1/1177` open row and the still incomplete command/doc-option
