...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 31/151 commands with matrix rows / 238/4632 represented doc-option pairs / 975 written rows / 763 written rows matching stock Git / 0 open written rows`
...
The latest closed guard classification is `commit-graph verify` header
validation. Stock Git rejects checksum-valid commit-graph bad signatures,
unsupported versions and unsupported hash versions with specific diagnostics
and exit `1`. Zmin now emits the same diagnostics for those invalid-input
variants instead of a generic unsupported-header fatal error.
...
This card is the exact handoff target after the current `975` written-row
...
| Implementation area | start at `crates/zmin-cli/src/cli/commands/pack_impl.rs:3499` (`unsupported pack index version '{other}'`) unless a new WebStorm replacement trace appears first |
...
`0/975`; the complete command matrices and complete doc-option matrices remain
`0/151` and `0/4632`.
...
Tracked closed blocks in this table: `624` verified variants.
