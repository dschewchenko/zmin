...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 98/151 commands with matrix rows / 560/4632 represented doc-option pairs / 2380 written rows / 2038/2380 written rows matching stock Git / 0 partial written rows / 1 open written rows`
...
backlog snapshot at `961` focused oracle functions: `509` already represented
or classified and `452` still `missing_or_unclassified`. Treat that snapshot as
...
The latest completed slice extends `commit_v2_47.tsv` with five edit/no-edit
message-source rows from existing focused stock-oracle evidence in
`git_commit_compat.rs`.

`commit_v2_47.tsv` now records closed rows for `git commit -e -m ...`,
`git commit -e -F msg.txt`, `git commit -e -C HEAD`,
`git commit --no-edit -c HEAD` and `git commit --no-edit -m ...`. Evidence
comes from
`git_commit_compat::commit_edit_and_no_edit_message_sources_match_stock_git`,
comparing stock Git and Zmin commit objects, command output, editor invocation
and editor input buffers. The oracle inventory now lists `509`
represented/classified functions and `452` `missing_or_unclassified`. Current
written rows are `2380`, with `2038/2380` matching stock Git, `1/2380` open
and `341/2380` invalid-input. No Rust behavior changed.
...
This card is the exact handoff target after the current `2380` written-row
...
`0/2380`; the `1/2380` open row and the still incomplete command/doc-option
