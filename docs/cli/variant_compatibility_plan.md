...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 48/151 commands with matrix rows / 252/4632 represented doc-option pairs / 1080 written rows / 815 written rows matching stock Git / 0 partial written rows / 1 open written rows`
...
The latest completed slice is `fetch --tags` with an explicit local path
branch fetch:

`git fetch --tags <path> main`
...
the branch and tag update lines on stderr and exits successfully. Zmin now
matches those side effects for explicit local path branch fetches. The row is
recorded in
`docs/cli/matrices/fetch_v2_47.tsv` with focused evidence in
`git_transport_local_compat::fetch_tags_direct_location_branch_follows_tags_like_stock_git`.
...
This card is the exact handoff target after the current `1080` written-row
...
`0/1080`; the `1/1080` open row and the still incomplete command/doc-option
