...
`0/151 complete command matrices / 0/4632 complete doc-option matrices / 26/151 commands with matrix rows / 236/4632 represented doc-option pairs / 926 written rows / 755 written rows matching stock Git / 0 open written rows`
...
The latest closed guard classification is `git blame -L /\{/ a.txt`,
`git blame -L /a\{x\}/ a.txt`, `git blame -L /a\{2/ a.txt`,
`git blame -L /a\{,2\}/ a.txt`, and `git blame -L /a\{3,2\}/ a.txt` for Git
basic-regex invalid interval diagnostics.
...
`0/926`; the complete command matrices and complete doc-option matrices remain
`0/151` and `0/4632`.
