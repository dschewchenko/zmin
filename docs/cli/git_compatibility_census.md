...
| Existing matrix rows | `2703` | evidence layer, not full denominator |
| Verified exact rows | `2315` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `794` | schema args and additional schema paths without exact matrix evidence |
