...
| Existing matrix rows | `2733` | evidence layer, not full denominator |
| Verified exact rows | `2345` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `763` | schema args and additional schema paths without exact matrix evidence |
