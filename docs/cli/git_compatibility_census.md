...
| Existing matrix rows | `2807` | evidence layer, not full denominator |
| Verified exact rows | `2418` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `712` | schema args and additional schema paths without exact matrix evidence |
