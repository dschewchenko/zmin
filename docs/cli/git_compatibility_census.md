...
| Existing matrix rows | `2824` | evidence layer, not full denominator |
| Verified exact rows | `2435` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `697` | schema args and additional schema paths without exact matrix evidence |
