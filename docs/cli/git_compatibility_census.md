...
| Existing matrix rows | `2731` | evidence layer, not full denominator |
| Verified exact rows | `2343` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `766` | schema args and additional schema paths without exact matrix evidence |
