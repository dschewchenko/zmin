...
| Existing matrix rows | `2728` | evidence layer, not full denominator |
| Verified exact rows | `2340` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `769` | schema args and additional schema paths without exact matrix evidence |
