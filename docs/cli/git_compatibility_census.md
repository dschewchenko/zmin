...
| Existing matrix rows | `2822` | evidence layer, not full denominator |
| Verified exact rows | `2433` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `699` | schema args and additional schema paths without exact matrix evidence |
