...
| Existing matrix rows | `2729` | evidence layer, not full denominator |
| Verified exact rows | `2341` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `768` | schema args and additional schema paths without exact matrix evidence |
