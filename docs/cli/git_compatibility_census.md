...
| Existing matrix rows | `2754` | evidence layer, not full denominator |
| Verified exact rows | `2366` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `741` | schema args and additional schema paths without exact matrix evidence |
