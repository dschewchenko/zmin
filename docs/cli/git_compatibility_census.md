...
| Existing matrix rows | `2687` | evidence layer, not full denominator |
| Verified exact rows | `2300` | closed behavior variants safe to skip exactly |
| Invalid-input parity rows | `386` | stock-compatible rejection variants |
| Exact open or partial matrix rows | `1` | row exists but is not closed |
| Implemented but unverified rows | `806` | schema args and additional schema paths without exact matrix evidence |
