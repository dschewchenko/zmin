...
| Existing matrix rows | `2763` | evidence layer, not full denominator |
| Verified exact rows | `2375` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `737` | schema args and additional schema paths without exact matrix evidence |
