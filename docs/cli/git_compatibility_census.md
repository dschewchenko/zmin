...
| Existing matrix rows | `2817` | evidence layer, not full denominator |
| Verified exact rows | `2428` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `702` | schema args and additional schema paths without exact matrix evidence |
