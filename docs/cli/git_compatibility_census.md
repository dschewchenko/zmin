...
| Existing matrix rows | `2724` | evidence layer, not full denominator |
| Verified exact rows | `2336` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `775` | schema args and additional schema paths without exact matrix evidence |
