...
| Existing matrix rows | `2805` | evidence layer, not full denominator |
| Verified exact rows | `2416` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `714` | schema args and additional schema paths without exact matrix evidence |
