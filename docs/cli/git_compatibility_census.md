...
| Existing matrix rows | `2727` | evidence layer, not full denominator |
| Verified exact rows | `2339` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `771` | schema args and additional schema paths without exact matrix evidence |
