...
| Existing matrix rows | `2726` | evidence layer, not full denominator |
| Verified exact rows | `2338` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `773` | schema args and additional schema paths without exact matrix evidence |
