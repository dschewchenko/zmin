...
| Existing matrix rows | `2764` | evidence layer, not full denominator |
| Verified exact rows | `2376` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `736` | schema args and additional schema paths without exact matrix evidence |
