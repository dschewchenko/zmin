...
| Existing matrix rows | `2710` | evidence layer, not full denominator |
| Verified exact rows | `2322` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `786` | schema args and additional schema paths without exact matrix evidence |
