...
| Existing matrix rows | `2795` | evidence layer, not full denominator |
| Verified exact rows | `2407` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `724` | schema args and additional schema paths without exact matrix evidence |
