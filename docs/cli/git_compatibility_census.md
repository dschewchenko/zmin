...
| Existing matrix rows | `2804` | evidence layer, not full denominator |
| Verified exact rows | `2415` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `715` | schema args and additional schema paths without exact matrix evidence |
