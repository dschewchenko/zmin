...
| Existing matrix rows | `2800` | evidence layer, not full denominator |
| Verified exact rows | `2411` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `719` | schema args and additional schema paths without exact matrix evidence |
