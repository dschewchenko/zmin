...
| Existing matrix rows | `2794` | evidence layer, not full denominator |
| Verified exact rows | `2406` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `725` | schema args and additional schema paths without exact matrix evidence |
