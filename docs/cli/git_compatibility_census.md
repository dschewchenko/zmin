...
| Existing matrix rows | `2718` | evidence layer, not full denominator |
| Verified exact rows | `2330` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `780` | schema args and additional schema paths without exact matrix evidence |
