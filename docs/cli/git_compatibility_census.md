...
| Existing matrix rows | `2803` | evidence layer, not full denominator |
| Verified exact rows | `2414` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `716` | schema args and additional schema paths without exact matrix evidence |
