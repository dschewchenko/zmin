...
| Existing matrix rows | `2725` | evidence layer, not full denominator |
| Verified exact rows | `2337` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `774` | schema args and additional schema paths without exact matrix evidence |
