...
| Existing matrix rows | `2802` | evidence layer, not full denominator |
| Verified exact rows | `2413` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `717` | schema args and additional schema paths without exact matrix evidence |
