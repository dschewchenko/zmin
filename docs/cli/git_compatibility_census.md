...
| Existing matrix rows | `2752` | evidence layer, not full denominator |
| Verified exact rows | `2364` | closed behavior variants safe to skip exactly |
...
| Implemented but unverified rows | `743` | schema args and additional schema paths without exact matrix evidence |
