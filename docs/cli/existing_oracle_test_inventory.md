...
Generated after reviewing the `git_stash_compat.rs` stash show whitespace
diff-options backlog on `compat/status-pathspec-matrix`.

| Layer | Count |
| --- | ---: |
| Stock-oracle test functions found | `961` |
| Test functions referenced by at least one matrix row, extension inventory entry or deferral entry | `432` |
| Test functions missing or not yet classified by matrix/extension/deferral evidence | `529` |
