...
| Written behavior rows | `785` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `709/785` | yes, row by row | exact written rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`785`** | **`709`** | **`0`** | **`0`** | **`76`** | **`453`** |
...
represented option pairs or `709/785` passing written rows as a Git support
percentage. The `709/785` number is audit progress for rows already written
