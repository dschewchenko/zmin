...
| Written behavior rows | `137` | no by itself | explicit rows currently written |
| Closed behavior rows | `122/137` | yes, row by row | exact written rows that match stock Git |
...
| Sharing and Updating Projects | `5` | `309` | `77` | `66` | `0` | `9` | `2` | `0` |
...
| **Git 2.47 unique total** | **`151`** | **`4632`** | **`137`** | **`122`** | **`0`** | **`9`** | **`6`** | **`253`** |
...
Never use `151/151` command presence, `4632` option spellings or `122/137`
closed written rows as a Git support percentage. The `122/137` number is audit
...
| `fetch` | `73` | `28` | `docs/cli/matrices/fetch_v2_47.tsv` | `77` | `66` | `0` | `9` | `2` | no |
...
`FETCH_HEAD` modes and multiple explicit refspecs. Zmin invokes the external
upload-pack command, indexes the returned pack, then updates refs and
`FETCH_HEAD` like stock Git. SSH upload-pack override plus shallow, `--all` and
`--multiple` combinations remain open.
