...
| Written behavior rows | `837` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `732/837` | yes, row by row | supported-behavior rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`837`** | **`732`** | **`0`** | **`0`** | **`105`** | **`501`** |
...
represented option pairs or `732/837` passing written rows as a Git support
percentage. The `732/837` number is audit progress for supported rows already
written down; `105/837` additional rows are stock-compatible invalid inputs. It
