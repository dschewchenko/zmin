...
| Git doc option pairs represented by rows | `579/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `2476` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2126/2476` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2476` | no | written rows with incomplete parity |
| Open written rows | `1/2476` | no | written rows that still do not match stock Git |
| Invalid input rows | `349/2476` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `270` | `243` | `0` | `0` | `27` | `145` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2476`** | **`2126`** | **`0`** | **`1`** | **`349`** | **`1018`** |
...
Never use `151/151` command presence, `4632` option spellings, `579/4632`
represented option pairs or `2126/2476` passing written rows as a Git support
percentage. The `2126/2476` number is audit progress for supported rows already
written down; `0/2476` rows are partial, `1/2476` rows are open and `349/2476`
