...
| Written behavior rows | `1040` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `784/1040` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `1/1040` | no | written rows with incomplete parity |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1040`** | **`784`** | **`1`** | **`0`** | **`255`** | **`673`** |
...
represented option pairs or `784/1040` passing written rows as a Git support
percentage. The `784/1040` number is audit progress for supported rows already
written down; `1/1040` row is partial and `255/1040` additional rows are
