...
| Written behavior rows | `855` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `739/855` | yes, row by row | supported-behavior rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`855`** | **`739`** | **`0`** | **`0`** | **`116`** | **`505`** |
...
represented option pairs or `739/855` passing written rows as a Git support
percentage. The `739/855` number is audit progress for supported rows already
written down; `116/855` additional rows are stock-compatible invalid inputs. It
