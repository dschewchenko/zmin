...
| Written behavior rows | `792` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `716/792` | yes, row by row | exact written rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`792`** | **`716`** | **`0`** | **`0`** | **`76`** | **`460`** |
...
represented option pairs or `716/792` passing written rows as a Git support
percentage. The `716/792` number is audit progress for rows already written
