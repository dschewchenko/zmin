...
| Written behavior rows | `2456` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2110/2456` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2456` | no | written rows with incomplete parity |
| Open written rows | `1/2456` | no | written rows that still do not match stock Git |
| Invalid input rows | `345/2456` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `250` | `227` | `0` | `0` | `23` | `145` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2456`** | **`2110`** | **`0`** | **`1`** | **`345`** | **`1018`** |
...
represented option pairs or `2110/2456` passing written rows as a Git support
percentage. The `2110/2456` number is audit progress for supported rows already
written down; `0/2456` rows are partial, `1/2456` rows are open and `345/2456`
