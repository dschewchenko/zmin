...
| Written behavior rows | `1003` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `770/1003` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `1/1003` | no | written rows with incomplete parity |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1003`** | **`770`** | **`1`** | **`0`** | **`232`** | **`648`** |
...
represented option pairs or `770/1003` passing written rows as a Git support
percentage. The `770/1003` number is audit progress for supported rows already
written down; `1/1003` row is partial and `232/1003` additional rows are
