...
| Written behavior rows | `2149` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `1837/2149` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2149` | no | written rows with incomplete parity |
| Open written rows | `1/2149` | no | written rows that still do not match stock Git |
| Invalid input rows | `311/2149` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `400` | `391` | `0` | `0` | `9` | `52` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2149`** | **`1837`** | **`0`** | **`1`** | **`311`** | **`1018`** |
...
represented option pairs or `1837/2149` passing written rows as a Git support
percentage. The `1837/2149` number is audit progress for supported rows already
written down; `0/2149` rows are partial, `1/2149` rows are open and `311/2149`
