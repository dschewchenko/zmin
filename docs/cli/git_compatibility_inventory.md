...
| Git doc option pairs represented by rows | `561/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `2384` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2042/2384` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2384` | no | written rows with incomplete parity |
| Open written rows | `1/2384` | no | written rows that still do not match stock Git |
| Invalid input rows | `341/2384` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `178` | `159` | `0` | `0` | `19` | `145` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2384`** | **`2042`** | **`0`** | **`1`** | **`341`** | **`1018`** |
...
Never use `151/151` command presence, `4632` option spellings, `561/4632`
represented option pairs or `2042/2384` passing written rows as a Git support
percentage. The `2042/2384` number is audit progress for supported rows already
written down; `0/2384` rows are partial, `1/2384` rows are open and `341/2384`
...
| `commit` | `63` | `0` | `11` | `docs/cli/matrices/commit_v2_47.tsv` | `30` | `30` | `0` | `0` | `0` | no |
