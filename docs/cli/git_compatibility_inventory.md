...
| Written behavior rows | `678` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `604/678` | yes, row by row | exact written rows with parity evidence |
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `218` | `212` | `0` | `2` | `4` | `95` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`678`** | **`604`** | **`0`** | **`2`** | **`72`** | **`361`** |
...
represented option pairs or `604/678` passing written rows as a Git support
percentage. The `604/678` number is audit progress for rows already written
...
| `fetch` | `73` | `0` | `28` | `docs/cli/matrices/fetch_v2_47.tsv` | `218` | `212` | `0` | `2` | `4` | no |
...
true, numeric true and on-demand forms for a changed submodule gitlink, plus
implicit yes, explicit yes and on-demand forms for a smart HTTP parent fetch
whose initialized submodule remote is local, and implicit yes plus on-demand
forms for a smart HTTP parent fetch with the submodule still uninitialized
locally. The latest nested slice also closes implicit yes for a smart HTTP
