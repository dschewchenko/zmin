...
| Written behavior rows | `2533` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2159/2533` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2533` | no | written rows with incomplete parity |
| Open written rows | `1/2533` | no | written rows that still do not match stock Git |
| Invalid input rows | `373/2533` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `361` | `336` | `0` | `0` | `25` | `160` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2533`** | **`2159`** | **`0`** | **`1`** | **`373`** | **`1018`** |
...
represented option pairs or `2159/2533` passing written rows as a Git support
percentage. The `2159/2533` number is audit progress for supported rows already
written down; `0/2533` rows are partial, `1/2533` rows are open and `373/2533`
