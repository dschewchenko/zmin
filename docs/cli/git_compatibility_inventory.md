...
| Written behavior rows | `1090` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `821/1090` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1090` | no | written rows with incomplete parity |
| Open written rows | `1/1090` | no | written rows that still do not match stock Git |
| Invalid input rows | `268/1090` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1090`** | **`821`** | **`0`** | **`1`** | **`268`** | **`690`** |
...
represented option pairs or `821/1090` passing written rows as a Git support
percentage. The `821/1090` number is audit progress for supported rows already
written down; `0/1090` rows are partial, `1/1090` rows are open and `268/1090`
