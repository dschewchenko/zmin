...
| Written behavior rows | `756` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `680/756` | yes, row by row | exact written rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`756`** | **`680`** | **`0`** | **`0`** | **`76`** | **`424`** |
...
represented option pairs or `680/756` passing written rows as a Git support
percentage. The `680/756` number is audit progress for rows already written
