...
| Commands with any matrix rows | `13/151` | no | audit rows exist for `branch`, `config`, `status`, `fetch`, `diff`, `log`, `for-each-ref`, `ls-files`, `merge-base`, `rev-parse`, `show-ref`, `symbolic-ref` and `tag` |
| Git doc option pairs represented by rows | `212/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `610` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `534/610` | yes, row by row | exact written rows with parity evidence |
...
| Plumbing Commands | `20` | `0` | `644` | `0` | `142` | `115` | `0` | `0` | `27` | `76` |
| Other Git 2.47 commands | `71` | `0` | `1075` | `0` | `0` | `0` | `0` | `0` | `0` | `4` |
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`610`** | **`534`** | **`0`** | **`9`** | **`67`** | **`330`** |
...
Never use `151/151` command presence, `4632` option spellings, `212/4632`
represented option pairs or `534/610` passing written rows as a Git support
percentage. The `534/610` number is audit progress for rows already written
...
local/file remotes and explicit local/file locations from shallow sources, and
named branch fetches over smart HTTP, SSH and git daemon. Network multi-refspec
and branchless `--update-shallow` modes remain open.
...
named local/file remotes and explicit local/file locations. The latest network
slice closed branch shallow-exclude fetches over smart HTTP, SSH and git
daemon; network multi-refspec, branchless and related modes remain open.
...
slice closed branch deepen fetches over smart HTTP, SSH and git daemon using
shallow boundary lines plus the `deepen-relative` capability; network
multi-refspec, branchless and related modes remain open.
...
explicit local/file locations in existing shallow repos. The latest network
slice closed branch unshallow fetches over smart HTTP, SSH and git daemon using
shallow boundary lines plus an absolute deepen request; network branchless
configured-fetch, multi-refspec and related modes remain open.
