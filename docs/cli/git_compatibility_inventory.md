...
| Written behavior rows | `2572` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2197/2572` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2572` | no | written rows with incomplete parity |
| Open written rows | `1/2572` | no | written rows that still do not match stock Git |
| Invalid input rows | `374/2572` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2572`** | **`2197`** | **`0`** | **`1`** | **`374`** | **`1018`** |
...
represented option pairs or `2197/2572` passing written rows as a Git support
percentage. The `2197/2572` number is audit progress for supported rows already
written down; `0/2572` rows are partial, `1/2572` rows are open and `374/2572`
