...
| Written behavior rows | `1069` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `808/1069` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1069` | no | written rows with incomplete parity |
| Open written rows | `0/1069` | no | written rows that still do not match stock Git |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1069`** | **`808`** | **`0`** | **`0`** | **`261`** | **`683`** |
...
represented option pairs or `808/1069` passing written rows as a Git support
percentage. The `808/1069` number is audit progress for supported rows already
written down; `0/1069` rows are partial and `261/1069` additional rows are
