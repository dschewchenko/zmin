...
| Git doc option pairs represented by rows | `285/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `1277` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `994/1277` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1277` | no | written rows with incomplete parity |
| Open written rows | `1/1277` | no | written rows that still do not match stock Git |
| Invalid input rows | `282/1277` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `172` | `165` | `0` | `0` | `7` | `26` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1277`** | **`994`** | **`0`** | **`1`** | **`282`** | **`721`** |
...
Never use `151/151` command presence, `4632` option spellings, `285/4632`
represented option pairs or `994/1277` passing written rows as a Git support
percentage. The `994/1277` number is audit progress for supported rows already
written down; `0/1277` rows are partial, `1/1277` rows are open and `282/1277`
...
| `diff` | `133` | `0` | `32` | `docs/cli/matrices/diff_v2_47.tsv` | `85` | `83` | `0` | `0` | `2` | no |
