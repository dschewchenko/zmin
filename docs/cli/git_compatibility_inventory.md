...
| Written behavior rows | `755` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `679/755` | yes, row by row | exact written rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`755`** | **`679`** | **`0`** | **`0`** | **`76`** | **`423`** |
...
represented option pairs or `679/755` passing written rows as a Git support
percentage. The `679/755` number is audit progress for rows already written
