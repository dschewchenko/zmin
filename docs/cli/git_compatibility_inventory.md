...
| Git doc option pairs represented by rows | `292/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `1286` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `1003/1286` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1286` | no | written rows with incomplete parity |
| Open written rows | `1/1286` | no | written rows that still do not match stock Git |
| Invalid input rows | `282/1286` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `181` | `174` | `0` | `0` | `7` | `26` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1286`** | **`1003`** | **`0`** | **`1`** | **`282`** | **`721`** |
...
Never use `151/151` command presence, `4632` option spellings, `292/4632`
represented option pairs or `1003/1286` passing written rows as a Git support
percentage. The `1003/1286` number is audit progress for supported rows already
written down; `0/1286` rows are partial, `1/1286` rows are open and `282/1286`
...
| `diff` | `133` | `0` | `39` | `docs/cli/matrices/diff_v2_47.tsv` | `94` | `92` | `0` | `0` | `2` | no |
