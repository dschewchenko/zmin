...
| Written behavior rows | `1187` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `905/1187` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1187` | no | written rows with incomplete parity |
| Open written rows | `1/1187` | no | written rows that still do not match stock Git |
| Invalid input rows | `281/1187` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1187`** | **`905`** | **`0`** | **`1`** | **`281`** | **`721`** |
...
represented option pairs or `905/1187` passing written rows as a Git support
percentage. The `905/1187` number is audit progress for supported rows already
written down; `0/1187` rows are partial, `1/1187` rows are open and `281/1187`
