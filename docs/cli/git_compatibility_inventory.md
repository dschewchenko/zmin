...
| Written behavior rows | `1256` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `973/1256` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1256` | no | written rows with incomplete parity |
| Open written rows | `1/1256` | no | written rows that still do not match stock Git |
| Invalid input rows | `282/1256` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `130` | `111` | `0` | `0` | `19` | `101` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1256`** | **`973`** | **`0`** | **`1`** | **`282`** | **`721`** |
...
represented option pairs or `973/1256` passing written rows as a Git support
percentage. The `973/1256` number is audit progress for supported rows already
written down; `0/1256` rows are partial, `1/1256` rows are open and `282/1256`
