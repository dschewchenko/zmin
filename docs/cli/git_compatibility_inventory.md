...
| Written behavior rows | `816` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `727/816` | yes, row by row | supported-behavior rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`816`** | **`727`** | **`0`** | **`0`** | **`89`** | **`480`** |
...
represented option pairs or `727/816` passing written rows as a Git support
percentage. The `727/816` number is audit progress for supported rows already
written down; `89/816` additional rows are stock-compatible invalid inputs. It
