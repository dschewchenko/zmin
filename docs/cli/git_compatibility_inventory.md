...
| Written behavior rows | `783` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `707/783` | yes, row by row | exact written rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`783`** | **`707`** | **`0`** | **`0`** | **`76`** | **`451`** |
...
represented option pairs or `707/783` passing written rows as a Git support
percentage. The `707/783` number is audit progress for rows already written
