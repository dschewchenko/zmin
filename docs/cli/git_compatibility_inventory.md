...
| Git doc option pairs represented by rows | `504/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `1995` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `1684/1995` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1995` | no | written rows with incomplete parity |
| Open written rows | `1/1995` | no | written rows that still do not match stock Git |
| Invalid input rows | `310/1995` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `366` | `358` | `0` | `0` | `8` | `52` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1995`** | **`1684`** | **`0`** | **`1`** | **`310`** | **`1018`** |
...
Never use `151/151` command presence, `4632` option spellings, `504/4632`
represented option pairs or `1684/1995` passing written rows as a Git support
percentage. The `1684/1995` number is audit progress for supported rows already
written down; `0/1995` rows are partial, `1/1995` rows are open and `310/1995`
...
| `diff` | `133` | `0` | `69` | `docs/cli/matrices/diff_v2_47.tsv` | `216` | `213` | `0` | `0` | `3` | no |
