...
- `docs/cli/matrices/diff_v2_47.tsv` tracks the first `diff` output format,
  ...
  binary-text and irreversible-delete variants.
...
| Git doc option pairs represented by rows | `370/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `1562` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `1270/1562` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1562` | no | written rows with incomplete parity |
| Open written rows | `1/1562` | no | written rows that still do not match stock Git |
| Invalid input rows | `291/1562` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `223` | `216` | `0` | `0` | `7` | `26` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1562`** | **`1270`** | **`0`** | **`1`** | **`291`** | **`721`** |
...
Never use `151/151` command presence, `4632` option spellings, `370/4632`
represented option pairs or `1270/1562` passing written rows as a Git support
percentage. The `1270/1562` number is audit progress for supported rows already
written down; `0/1562` rows are partial, `1/1562` rows are open and `291/1562`
...
| `diff` | `133` | `0` | `53` | `docs/cli/matrices/diff_v2_47.tsv` | `125` | `123` | `0` | `0` | `2` | no |
