...
| Commands with any matrix rows | `64/151` | no | audit rows exist for `add`, `archive`, `bisect`, `blame`, `branch`, `bundle`, `cat-file`, `check-attr`, `check-ignore`, `check-ref-format`, `checkout`, `checkout-index`, `clean`, `clone`, `column`, `commit-graph`, `config`, `credential`, `credential-cache`, `credential-store`, `describe`, `diff`, `diff-files`, `diff-index`, `diff-tree`, `fast-import`, `fetch`, `filter-branch`, `for-each-ref`, `grep`, `http-fetch`, `index-pack`, `init`, `log`, `ls-files`, `ls-remote`, `ls-tree`, `maintenance`, `merge`, `merge-base`, `multi-pack-index`, `notes`, `p4`, `pack-objects`, `prune`, `push`, `rebase`, `reflog`, `remote`, `rerere`, `rev-parse`, `rm`, `show-index`, `show-ref`, `sparse-checkout`, `stash`, `status`, `submodule`, `symbolic-ref`, `tag`, `var`, `verify-pack`, `version` and `worktree` |
| Git doc option pairs represented by rows | `398/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `1664` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `1365/1664` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1664` | no | written rows with incomplete parity |
| Open written rows | `1/1664` | no | written rows that still do not match stock Git |
| Invalid input rows | `298/1664` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `259` | `252` | `0` | `0` | `7` | `36` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1664`** | **`1365`** | **`0`** | **`1`** | **`298`** | **`796`** |
...
Never use `151/151` command presence, `4632` option spellings, `398/4632`
represented option pairs or `1365/1664` passing written rows as a Git support
percentage. The `1365/1664` number is audit progress for supported rows already
written down; `0/1664` rows are partial, `1/1664` rows are open and `298/1664`
