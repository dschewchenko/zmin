...
| Written behavior rows | `790` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `714/790` | yes, row by row | exact written rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`790`** | **`714`** | **`0`** | **`0`** | **`76`** | **`458`** |
...
represented option pairs or `714/790` passing written rows as a Git support
percentage. The `714/790` number is audit progress for rows already written
