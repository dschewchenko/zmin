...
| Git doc option pairs represented by rows | `532/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `2065` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `1754/2065` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2065` | no | written rows with incomplete parity |
| Open written rows | `1/2065` | no | written rows that still do not match stock Git |
| Invalid input rows | `310/2065` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Debugging | `3` | `0` | `132` | `0` | `144` | `63` | `0` | `0` | `81` | `166` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2065`** | **`1754`** | **`0`** | **`1`** | **`310`** | **`1018`** |
...
Never use `151/151` command presence, `4632` option spellings, `532/4632`
represented option pairs or `1754/2065` passing written rows as a Git support
percentage. The `1754/2065` number is audit progress for supported rows already
written down; `0/2065` rows are partial, `1/2065` rows are open and `310/2065`
...
| `blame` | `39` | `0` | `22` | `docs/cli/matrices/blame_v2_47.tsv` | `131` | `51` | `0` | `0` | `80` | no |
