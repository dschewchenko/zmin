...
| Git doc option pairs represented by rows | `282/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `1177` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `895/1177` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1177` | no | written rows with incomplete parity |
| Open written rows | `1/1177` | no | written rows that still do not match stock Git |
| Invalid input rows | `281/1177` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Branching and Merging | `9` | `0` | `581` | `0` | `103` | `73` | `0` | `0` | `30` | `50` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1177`** | **`895`** | **`0`** | **`1`** | **`281`** | **`721`** |
...
Never use `151/151` command presence, `4632` option spellings, `282/4632`
represented option pairs or `895/1177` passing written rows as a Git support
percentage. The `895/1177` number is audit progress for supported rows already
written down; `0/1177` rows are partial, `1/1177` rows are open and `281/1177`
...
| `stash` | `30` | `0` | `0` | `docs/cli/matrices/stash_v2_47.tsv` | `42` | `38` | `0` | `0` | `4` | no |
