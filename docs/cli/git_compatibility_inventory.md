...
| Git doc option pairs represented by rows | `235/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `856` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `739/856` | yes, row by row | supported-behavior rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`856`** | **`739`** | **`0`** | **`0`** | **`117`** | **`506`** |
...
Never use `151/151` command presence, `4632` option spellings, `235/4632`
represented option pairs or `739/856` passing written rows as a Git support
percentage. The `739/856` number is audit progress for supported rows already
written down; `117/856` additional rows are stock-compatible invalid inputs. It
