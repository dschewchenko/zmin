...
| Written behavior rows | `1024` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `778/1024` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `1/1024` | no | written rows with incomplete parity |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1024`** | **`778`** | **`1`** | **`0`** | **`245`** | **`656`** |
...
represented option pairs or `778/1024` passing written rows as a Git support
percentage. The `778/1024` number is audit progress for supported rows already
written down; `1/1024` row is partial and `245/1024` additional rows are
