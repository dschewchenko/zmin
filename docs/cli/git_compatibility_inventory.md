...
- `docs/cli/matrices/add_v2_47.tsv` tracks the first `add` process-filter
  invalid-input variants, intent-to-add rows and global pathspec update
  variants.
- `docs/cli/matrices/am_v2_47.tsv` tracks the first `am` stock format-patch
  mail application variant.
- `docs/cli/matrices/apply_v2_47.tsv` tracks the first `apply` stdin patch,
  check, cached, index, reverse, binary, rename, mode-only and invalid patch
  input variants.
...
- `docs/cli/matrices/check_mailmap_v2_47.tsv` tracks the first
  `check-mailmap` positional identity and stdin identity variants.
- `docs/cli/matrices/check_ref_format_v2_47.tsv` tracks the first
  `check-ref-format` refname, allow-onelevel, normalize, branch and
...
- `docs/cli/matrices/checkout_index_v2_47.tsv` tracks the first
  `checkout-index` process-filter invalid-input variant plus exact `-a`,
  path, `--prefix`, `--stdin`, `--quiet` and `-q` worktree restoration rows.
- `docs/cli/matrices/commit_v2_47.tsv` tracks the first `commit`
  initial commit, amend and dot-pathspec variants.
...
- `docs/cli/matrices/credential_v2_47.tsv` tracks the first `credential`
  fill, approve, reject and missing-field variants.
- `docs/cli/matrices/credential_cache_v2_47.tsv` tracks the first
  `credential-cache --socket` store, get, erase and get-after-erase variants.
- `docs/cli/matrices/credential_store_v2_47.tsv` tracks the first
  `credential-store` store, get and erase variants.
- `docs/cli/matrices/describe_v2_47.tsv` tracks the first `describe` tag,
  ref-filter, abbreviation, always and dirty-worktree variants.
...
- `docs/cli/matrices/fmt_merge_msg_v2_47.tsv` tracks the first
  `fmt-merge-msg` stdin and file-input merge title variants.
- `docs/cli/matrices/for_each_repo_v2_47.tsv` tracks the first
  `for-each-repo` configured repository iteration and missing-repository
  variants.
- `docs/cli/matrices/format_patch_v2_47.tsv` tracks the first `format-patch`
  stdout, inline, subject-prefix, cover-letter, numbered and merge-commit
  variants.
...
- `docs/cli/matrices/http_backend_v2_47.tsv` tracks the first `http-backend`
  smart HTTP upload-pack filter rows.
- `docs/cli/matrices/send_pack_v2_47.tsv` tracks the first `send-pack` local
  repository, thin, mirror and atomic failure rows.
...
- `docs/cli/matrices/clone_v2_47.tsv` tracks the first `clone`
  unsupported remote-helper, local-dot source, local option, shallow, branch,
  checkout-order, separate-git-dir, bare, reftable and dumb HTTP reference
  variants.
...
- `docs/cli/matrices/diff_index_v2_47.tsv` tracks the first `diff-index`
  algorithm and irreversible-delete variants.
...
- `docs/cli/matrices/grep_v2_47.tsv` tracks the first `grep` pattern,
  line-number, filename-only, fixed-string, cached, treeish, pathspec and
  no-match variants.
- `docs/cli/matrices/hash_object_v2_47.tsv` tracks the first `hash-object`
  path, stdin and write-object variants.
- `docs/cli/matrices/init_v2_47.tsv` tracks the first `init` quiet option
  variants.
- `docs/cli/matrices/interpret_trailers_v2_47.tsv` tracks the first
  `interpret-trailers` stdin parse, output, insertion and duplicate-handling
  variants.
- `docs/cli/matrices/mailinfo_v2_47.tsv` tracks the first `mailinfo`
  stdin patch-mail msg/patch split variants.
- `docs/cli/matrices/mailsplit_v2_47.tsv` tracks the first `mailsplit`
  mbox and maildir split output variants.
...
- `docs/cli/matrices/patch_id_v2_47.tsv` tracks the first `patch-id`
  default, stable, unstable and verbatim stdin patch stream variants.
...
- `docs/cli/matrices/remote_v2_47.tsv` tracks the first `remote` add, list,
  verbose, get-url, set-url, set-branches, rename, remove, show and prune
  variants, plus the first update variants.
- `docs/cli/matrices/request_pull_v2_47.tsv` tracks the first `request-pull`
  local pushed branch and file URL remote variant.
- `docs/cli/matrices/range_diff_v2_47.tsv` tracks the first `range-diff`
  two-range, three-argument and no-dual-color variants.
- `docs/cli/matrices/replay_v2_47.tsv` tracks the first `replay` linear-range
  usage failure and replay mode variants.
- `docs/cli/matrices/rerere_v2_47.tsv` tracks the first `rerere`
  invalid-operation variant.
- `docs/cli/matrices/rm_v2_47.tsv` tracks the first `rm` cached pathspec
  variant.
- `docs/cli/matrices/send_email_v2_47.tsv` tracks the first `send-email`
  alias dump and alias translation variants.
- `docs/cli/matrices/rev_list_v2_47.tsv` tracks the first `rev-list`
  traversal, range, exclusion, count, parent and object variants.
...
- `docs/cli/matrices/show_v2_47.tsv` tracks the first `show` root commit,
  root patch, raw format and NUL-delimited name-status variants.
...
- `docs/cli/matrices/shortlog_v2_47.tsv` tracks the first `shortlog`
  author-summary, summary-sort, email, no-merges and range variants.
- `docs/cli/matrices/stripspace_v2_47.tsv` tracks the first `stripspace`
  default, strip, comment-lines and strip-comments stdin text variants.
...
| Commands with any matrix rows | `106/151` | no | audit rows exist for `add`, `am`, `apply`, `archive`, `bisect`, `blame`, `branch`, `bugreport`, `bundle`, `cat-file`, `check-attr`, `check-ignore`, `check-mailmap`, `check-ref-format`, `cherry`, `cherry-pick`, `checkout`, `checkout-index`, `clean`, `clone`, `column`, `commit`, `commit-graph`, `commit-tree`, `config`, `count-objects`, `credential`, `credential-cache`, `credential-store`, `describe`, `diff`, `diff-files`, `diff-index`, `diff-tree`, `difftool`, `fast-import`, `fetch`, `fetch-pack`, `filter-branch`, `fmt-merge-msg`, `for-each-ref`, `format-patch`, `fsck`, `get-tar-commit-id`, `grep`, `hash-object`, `http-backend`, `http-fetch`, `index-pack`, `init`, `interpret-trailers`, `log`, `ls-files`, `ls-remote`, `ls-tree`, `mailinfo`, `mailsplit`, `maintenance`, `merge`, `merge-base`, `mktag`, `mktree`, `multi-pack-index`, `notes`, `p4`, `pack-objects`, `patch-id`, `prune`, `pull`, `push`, `quiltimport`, `range-diff`, `read-tree`, `rebase`, `reflog`, `remote`, `replace`, `request-pull`, `replay`, `rerere`, `rev-list`, `rev-parse`, `rm`, `send-email`, `send-pack`, `show`, `show-index`, `show-ref`, `shortlog`, `sparse-checkout`, `stash`, `status`, `stripspace`, `submodule`, `symbolic-ref`, `tag`, `unpack-file`, `update-index`, `update-ref`, `update-server-info`, `var`, `verify-pack`, `version`, `worktree` and `write-tree` |
| Git doc option pairs represented by rows | `716/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `2826` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2437/2826` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2826` | no | written rows with incomplete parity |
| Open written rows | `1/2826` | no | written rows that still do not match stock Git |
| Invalid input rows | `388/2826` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Setup and Config | `6` | `0` | `276` | `0` | `166` | `149` | `0` | `0` | `17` | `24` |
| Getting and Creating Projects | `2` | `0` | `66` | `0` | `76` | `64` | `0` | `1` | `11` | `47` |
| Basic Snapshotting | `9` | `0` | `371` | `0` | `316` | `283` | `0` | `0` | `33` | `145` |
| Branching and Merging | `9` | `0` | `581` | `0` | `292` | `248` | `0` | `0` | `44` | `50` |
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `413` | `388` | `0` | `0` | `25` | `160` |
| Inspection and Comparison | `7` | `0` | `774` | `0` | `405` | `396` | `0` | `0` | `9` | `52` |
| Patching | `5` | `0` | `333` | `0` | `21` | `19` | `0` | `0` | `2` | `11` |
| Debugging | `3` | `0` | `132` | `0` | `184` | `103` | `0` | `0` | `81` | `166` |
| Email | `6` | `0` | `361` | `0` | `40` | `40` | `0` | `0` | `0` | `17` |
| External Systems | `2` | `0` | `120` | `0` | `0` | `0` | `0` | `0` | `0` | `0` |
| Administration | `8` | `0` | `147` | `0` | `155` | `94` | `0` | `0` | `61` | `58` |
| Server Admin | `2` | `0` | `30` | `0` | `2` | `2` | `0` | `0` | `0` | `0` |
| Plumbing Commands | `21` | `0` | `650` | `0` | `580` | `512` | `0` | `0` | `68` | `186` |
| Other Git 2.47 commands | `70` | `0` | `1069` | `0` | `174` | `137` | `0` | `0` | `37` | `102` |
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2826`** | **`2437`** | **`0`** | **`1`** | **`388`** | **`1018`** |
...
Never use `151/151` command presence, `4632` option spellings, `716/4632`
represented option pairs or `2437/2826` passing written rows as a Git support
percentage. The `2437/2826` number is audit progress for supported rows already
written down; `0/2826` rows are partial, `1/2826` rows are open and `388/2826`
...
| --- | ---: | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | --- |
| `add` | `34` | `0` | `18` | `docs/cli/matrices/add_v2_47.tsv` | `51` | `39` | `0` | `0` | `12` | no |
| `am` | `55` | `0` | `0` | `docs/cli/matrices/am_v2_47.tsv` | `1` | `1` | `0` | `0` | `0` | no |
| `apply` | `39` | `0` | `4` | `docs/cli/matrices/apply_v2_47.tsv` | `20` | `19` | `0` | `0` | `1` | no |
| `archive` | `17` | `0` | `7` | `docs/cli/matrices/archive_v2_47.tsv` | `17` | `16` | `0` | `0` | `1` | no |
...
| `checkout-index` | `19` | `0` | `5` | `docs/cli/matrices/checkout_index_v2_47.tsv` | `7` | `6` | `0` | `0` | `1` | no |
...
| `credential-cache` | `2` | `0` | `2` | `docs/cli/matrices/credential_cache_v2_47.tsv` | `6` | `6` | `0` | `0` | `0` | no |
| `credential-store` | `1` | `0` | `1` | `docs/cli/matrices/credential_store_v2_47.tsv` | `5` | `5` | `0` | `0` | `0` | no |
| `describe` | `18` | `0` | `8` | `docs/cli/matrices/describe_v2_47.tsv` | `12` | `12` | `0` | `0` | `0` | no |
| `diff` | `133` | `0` | `69` | `docs/cli/matrices/diff_v2_47.tsv` | `239` | `235` | `0` | `0` | `4` | no |
...
| `fmt-merge-msg` | `9` | `0` | `3` | `docs/cli/matrices/fmt_merge_msg_v2_47.tsv` | `4` | `4` | `0` | `0` | `0` | no |
...
| `fsck` | `17` | `0` | `0` | `docs/cli/matrices/fsck_v2_47.tsv` | `35` | `0` | `0` | `0` | `35` | no |
...
| `hash-object` | `7` | `0` | `3` | `docs/cli/matrices/hash_object_v2_47.tsv` | `7` | `6` | `0` | `0` | `1` | no |
...
| `mktree` | `3` | `0` | `3` | `docs/cli/matrices/mktree_v2_47.tsv` | `4` | `4` | `0` | `0` | `0` | no |
| `multi-pack-index` | `10` | `0` | `1` | `docs/cli/matrices/multi_pack_index_v2_47.tsv` | `6` | `3` | `0` | `0` | `3` | no |
| `notes` | `33` | `0` | `0` | `docs/cli/matrices/notes_v2_47.tsv` | `42` | `35` | `0` | `0` | `7` | no |
| `p4` | `40` | `0` | `0` | `docs/cli/matrices/p4_v2_47.tsv` | `1` | `1` | `0` | `0` | `0` | no |
| `pack-objects` | `44` | `0` | `7` | `docs/cli/matrices/pack_objects_v2_47.tsv` | `18` | `12` | `0` | `0` | `6` | no |
...
| `push` | `57` | `0` | `1` | `docs/cli/matrices/push_v2_47.tsv` | `7` | `6` | `0` | `0` | `1` | no |
...
| `request-pull` | `1` | `0` | `0` | `docs/cli/matrices/request_pull_v2_47.tsv` | `3` | `3` | `0` | `0` | `0` | no |
...
| `rev-parse` | `72` | `0` | `24` | `docs/cli/matrices/rev_parse_v2_47.tsv` | `74` | `68` | `0` | `0` | `6` | no |
...
| `show-ref` | `14` | `0` | `9` | `docs/cli/matrices/show_ref_v2_47.tsv` | `19` | `16` | `0` | `0` | `3` | no |
...
| `symbolic-ref` | `8` | `0` | `3` | `docs/cli/matrices/symbolic_ref_v2_47.tsv` | `10` | `9` | `0` | `0` | `1` | no |
| `tag` | `40` | `0` | `11` | `docs/cli/matrices/tag_v2_47.tsv` | `27` | `17` | `0` | `0` | `10` | no |
| `unpack-file` | `0` | `0` | `0` | `docs/cli/matrices/unpack_file_v2_47.tsv` | `4` | `2` | `0` | `0` | `2` | no |
| `update-index` | `53` | `0` | `1` | `docs/cli/matrices/update_index_v2_47.tsv` | `2` | `2` | `0` | `0` | `0` | no |
...
| `verify-pack` | `4` | `0` | `2` | `docs/cli/matrices/verify_pack_v2_47.tsv` | `6` | `4` | `0` | `0` | `2` | no |
...
| `write-tree` | `2` | `0` | `2` | `docs/cli/matrices/write_tree_v2_47.tsv` | `3` | `3` | `0` | `0` | `0` | no |
