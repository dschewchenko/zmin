...
| Written behavior rows | `670` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `597/670` | yes, row by row | exact written rows with parity evidence |
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `210` | `205` | `0` | `2` | `3` | `94` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`670`** | **`597`** | **`0`** | **`2`** | **`71`** | **`360`** |
...
represented option pairs or `597/670` passing written rows as a Git support
percentage. The `597/670` number is audit progress for rows already written
...
| `fetch` | `73` | `0` | `28` | `docs/cli/matrices/fetch_v2_47.tsv` | `210` | `205` | `0` | `2` | `3` | no |
...
uninitialized locally:
Zmin fetches the target submodule commit into the initialized submodule object
database without checking it out for initialized submodules and leaves
uninitialized submodules uninitialized, matching stock Git. Remaining submodule
recursion modes, non-local submodule remotes, nested recursion and path-limited
recursion behavior remain open.
