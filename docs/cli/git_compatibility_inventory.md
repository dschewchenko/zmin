...
| Written behavior rows | `751` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `675/751` | yes, row by row | exact written rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`751`** | **`675`** | **`0`** | **`0`** | **`76`** | **`419`** |
...
represented option pairs or `675/751` passing written rows as a Git support
percentage. The `675/751` number is audit progress for rows already written
