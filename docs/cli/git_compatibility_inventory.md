...
| Commands with any matrix rows | `35/151` | no | audit rows exist for `archive`, `bisect`, `blame`, `branch`, `bundle`, `cat-file`, `clean`, `column`, `commit-graph`, `config`, `diff`, `fast-import`, `fetch`, `for-each-ref`, `index-pack`, `log`, `ls-files`, `ls-tree`, `merge`, `merge-base`, `multi-pack-index`, `notes`, `pack-objects`, `rebase`, `reflog`, `rerere`, `rev-parse`, `show-index`, `show-ref`, `status`, `submodule`, `symbolic-ref`, `tag`, `verify-pack` and `version` |
| Git doc option pairs represented by rows | `241/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `1000` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `770/1000` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `1/1000` | no | written rows with incomplete parity |
...
| Other Git 2.47 commands | `71` | `0` | `1075` | `0` | `26` | `7` | `1` | `0` | `18` | `27` |
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1000`** | **`770`** | **`1`** | **`0`** | **`229`** | **`647`** |
...
represented option pairs or `770/1000` passing written rows as a Git support
percentage. The `770/1000` number is audit progress for supported rows already
written down; `1/1000` row is partial and `229/1000` additional rows are
...
| `fast-import` | `25` | `0` | `1` | `docs/cli/matrices/fast_import_v2_47.tsv` | `2` | `0` | `1` | `0` | `1` | no |
