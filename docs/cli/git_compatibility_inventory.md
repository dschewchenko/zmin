...
| Written behavior rows | `2567` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2192/2567` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2567` | no | written rows with incomplete parity |
| Open written rows | `1/2567` | no | written rows that still do not match stock Git |
| Invalid input rows | `374/2567` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2567`** | **`2192`** | **`0`** | **`1`** | **`374`** | **`1018`** |
...
represented option pairs or `2192/2567` passing written rows as a Git support
percentage. The `2192/2567` number is audit progress for supported rows already
written down; `0/2567` rows are partial, `1/2567` rows are open and `374/2567`
...
| `fetch` | `73` | `0` | `30` | `docs/cli/matrices/fetch_v2_47.tsv` | `321` | `311` | `0` | `0` | `10` | no |
