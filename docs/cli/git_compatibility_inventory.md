...
| Git doc option pairs represented by rows | `280/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `1150` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `868/1150` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1150` | no | written rows with incomplete parity |
| Open written rows | `1/1150` | no | written rows that still do not match stock Git |
| Invalid input rows | `281/1150` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Plumbing Commands | `21` | `0` | `650` | `0` | `227` | `165` | `0` | `0` | `62` | `116` |
| Other Git 2.47 commands | `70` | `0` | `1069` | `0` | `48` | `21` | `0` | `0` | `27` | `40` |
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1150`** | **`868`** | **`0`** | **`1`** | **`281`** | **`721`** |
...
Never use `151/151` command presence, `4632` option spellings, `280/4632`
represented option pairs or `868/1150` passing written rows as a Git support
percentage. The `868/1150` number is audit progress for supported rows already
written down; `0/1150` rows are partial, `1/1150` rows are open and `281/1150`
