...
| Written behavior rows | `1126` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `854/1126` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1126` | no | written rows with incomplete parity |
| Open written rows | `1/1126` | no | written rows that still do not match stock Git |
| Invalid input rows | `271/1126` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1126`** | **`854`** | **`0`** | **`1`** | **`271`** | **`721`** |
...
represented option pairs or `854/1126` passing written rows as a Git support
percentage. The `854/1126` number is audit progress for supported rows already
written down; `0/1126` rows are partial, `1/1126` rows are open and `271/1126`
