...
| Written behavior rows | `766` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `690/766` | yes, row by row | exact written rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`766`** | **`690`** | **`0`** | **`0`** | **`76`** | **`434`** |
...
represented option pairs or `690/766` passing written rows as a Git support
percentage. The `690/766` number is audit progress for rows already written
