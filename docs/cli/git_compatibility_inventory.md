...
| Written behavior rows | `858` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `739/858` | yes, row by row | supported-behavior rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`858`** | **`739`** | **`0`** | **`0`** | **`119`** | **`508`** |
...
represented option pairs or `739/858` passing written rows as a Git support
percentage. The `739/858` number is audit progress for supported rows already
written down; `119/858` additional rows are stock-compatible invalid inputs. It
