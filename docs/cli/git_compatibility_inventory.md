...
| Written behavior rows | `688` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `614/688` | yes, row by row | exact written rows with parity evidence |
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `228` | `222` | `0` | `2` | `4` | `110` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`688`** | **`614`** | **`0`** | **`2`** | **`72`** | **`376`** |
...
represented option pairs or `614/688` passing written rows as a Git support
percentage. The `614/688` number is audit progress for rows already written
...
| `fetch` | `73` | `0` | `28` | `docs/cli/matrices/fetch_v2_47.tsv` | `228` | `222` | `0` | `2` | `4` | no |
...
and `--no-recurse-submodules` value forms. The latest uninitialized slice
closes implicit yes plus on-demand forms for a smart HTTP parent fetch with the
submodule still uninitialized locally. The latest nested slice also closes
