...
| Written behavior rows | `2480` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2129/2480` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2480` | no | written rows with incomplete parity |
| Open written rows | `1/2480` | no | written rows that still do not match stock Git |
| Invalid input rows | `350/2480` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `274` | `246` | `0` | `0` | `28` | `145` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2480`** | **`2129`** | **`0`** | **`1`** | **`350`** | **`1018`** |
...
represented option pairs or `2129/2480` passing written rows as a Git support
percentage. The `2129/2480` number is audit progress for supported rows already
written down; `0/2480` rows are partial, `1/2480` rows are open and `350/2480`
