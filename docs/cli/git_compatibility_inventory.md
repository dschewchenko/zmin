...
| Written behavior rows | `832` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `732/832` | yes, row by row | supported-behavior rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`832`** | **`732`** | **`0`** | **`0`** | **`100`** | **`496`** |
...
represented option pairs or `732/832` passing written rows as a Git support
percentage. The `732/832` number is audit progress for supported rows already
written down; `100/832` additional rows are stock-compatible invalid inputs. It
