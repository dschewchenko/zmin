...
| Git doc option pairs represented by rows | `459/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `1857` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `1550/1857` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1857` | no | written rows with incomplete parity |
| Open written rows | `1/1857` | no | written rows that still do not match stock Git |
| Invalid input rows | `306/1857` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1857`** | **`1550`** | **`0`** | **`1`** | **`306`** | **`993`** |
...
Never use `151/151` command presence, `4632` option spellings, `459/4632`
represented option pairs or `1550/1857` passing written rows as a Git support
percentage. The `1550/1857` number is audit progress for supported rows already
written down; `0/1857` rows are partial, `1/1857` rows are open and `306/1857`
