...
- `docs/cli/matrices/clone_v2_47.tsv` tracks the first `clone`
  ...
  checkout-order, reftable and dumb HTTP reference variants.
...
| Git doc option pairs represented by rows | `377/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `1598` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `1306/1598` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1598` | no | written rows with incomplete parity |
| Open written rows | `1/1598` | no | written rows that still do not match stock Git |
| Invalid input rows | `291/1598` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Getting and Creating Projects | `2` | `0` | `66` | `0` | `34` | `31` | `0` | `1` | `2` | `28` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1598`** | **`1306`** | **`0`** | **`1`** | **`291`** | **`731`** |
...
Never use `151/151` command presence, `4632` option spellings, `377/4632`
represented option pairs or `1306/1598` passing written rows as a Git support
percentage. The `1306/1598` number is audit progress for supported rows already
written down; `0/1598` rows are partial, `1/1598` rows are open and `291/1598`
...
| `clone` | `56` | `0` | `17` | `docs/cli/matrices/clone_v2_47.tsv` | `32` | `29` | `0` | `1` | `2` | no |
