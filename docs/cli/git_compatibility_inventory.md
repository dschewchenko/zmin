...
| Git doc option pairs represented by rows | `279/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `1145` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `863/1145` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1145` | no | written rows with incomplete parity |
| Open written rows | `1/1145` | no | written rows that still do not match stock Git |
| Invalid input rows | `281/1145` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Plumbing Commands | `20` | `0` | `644` | `0` | `222` | `160` | `0` | `0` | `62` | `116` |
| Other Git 2.47 commands | `71` | `0` | `1075` | `0` | `48` | `21` | `0` | `0` | `27` | `40` |
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1145`** | **`863`** | **`0`** | **`1`** | **`281`** | **`721`** |
...
Never use `151/151` command presence, `4632` option spellings, `279/4632`
represented option pairs or `863/1145` passing written rows as a Git support
percentage. The `863/1145` number is audit progress for supported rows already
written down; `0/1145` rows are partial, `1/1145` rows are open and `281/1145`
