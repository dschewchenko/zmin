...
| Written behavior rows | `1013` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `774/1013` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `1/1013` | no | written rows with incomplete parity |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1013`** | **`774`** | **`1`** | **`0`** | **`238`** | **`650`** |
...
represented option pairs or `774/1013` passing written rows as a Git support
percentage. The `774/1013` number is audit progress for supported rows already
written down; `1/1013` row is partial and `238/1013` additional rows are
