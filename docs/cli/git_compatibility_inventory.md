...
| Written behavior rows | `2539` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2165/2539` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2539` | no | written rows with incomplete parity |
| Open written rows | `1/2539` | no | written rows that still do not match stock Git |
| Invalid input rows | `373/2539` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `367` | `342` | `0` | `0` | `25` | `160` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2539`** | **`2165`** | **`0`** | **`1`** | **`373`** | **`1018`** |
...
represented option pairs or `2165/2539` passing written rows as a Git support
percentage. The `2165/2539` number is audit progress for supported rows already
written down; `0/2539` rows are partial, `1/2539` rows are open and `373/2539`
