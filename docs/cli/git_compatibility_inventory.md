...
| Written behavior rows | `802` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `726/802` | yes, row by row | exact written rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`802`** | **`726`** | **`0`** | **`0`** | **`76`** | **`470`** |
...
represented option pairs or `726/802` passing written rows as a Git support
percentage. The `726/802` number is audit progress for rows already written
