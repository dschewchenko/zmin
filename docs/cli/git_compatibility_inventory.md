...
| Written behavior rows | `138` | no by itself | explicit rows currently written |
| Closed behavior rows | `123/138` | yes, row by row | exact written rows that match stock Git |
...
| Sharing and Updating Projects | `5` | `309` | `78` | `67` | `0` | `9` | `2` | `0` |
...
| **Git 2.47 unique total** | **`151`** | **`4632`** | **`138`** | **`123`** | **`0`** | **`9`** | **`6`** | **`253`** |
...
Never use `151/151` command presence, `4632` option spellings or `123/138`
closed written rows as a Git support percentage. The `123/138` number is audit
...
| `fetch` | `73` | `28` | `docs/cli/matrices/fetch_v2_47.tsv` | `78` | `67` | `0` | `9` | `2` | no |
...
`FETCH_HEAD` modes, multiple explicit refspecs and local/file `--all`
acceptance. Zmin invokes the external upload-pack command where stock Git does
for those local/file forms and preserves stock Git's local/file `--all`
behavior, where the custom upload-pack command is not invoked. SSH upload-pack
override plus shallow and `--multiple` combinations remain open.
