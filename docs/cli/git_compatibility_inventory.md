...
| Written behavior rows | `2646` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2269/2646` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2646` | no | written rows with incomplete parity |
| Open written rows | `1/2646` | no | written rows that still do not match stock Git |
| Invalid input rows | `376/2646` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2646`** | **`2269`** | **`0`** | **`1`** | **`376`** | **`1018`** |
...
represented option pairs or `2269/2646` passing written rows as a Git support
percentage. The `2269/2646` number is audit progress for supported rows already
written down; `0/2646` rows are partial, `1/2646` rows are open and `376/2646`
