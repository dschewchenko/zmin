...
| Written behavior rows | `1249` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `966/1249` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1249` | no | written rows with incomplete parity |
| Open written rows | `1/1249` | no | written rows that still do not match stock Git |
| Invalid input rows | `282/1249` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `123` | `104` | `0` | `0` | `19` | `101` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1249`** | **`966`** | **`0`** | **`1`** | **`282`** | **`721`** |
...
represented option pairs or `966/1249` passing written rows as a Git support
percentage. The `966/1249` number is audit progress for supported rows already
written down; `0/1249` rows are partial, `1/1249` rows are open and `282/1249`
