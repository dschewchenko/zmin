...
| Git doc option pairs represented by rows | `568/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `2408` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2063/2408` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2408` | no | written rows with incomplete parity |
| Open written rows | `1/2408` | no | written rows that still do not match stock Git |
| Invalid input rows | `344/2408` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `202` | `180` | `0` | `0` | `22` | `145` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2408`** | **`2063`** | **`0`** | **`1`** | **`344`** | **`1018`** |
...
Never use `151/151` command presence, `4632` option spellings, `568/4632`
represented option pairs or `2063/2408` passing written rows as a Git support
percentage. The `2063/2408` number is audit progress for supported rows already
written down; `0/2408` rows are partial, `1/2408` rows are open and `344/2408`
