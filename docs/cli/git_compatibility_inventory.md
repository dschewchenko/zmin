...
| Written behavior rows | `2140` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `1829/2140` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2140` | no | written rows with incomplete parity |
| Open written rows | `1/2140` | no | written rows that still do not match stock Git |
| Invalid input rows | `310/2140` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Other Git 2.47 commands | `70` | `0` | `1069` | `0` | `128` | `94` | `0` | `0` | `34` | `102` |
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2140`** | **`1829`** | **`0`** | **`1`** | **`310`** | **`1018`** |
...
represented option pairs or `1829/2140` passing written rows as a Git support
percentage. The `1829/2140` number is audit progress for supported rows already
written down; `0/2140` rows are partial, `1/2140` rows are open and `310/2140`
