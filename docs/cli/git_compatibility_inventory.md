...
| Commands with any matrix rows | `51/151` | no | audit rows exist for `add`, `archive`, `bisect`, `blame`, `branch`, `bundle`, `cat-file`, `check-ignore`, `checkout`, `checkout-index`, `clean`, `clone`, `column`, `commit-graph`, `config`, `diff`, `fast-import`, `fetch`, `filter-branch`, `for-each-ref`, `http-fetch`, `index-pack`, `init`, `log`, `ls-files`, `ls-remote`, `ls-tree`, `maintenance`, `merge`, `merge-base`, `multi-pack-index`, `notes`, `p4`, `pack-objects`, `prune`, `push`, `rebase`, `reflog`, `rerere`, `rev-parse`, `show-index`, `show-ref`, `sparse-checkout`, `stash`, `status`, `submodule`, `symbolic-ref`, `tag`, `verify-pack`, `version` and `worktree` |
| Git doc option pairs represented by rows | `278/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `1136` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `863/1136` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1136` | no | written rows with incomplete parity |
| Open written rows | `1/1136` | no | written rows that still do not match stock Git |
| Invalid input rows | `272/1136` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `85` | `66` | `0` | `0` | `19` | `101` |
...
| Plumbing Commands | `20` | `0` | `644` | `0` | `213` | `160` | `0` | `0` | `53` | `116` |
| Other Git 2.47 commands | `71` | `0` | `1075` | `0` | `48` | `21` | `0` | `0` | `27` | `40` |
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1136`** | **`863`** | **`0`** | **`1`** | **`272`** | **`721`** |
...
Never use `151/151` command presence, `4632` option spellings, `278/4632`
represented option pairs or `863/1136` passing written rows as a Git support
percentage. The `863/1136` number is audit progress for supported rows already
written down; `0/1136` rows are partial, `1/1136` rows are open and `272/1136`
...
| `check-ignore` | `10` | `0` | `6` | `docs/cli/matrices/check_ignore_v2_47.tsv` | `10` | `9` | `0` | `0` | `1` | no |
