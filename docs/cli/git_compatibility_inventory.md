...
| Written behavior rows | `997` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `770/997` | yes, row by row | supported-behavior rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`997`** | **`770`** | **`0`** | **`0`** | **`227`** | **`646`** |
...
represented option pairs or `770/997` passing written rows as a Git support
percentage. The `770/997` number is audit progress for supported rows already
written down; `227/997` additional rows are stock-compatible invalid inputs. It
