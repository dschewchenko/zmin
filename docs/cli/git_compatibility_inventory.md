...
| Written behavior rows | `848` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `732/848` | yes, row by row | supported-behavior rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`848`** | **`732`** | **`0`** | **`0`** | **`116`** | **`505`** |
...
represented option pairs or `732/848` passing written rows as a Git support
percentage. The `732/848` number is audit progress for supported rows already
written down; `116/848` additional rows are stock-compatible invalid inputs. It
