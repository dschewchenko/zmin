...
| Written behavior rows | `1085` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `818/1085` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1085` | no | written rows with incomplete parity |
| Open written rows | `1/1085` | no | written rows that still do not match stock Git |
| Invalid input rows | `266/1085` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `85` | `66` | `0` | `0` | `19` | `99` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1085`** | **`818`** | **`0`** | **`1`** | **`266`** | **`683`** |
...
represented option pairs or `818/1085` passing written rows as a Git support
percentage. The `818/1085` number is audit progress for supported rows already
written down; `0/1085` rows are partial, `1/1085` rows are open and `266/1085`
