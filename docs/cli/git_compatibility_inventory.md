...
| Written behavior rows | `2312` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `1986/2312` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2312` | no | written rows with incomplete parity |
| Open written rows | `1/2312` | no | written rows that still do not match stock Git |
| Invalid input rows | `325/2312` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Branching and Merging | `9` | `0` | `581` | `0` | `255` | `213` | `0` | `0` | `42` | `50` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2312`** | **`1986`** | **`0`** | **`1`** | **`325`** | **`1018`** |
...
represented option pairs or `1986/2312` passing written rows as a Git support
percentage. The `1986/2312` number is audit progress for supported rows already
written down; `0/2312` rows are partial, `1/2312` rows are open and `325/2312`
