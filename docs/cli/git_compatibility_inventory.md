...
| Written behavior rows | `189` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Behavior rows matching stock Git | `174/189` | yes, row by row | exact written rows with parity evidence |
...
| Sharing and Updating Projects | `5` | `309` | `129` | `118` | `0` | `9` | `2` | `18` |
...
| **Git 2.47 unique total** | **`151`** | **`4632`** | **`189`** | **`174`** | **`0`** | **`9`** | **`6`** | **`271`** |
...
Never use `151/151` command presence, `4632` option spellings or `174/189`
passing written rows as a Git support percentage. The `174/189` number is audit
...
| `fetch` | `73` | `28` | `docs/cli/matrices/fetch_v2_47.tsv` | `129` | `118` | `0` | `9` | `2` | no |
...
The latest `fetch --unshallow` slice closed explicit local path/file URL branch
fetches for existing shallow repos.
