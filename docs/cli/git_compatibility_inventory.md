...
| Written behavior rows | `769` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `693/769` | yes, row by row | exact written rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`769`** | **`693`** | **`0`** | **`0`** | **`76`** | **`437`** |
...
represented option pairs or `693/769` passing written rows as a Git support
percentage. The `693/769` number is audit progress for rows already written
