...
| Written behavior rows | `2430` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2084/2430` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2430` | no | written rows with incomplete parity |
| Open written rows | `1/2430` | no | written rows that still do not match stock Git |
| Invalid input rows | `345/2430` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `224` | `201` | `0` | `0` | `23` | `145` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2430`** | **`2084`** | **`0`** | **`1`** | **`345`** | **`1018`** |
...
represented option pairs or `2084/2430` passing written rows as a Git support
percentage. The `2084/2430` number is audit progress for supported rows already
written down; `0/2430` rows are partial, `1/2430` rows are open and `345/2430`
