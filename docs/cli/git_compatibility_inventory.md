...
| Written behavior rows | `2420` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2075/2420` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2420` | no | written rows with incomplete parity |
| Open written rows | `1/2420` | no | written rows that still do not match stock Git |
| Invalid input rows | `344/2420` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `214` | `192` | `0` | `0` | `22` | `145` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2420`** | **`2075`** | **`0`** | **`1`** | **`344`** | **`1018`** |
...
represented option pairs or `2075/2420` passing written rows as a Git support
percentage. The `2075/2420` number is audit progress for supported rows already
written down; `0/2420` rows are partial, `1/2420` rows are open and `344/2420`
