...
| Written behavior rows | `2498` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2144/2498` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2498` | no | written rows with incomplete parity |
| Open written rows | `1/2498` | no | written rows that still do not match stock Git |
| Invalid input rows | `353/2498` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `403` | `391` | `0` | `0` | `9` | `52` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2498`** | **`2144`** | **`0`** | **`1`** | **`353`** | **`1018`** |
...
represented option pairs or `2144/2498` passing written rows as a Git support
percentage. The `2144/2498` number is audit progress for supported rows already
written down; `0/2498` rows are partial, `1/2498` rows are open and `353/2498`
