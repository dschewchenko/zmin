...
| Written behavior rows | `1079` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `814/1079` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1079` | no | written rows with incomplete parity |
| Open written rows | `1/1079` | no | written rows that still do not match stock Git |
| Invalid input rows | `264/1079` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `303` | `286` | `0` | `0` | `17` | `159` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1079`** | **`814`** | **`0`** | **`1`** | **`264`** | **`683`** |
...
represented option pairs or `814/1079` passing written rows as a Git support
percentage. The `814/1079` number is audit progress for supported rows already
written down; `0/1079` rows are partial, `1/1079` rows are open and `264/1079`
...
| `fetch` | `73` | `0` | `30` | `docs/cli/matrices/fetch_v2_47.tsv` | `294` | `286` | `0` | `0` | `8` | no |
