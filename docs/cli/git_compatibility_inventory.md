...
| Written behavior rows | `754` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `678/754` | yes, row by row | exact written rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`754`** | **`678`** | **`0`** | **`0`** | **`76`** | **`422`** |
...
represented option pairs or `678/754` passing written rows as a Git support
percentage. The `678/754` number is audit progress for rows already written
