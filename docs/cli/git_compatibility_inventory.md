...
| Written behavior rows | `250` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `234/250` | yes, row by row | exact written rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`250`** | **`234`** | **`0`** | **`9`** | **`7`** | **`330`** |
...
represented option pairs or `234/250` passing written rows as a Git support
percentage. The `234/250` number is audit progress for rows already written
