...
| Written behavior rows | `2764` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2376/2764` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2764` | no | written rows with incomplete parity |
| Open written rows | `1/2764` | no | written rows that still do not match stock Git |
| Invalid input rows | `387/2764` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2764`** | **`2376`** | **`0`** | **`1`** | **`387`** | **`1018`** |
...
represented option pairs or `2376/2764` passing written rows as a Git support
percentage. The `2376/2764` number is audit progress for supported rows already
written down; `0/2764` rows are partial, `1/2764` rows are open and `387/2764`
