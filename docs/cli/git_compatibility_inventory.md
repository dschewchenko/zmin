...
| Written behavior rows | `710` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `636/710` | yes, row by row | exact written rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`710`** | **`636`** | **`0`** | **`0`** | **`74`** | **`401`** |
...
represented option pairs or `636/710` passing written rows as a Git support
percentage. The `636/710` number is audit progress for rows already written
