...
| Git doc option pairs represented by rows | `697/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `2790` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2402/2790` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2790` | no | written rows with incomplete parity |
| Open written rows | `1/2790` | no | written rows that still do not match stock Git |
| Invalid input rows | `387/2790` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Setup and Config | `6` | `0` | `276` | `0` | `164` | `147` | `0` | `0` | `17` | `24` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2790`** | **`2402`** | **`0`** | **`1`** | **`387`** | **`1018`** |
...
Never use `151/151` command presence, `4632` option spellings, `697/4632`
represented option pairs or `2402/2790` passing written rows as a Git support
percentage. The `2402/2790` number is audit progress for supported rows already
written down; `0/2790` rows are partial, `1/2790` rows are open and `387/2790`
