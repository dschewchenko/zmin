...
| Written behavior rows | `173` | no by itself | explicit rows currently written |
| Behavior rows matching stock Git | `158/173` | yes, row by row | exact written rows with parity evidence |
...
| Sharing and Updating Projects | `5` | `309` | `113` | `102` | `0` | `9` | `2` | `2` |
...
| **Git 2.47 unique total** | **`151`** | **`4632`** | **`173`** | **`158`** | **`0`** | **`9`** | **`6`** | **`255`** |
...
Never use `151/151` command presence, `4632` option spellings or `158/173`
passing written rows as a Git support percentage. The `158/173` number is audit
...
| `fetch` | `73` | `29` | `docs/cli/matrices/fetch_v2_47.tsv` | `113` | `102` | `0` | `9` | `2` | no |
...
The latest `fetch --update-shallow` slice closed named local path and file URL
remotes where the source remote itself is shallow.
