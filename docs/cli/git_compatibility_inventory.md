...
| Written behavior rows | `1534` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `1242/1534` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1534` | no | written rows with incomplete parity |
| Open written rows | `1/1534` | no | written rows that still do not match stock Git |
| Invalid input rows | `291/1534` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `144` | `125` | `0` | `0` | `19` | `101` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1534`** | **`1242`** | **`0`** | **`1`** | **`291`** | **`721`** |
...
represented option pairs or `1242/1534` passing written rows as a Git support
percentage. The `1242/1534` number is audit progress for supported rows already
written down; `0/1534` rows are partial, `1/1534` rows are open and `291/1534`
