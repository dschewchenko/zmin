...
| Written behavior rows | `2494` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2142/2494` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2494` | no | written rows with incomplete parity |
| Open written rows | `1/2494` | no | written rows that still do not match stock Git |
| Invalid input rows | `351/2494` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `288` | `259` | `0` | `0` | `29` | `145` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2494`** | **`2142`** | **`0`** | **`1`** | **`351`** | **`1018`** |
...
represented option pairs or `2142/2494` passing written rows as a Git support
percentage. The `2142/2494` number is audit progress for supported rows already
written down; `0/2494` rows are partial, `1/2494` rows are open and `351/2494`
