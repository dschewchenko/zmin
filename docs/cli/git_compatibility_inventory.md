...
| Git doc option pairs represented by rows | `344/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `1377` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `1094/1377` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1377` | no | written rows with incomplete parity |
| Open written rows | `1/1377` | no | written rows that still do not match stock Git |
| Invalid input rows | `282/1377` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Inspection and Comparison | `7` | `0` | `774` | `0` | `198` | `191` | `0` | `0` | `7` | `26` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1377`** | **`1094`** | **`0`** | **`1`** | **`282`** | **`721`** |
...
Never use `151/151` command presence, `4632` option spellings, `344/4632`
represented option pairs or `1094/1377` passing written rows as a Git support
percentage. The `1094/1377` number is audit progress for supported rows already
written down; `0/1377` rows are partial, `1/1377` rows are open and `282/1377`
...
| `log` | `282` | `0` | `35` | `docs/cli/matrices/log_v2_47.tsv` | `95` | `90` | `0` | `0` | `5` | no |
