...
| Commands with any matrix rows | `56/151` | no | audit rows exist for `add`, `archive`, `bisect`, `blame`, `branch`, `bundle`, `cat-file`, `check-attr`, `check-ignore`, `checkout`, `checkout-index`, `clean`, `clone`, `column`, `commit-graph`, `config`, `diff`, `diff-tree`, `fast-import`, `fetch`, `filter-branch`, `for-each-ref`, `http-fetch`, `index-pack`, `init`, `log`, `ls-files`, `ls-remote`, `ls-tree`, `maintenance`, `merge`, `merge-base`, `multi-pack-index`, `notes`, `p4`, `pack-objects`, `prune`, `push`, `rebase`, `reflog`, `remote`, `rerere`, `rev-parse`, `rm`, `show-index`, `show-ref`, `sparse-checkout`, `stash`, `status`, `submodule`, `symbolic-ref`, `tag`, `var`, `verify-pack`, `version` and `worktree` |
| Git doc option pairs represented by rows | `353/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `1538` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `1246/1538` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1538` | no | written rows with incomplete parity |
| Open written rows | `1/1538` | no | written rows that still do not match stock Git |
| Invalid input rows | `291/1538` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1538`** | **`1246`** | **`0`** | **`1`** | **`291`** | **`721`** |
...
Never use `151/151` command presence, `4632` option spellings, `353/4632`
represented option pairs or `1246/1538` passing written rows as a Git support
percentage. The `1246/1538` number is audit progress for supported rows already
written down; `0/1538` rows are partial, `1/1538` rows are open and `291/1538`
