...
| Written behavior rows | `1073` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `811/1073` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1073` | no | written rows with incomplete parity |
| Open written rows | `1/1073` | no | written rows that still do not match stock Git |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1073`** | **`811`** | **`0`** | **`1`** | **`261`** | **`683`** |
...
represented option pairs or `811/1073` passing written rows as a Git support
percentage. The `811/1073` number is audit progress for supported rows already
written down; `0/1073` rows are partial, `1/1073` rows are open and `261/1073`
