...
| Written behavior rows | `2396` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2054/2396` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2396` | no | written rows with incomplete parity |
| Open written rows | `1/2396` | no | written rows that still do not match stock Git |
| Invalid input rows | `341/2396` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Basic Snapshotting | `9` | `0` | `371` | `0` | `190` | `171` | `0` | `0` | `19` | `145` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2396`** | **`2054`** | **`0`** | **`1`** | **`341`** | **`1018`** |
...
represented option pairs or `2054/2396` passing written rows as a Git support
percentage. The `2054/2396` number is audit progress for supported rows already
written down; `0/2396` rows are partial, `1/2396` rows are open and `341/2396`
