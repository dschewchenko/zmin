...
| Written behavior rows | `768` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `692/768` | yes, row by row | exact written rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`768`** | **`692`** | **`0`** | **`0`** | **`76`** | **`436`** |
...
represented option pairs or `692/768` passing written rows as a Git support
percentage. The `692/768` number is audit progress for rows already written
