...
| Written behavior rows | `1828` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `1521/1828` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1828` | no | written rows with incomplete parity |
| Open written rows | `1/1828` | no | written rows that still do not match stock Git |
| Invalid input rows | `306/1828` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Administration | `8` | `0` | `147` | `0` | `66` | `42` | `0` | `0` | `24` | `54` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1828`** | **`1521`** | **`0`** | **`1`** | **`306`** | **`964`** |
...
represented option pairs or `1521/1828` passing written rows as a Git support
percentage. The `1521/1828` number is audit progress for supported rows already
written down; `0/1828` rows are partial, `1/1828` rows are open and `306/1828`
