...
| Written behavior rows | `155` | no by itself | explicit rows currently written |
| Closed behavior rows | `140/155` | yes, row by row | exact written rows that match stock Git |
...
| Sharing and Updating Projects | `5` | `309` | `95` | `84` | `0` | `9` | `2` | `0` |
...
| **Git 2.47 unique total** | **`151`** | **`4632`** | **`155`** | **`140`** | **`0`** | **`9`** | **`6`** | **`253`** |
...
Never use `151/151` command presence, `4632` option spellings or `140/155`
closed written rows as a Git support percentage. The `140/155` number is audit
...
| `fetch` | `73` | `28` | `docs/cli/matrices/fetch_v2_47.tsv` | `95` | `84` | `0` | `9` | `2` | no |
...
local/file explicit-branch `--deepen=1` and local/file `--unshallow` forms in
...
override plus shallow-since and shallow-exclude combinations remain open.
