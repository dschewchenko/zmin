...
| Written behavior rows | `809` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `727/809` | yes, row by row | supported-behavior rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`809`** | **`727`** | **`0`** | **`0`** | **`82`** | **`473`** |
...
represented option pairs or `727/809` passing written rows as a Git support
percentage. The `727/809` number is audit progress for supported rows already
written down; `82/809` additional rows are stock-compatible invalid inputs. It
