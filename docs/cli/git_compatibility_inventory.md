...
| Written behavior rows | `777` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `701/777` | yes, row by row | exact written rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`777`** | **`701`** | **`0`** | **`0`** | **`76`** | **`445`** |
...
represented option pairs or `701/777` passing written rows as a Git support
percentage. The `701/777` number is audit progress for rows already written
