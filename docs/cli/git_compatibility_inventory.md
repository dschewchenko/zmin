...
| Written behavior rows | `1969` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `1658/1969` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/1969` | no | written rows with incomplete parity |
| Open written rows | `1/1969` | no | written rows that still do not match stock Git |
| Invalid input rows | `310/1969` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`1969`** | **`1658`** | **`0`** | **`1`** | **`310`** | **`1018`** |
...
represented option pairs or `1658/1969` passing written rows as a Git support
percentage. The `1658/1969` number is audit progress for supported rows already
written down; `0/1969` rows are partial, `1/1969` rows are open and `310/1969`
