...
| Commands with any matrix rows | `12/151` | no | audit rows exist for `branch`, `config`, `status`, `fetch`, `diff`, `log`, `for-each-ref`, `ls-files`, `rev-parse`, `show-ref`, `symbolic-ref` and `tag` |
| Git doc option pairs represented by rows | `210/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `598` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `524/598` | yes, row by row | exact written rows with parity evidence |
...
| Plumbing Commands | `20` | `0` | `644` | `0` | `130` | `105` | `0` | `0` | `25` | `76` |
| Other Git 2.47 commands | `71` | `0` | `1075` | `0` | `0` | `0` | `0` | `0` | `0` | `4` |
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`598`** | **`524`** | **`0`** | **`9`** | **`65`** | **`330`** |
...
Never use `151/151` command presence, `4632` option spellings, `210/4632`
represented option pairs or `524/598` passing written rows as a Git support
percentage. The `524/598` number is audit progress for rows already written
