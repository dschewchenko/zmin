...
| Git doc option pairs represented by rows | `218/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `671` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `598/671` | yes, row by row | exact written rows with parity evidence |
...
| Sharing and Updating Projects | `5` | `0` | `309` | `0` | `211` | `206` | `0` | `2` | `3` | `95` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`671`** | **`598`** | **`0`** | **`2`** | **`71`** | **`361`** |
...
Never use `151/151` command presence, `4632` option spellings, `218/4632`
represented option pairs or `598/671` passing written rows as a Git support
percentage. The `598/671` number is audit progress for rows already written
...
| `fetch` | `73` | `0` | `28` | `docs/cli/matrices/fetch_v2_47.tsv` | `211` | `206` | `0` | `2` | `3` | no |
...
| `rev-parse` | `72` | `0` | `21` | `docs/cli/matrices/rev_parse_v2_47.tsv` | `31` | `29` | `0` | `0` | `2` | no |
...
The latest `fetch --recurse-submodules` slices closed initialized local/file
implicit yes, explicit yes and on-demand forms for a changed submodule gitlink,
plus the same implicit yes, explicit yes and on-demand forms for a smart HTTP
parent fetch whose initialized submodule remote is local, and implicit yes plus
on-demand forms for a smart HTTP parent fetch with the submodule still
uninitialized locally. The latest nested slice also closes implicit yes for a
smart HTTP parent with initialized nested local submodule remotes:
...
uninitialized, matching stock Git. Remaining submodule recursion modes,
non-local submodule remotes and path-limited recursion behavior remain open.
...
slice closed branch deepen fetches and multiple explicit refspec fetches over
smart HTTP, SSH and git daemon using shallow boundary lines plus the
`deepen-relative` capability; network branchless and related modes remain
open.
