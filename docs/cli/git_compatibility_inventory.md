...
| Written behavior rows | `841` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `732/841` | yes, row by row | supported-behavior rows with parity evidence |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`841`** | **`732`** | **`0`** | **`0`** | **`109`** | **`505`** |
...
represented option pairs or `732/841` passing written rows as a Git support
percentage. The `732/841` number is audit progress for supported rows already
written down; `109/841` additional rows are stock-compatible invalid inputs. It
