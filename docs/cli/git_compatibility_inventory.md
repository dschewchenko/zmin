...
| Git doc option pairs represented by rows | `656/4632` | no | documented command-option pairs with at least one behavior row |
| Written behavior rows | `2727` | no by itself | explicit command/option/value/combination/state/transport/platform rows currently written |
| Written rows matching stock Git | `2339/2727` | yes, row by row | supported-behavior rows with parity evidence |
| Partial written rows | `0/2727` | no | written rows with incomplete parity |
| Open written rows | `1/2727` | no | written rows that still do not match stock Git |
| Invalid input rows | `387/2727` | yes, as invalid-input compatibility | rows where stock Git rejects the input and Zmin matches that rejection |
...
| Administration | `8` | `0` | `147` | `0` | `151` | `90` | `0` | `0` | `61` | `58` |
...
| **Git 2.47 unique total** | **`151`** | **`0`** | **`4632`** | **`0`** | **`2727`** | **`2339`** | **`0`** | **`1`** | **`387`** | **`1018`** |
...
Never use `151/151` command presence, `4632` option spellings, `656/4632`
represented option pairs or `2339/2727` passing written rows as a Git support
percentage. The `2339/2727` number is audit progress for supported rows already
written down; `0/2727` rows are partial, `1/2727` rows are open and `387/2727`
