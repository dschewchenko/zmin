...
The current frozen focused-oracle backlog is `961` functions: `696`
represented or classified and `265` `missing_or_unclassified`. Treat
...
`git_maintenance_compat.rs`, `git_transport_http_compat.rs`,
`git_pack_integrity_compat.rs`, `git_transport_local_compat.rs` and
`git_worktree_state_compat.rs`. Before any TSV edit, write the selected
