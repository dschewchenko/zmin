...
- `2338` verified exact behavior rows
- `387` invalid-input parity rows
- `773` implemented-but-unverified schema rows
- `4681` remaining rows to fix, expand or verify
...
The current frozen focused-oracle backlog is `961` functions: `703`
represented or classified and `258` `missing_or_unclassified`. Treat
...
coherent missing-or-unclassified buckets, currently led by
`git_transport_http_compat.rs`, `git_transport_local_compat.rs`,
`git_worktree_state_compat.rs`, `git_maintenance_compat.rs` and
`git_pack_integrity_compat.rs`. Before any TSV edit, write the selected
functions and expected row/status delta in the row-growth audit; after the
edit, prove the generated inventory moved by that declared amount.
