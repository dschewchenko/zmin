...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/zmin-fast-export-gap.XXXXXX")"
cleanup() {
  rm -rf "$tmpdir"
}
trap cleanup EXIT
...
run_gap() {
  ...
  set +e
  "$GIT_BIN" -C "$git_work" fast-export "$@" >"$tmpdir/${name}.git.out" 2>"$tmpdir/${name}.git.err"
  git_exit=$?
  "$ZMIN_BIN" -C "$zmin_work" fast-export "$@" >"$tmpdir/${name}.zmin.out" 2>"$tmpdir/${name}.zmin.err"
  zmin_exit=$?
  set -e
