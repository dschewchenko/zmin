...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/zmin-show-merge-schema-oracle.XXXXXX")"
cleanup() {
  rm -rf "$tmpdir"
}
trap cleanup EXIT
...
make_merge_repo() {
  ...
  "$GIT_BIN" -C "$repo" merge --no-ff -q -m merge side
}
...
run_case show_first_parent_merge_name_only show --first-parent --format=%H --name-only HEAD
run_case show_separate_merges_patch_with_stat show -m --patch-with-stat HEAD
