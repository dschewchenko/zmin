...
tmpdir="$(mktemp -d /tmp/zmin-describe-schema-oracle.XXXXXX)"
cleanup() {
  rm -rf "$tmpdir"
}
trap cleanup EXIT
...
seed_describe_repo() {
  ...
  "$GIT_BIN" -C "$repo" tag -a v1.0.0 -m v1
  printf 'next\n' >>"$repo/file.txt"
  "$GIT_BIN" -C "$repo" commit -q -am next
}
...
run_case describe_positional_commitish describe v1.0.0
run_case describe_exact_match_tag describe --exact-match v1.0.0
