...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/zmin-checkout-reset-branch-oracle.XXXXXX")"
cleanup() {
  rm -rf "$tmpdir"
}
trap cleanup EXIT
...
record_repo_state() {
  ...
  "$GIT_BIN" -C "$repo" rev-parse HEAD >"$prefix.head-oid"
  "$GIT_BIN" -C "$repo" rev-parse main >"$prefix.main-oid"
  "$GIT_BIN" -C "$repo" status --short >"$prefix.status"
  "$GIT_BIN" -C "$repo" ls-files -s >"$prefix.index"
  cat "$repo/a.txt" >"$prefix.a-txt"
  cp "$repo/.git/logs/HEAD" "$prefix.head-log"
  cp "$repo/.git/logs/refs/heads/main" "$prefix.main-log"
}
...
run_case() {
  ...
  compare_files head-oid "$tmpdir/${name}.git.head-oid" "$tmpdir/${name}.zmin.head-oid"
  compare_files main-oid "$tmpdir/${name}.git.main-oid" "$tmpdir/${name}.zmin.main-oid"
  ...
  compare_files head-log "$tmpdir/${name}.git.head-log" "$tmpdir/${name}.zmin.head-log"
  compare_files main-log "$tmpdir/${name}.git.main-log" "$tmpdir/${name}.zmin.main-log"
  printf '%s\tok\texit=%s\n' "$name" "$git_exit"
}

run_case checkout_reset_current_branch checkout -B main HEAD~1
