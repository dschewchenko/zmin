...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/zmin-cherry-schema-oracle.XXXXXX")"
cleanup() {
  rm -rf "$tmpdir"
}
trap cleanup EXIT
...
seed_repo() {
  ...
  "$GIT_BIN" -C "$repo" branch upstream
  "$GIT_BIN" -C "$repo" checkout -q -b topic
  printf 'topic\n' >"$repo/a.txt"
  "$GIT_BIN" -C "$repo" commit -am topic -q
}
