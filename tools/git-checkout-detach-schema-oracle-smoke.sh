...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/zmin-checkout-detach-schema-oracle.XXXXXX")"
cleanup() {
  rm -rf "$tmpdir"
}
trap cleanup EXIT
...
head_state() {
  local repo="$1"
  if "$GIT_BIN" -C "$repo" symbolic-ref -q HEAD; then
    return 0
  fi
...
run_case() {
  ...
  head_state "$git_work" >"$tmpdir/${name}.git.head"
  head_state "$zmin_work" >"$tmpdir/${name}.zmin.head"
...
run_case checkout_detach_target checkout --detach HEAD~1
run_case checkout_detach_default checkout --detach
