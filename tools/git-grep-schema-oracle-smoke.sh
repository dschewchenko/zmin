...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/zmin-grep-oracle.XXXXXX")"
cleanup() {
  rm -rf "$tmpdir"
}
trap cleanup EXIT
...
run_case() {
  ...
  set +e
  "$GIT_BIN" -C "$git_work" grep "$@" >"$tmpdir/${name}.git.out" 2>"$tmpdir/${name}.git.err"
  git_exit=$?
  "$ZMIN_BIN" -C "$zmin_work" grep "$@" >"$tmpdir/${name}.zmin.out" 2>"$tmpdir/${name}.zmin.err"
  zmin_exit=$?
  set -e
...
run_case grep_line_number_long --line-number hello
run_case grep_files_with_matches_long --files-with-matches hello
run_case grep_fixed_strings_long --fixed-strings hello.world
