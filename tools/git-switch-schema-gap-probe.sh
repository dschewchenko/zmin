...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/zmin-switch-gap.XXXXXX")"
cleanup() {
  rm -rf "$tmpdir"
}
trap cleanup EXIT
...
run_gap() {
  ...
  set +e
  "$GIT_BIN" -C "$git_work" switch "$@" >"$tmpdir/${name}.git.out" 2>"$tmpdir/${name}.git.err"
  git_exit=$?
  "$ZMIN_BIN" -C "$zmin_work" switch "$@" >"$tmpdir/${name}.zmin.out" 2>"$tmpdir/${name}.zmin.err"
  zmin_exit=$?
  set -e
...
run_gap switch_create_long --create feature
