...
profile="${SKRON_STRESS_PROFILE:-small}"

case "$profile" in
  small)
    files="${SKRON_STRESS_FILES:-120}"
    big_kib="${SKRON_STRESS_BIG_KIB:-512}"
    ;;
  medium)
    files="${SKRON_STRESS_FILES:-800}"
    big_kib="${SKRON_STRESS_BIG_KIB:-4096}"
    ;;
  large)
    files="${SKRON_STRESS_FILES:-3000}"
    big_kib="${SKRON_STRESS_BIG_KIB:-16384}"
    ;;
  *)
    echo "unknown SKRON_STRESS_PROFILE: $profile" >&2
    exit 2
    ;;
esac
...
(cd "$skron_repo" 2>/dev/null && true) || mkdir -p "$skron_repo"
(cd "$skron_repo" && "$skron_bin" init >/dev/null)
for repo in "$git_repo" "$skron_repo"; do
...
compare_output() {
  ...
  local skron_out="$tmp_dir/skron-$label.out"
  local git_norm="$tmp_dir/git-$label.norm"
  local skron_norm="$tmp_dir/skron-$label.norm"
  git -C "$git_repo" -c core.abbrev=7 "$@" >"$git_out"
  (cd "$skron_repo" && "$skron_bin" -c core.abbrev=7 "$@") >"$skron_out"
  sed -E \
  ...
  sed -E \
    -e 's/index ([0-9a-f]{7})[0-9a-f]*\.{2}([0-9a-f]{7})[0-9a-f]*/index \1..\2/g' \
    -e 's/^([0-9a-f]{7})[0-9a-f]* /\1 /' \
    "$skron_out" >"$skron_norm"
  diff -u "$git_norm" "$skron_norm"
  echo "ok: $label"
}
...
compare_status() {
  local label="$1"
  shift
  local git_status skron_status
  set +e
  git -C "$git_repo" "$@" >/dev/null 2>&1
  git_status="$?"
  (cd "$skron_repo" && "$skron_bin" "$@" >/dev/null 2>&1)
  skron_status="$?"
  set -e
  if [[ "$git_status" != "$skron_status" ]]; then
    echo "status mismatch for $label: git=$git_status skron=$skron_status" >&2
    exit 1
  fi
  echo "ok: $label"
}
...
compare_git_state() {
  ...
  git -C "$skron_repo" cat-file -p HEAD^{tree} >"$tmp_dir/skron-tree.out"
  diff -u "$tmp_dir/git-tree.out" "$tmp_dir/skron-tree.out"
  git -C "$git_repo" ls-files --stage >"$tmp_dir/git-index.out"
  git -C "$skron_repo" ls-files --stage >"$tmp_dir/skron-index.out"
  diff -u "$tmp_dir/git-index.out" "$tmp_dir/skron-index.out"
  echo "ok: $label"
}
...
write_fixture "$skron_repo"

git -C "$git_repo" add -A
(cd "$skron_repo" && "$skron_bin" add -A)
git -C "$git_repo" ls-files --stage >"$tmp_dir/git-index-initial.out"
git -C "$skron_repo" ls-files --stage >"$tmp_dir/skron-index-initial.out"
diff -u "$tmp_dir/git-index-initial.out" "$tmp_dir/skron-index-initial.out"
env "${commit_env[@]}" git -C "$git_repo" -c commit.gpgsign=false commit -qm "initial stress"
(cd "$skron_repo" && env "${commit_env[@]}" "$skron_bin" commit -m "initial stress" >/dev/null)
compare_git_state "initial commit tree and index"

git_file_001="$(fixture_file "$git_repo" "001")"
skron_file_001="$(fixture_file "$skron_repo" "001")"
printf 'changed\n' >>"$git_repo/$git_file_001"
printf 'changed\n' >>"$skron_repo/$skron_file_001"

rm_file_002="$(fixture_file "$git_repo" "002")"
rm_file_002_skron="$(fixture_file "$skron_repo" "002")"
rm "$git_repo/$rm_file_002" "$skron_repo/$rm_file_002_skron"
mkdir -p "$git_repo/src/new" "$skron_repo/src/new"
printf 'new tracked\n' >"$git_repo/src/new/new.txt"
printf 'new tracked\n' >"$skron_repo/src/new/new.txt"
printf 'ignored by diff\n' >"$git_repo/untracked.tmp"
printf 'ignored by diff\n' >"$skron_repo/untracked.tmp"
...
(cd "$skron_repo" && env "${commit_env[@]}" "$skron_bin" commit -m "second stress" >/dev/null)
compare_git_state "second commit tree and index"

git -C "$git_repo" mv src/new/new.txt src/new/renamed.txt
(cd "$skron_repo" && "$skron_bin" mv src/new/new.txt src/new/renamed.txt)
rm_file_003="$(fixture_file "$git_repo" "003")"
rm_file_003_skron="$(fixture_file "$skron_repo" "003")"
git -C "$git_repo" rm -q "$rm_file_003"
(cd "$skron_repo" && "$skron_bin" rm "$rm_file_003_skron" >/dev/null)
git -C "$git_repo" status --porcelain=v1 --no-renames >"$tmp_dir/git-mv-rm-status.out"
(cd "$skron_repo" && "$skron_bin" status --porcelain=v1) >"$tmp_dir/skron-mv-rm-status.out"
diff -u "$tmp_dir/git-mv-rm-status.out" "$tmp_dir/skron-mv-rm-status.out"
echo "ok: mv-rm-status"
git -C "$git_repo" add -A
(cd "$skron_repo" && "$skron_bin" add -A)
env "${commit_env[@]}" git -C "$git_repo" -c commit.gpgsign=false commit -qm "rename remove stress"
(cd "$skron_repo" && env "${commit_env[@]}" "$skron_bin" commit -m "rename remove stress" >/dev/null)
...
skron_pull_client="$tmp_dir/skron-pull-client"
...
(cd "$tmp_dir" && "$skron_bin" clone "$git_pull_source" "$skron_pull_client" >/dev/null)
for repo in "$git_pull_client" "$skron_pull_client"; do
  git -C "$repo" config user.name Stress
  git -C "$repo" config user.email stress@example.test
  git -C "$repo" config commit.gpgsign false
  printf 'local\n' >"$repo/local.txt"
...
(cd "$skron_pull_client" && env "${commit_env[@]}" "$skron_bin" pull --rebase >/dev/null)
git -C "$git_pull_client" cat-file -p HEAD^{tree} >"$tmp_dir/git-pull-rebase-tree.out"
git -C "$skron_pull_client" cat-file -p HEAD^{tree} >"$tmp_dir/skron-pull-rebase-tree.out"
diff -u "$tmp_dir/git-pull-rebase-tree.out" "$tmp_dir/skron-pull-rebase-tree.out"
git -C "$git_pull_client" log --format=%s --max-count=3 >"$tmp_dir/git-pull-rebase-log.out"
git -C "$skron_pull_client" log --format=%s --max-count=3 >"$tmp_dir/skron-pull-rebase-log.out"
diff -u "$tmp_dir/git-pull-rebase-log.out" "$tmp_dir/skron-pull-rebase-log.out"
git -C "$git_pull_client" status --porcelain=v1 --branch >"$tmp_dir/git-pull-rebase-status.out"
(cd "$skron_pull_client" && "$skron_bin" status --porcelain=v1 --branch) >"$tmp_dir/skron-pull-rebase-status.out"
diff -u "$tmp_dir/git-pull-rebase-status.out" "$tmp_dir/skron-pull-rebase-status.out"
echo "ok: pull-rebase-local"

compare_merge_case_state() {
  local label="$1"
  git -C "$git_case" status --porcelain=v1 >"$tmp_dir/git-$label-status.out"
  (cd "$skron_case" && "$skron_bin" status --porcelain=v1) >"$tmp_dir/skron-$label-status.out"
  diff -u "$tmp_dir/git-$label-status.out" "$tmp_dir/skron-$label-status.out"
  git -C "$git_case" write-tree >"$tmp_dir/git-$label-tree.out"
  git -C "$skron_case" write-tree >"$tmp_dir/skron-$label-tree.out"
  diff -u "$tmp_dir/git-$label-tree.out" "$tmp_dir/skron-$label-tree.out"
...
compare_conflict_state() {
  ...
  git -C "$skron_case" ls-files -u >"$tmp_dir/skron-$label-unmerged.out"
  diff -u "$tmp_dir/git-$label-unmerged.out" "$tmp_dir/skron-$label-unmerged.out"
  git -C "$git_case" status --porcelain=v1 >"$tmp_dir/git-$label-status.out"
  (cd "$skron_case" && "$skron_bin" status --porcelain=v1) >"$tmp_dir/skron-$label-status.out"
  diff -u "$tmp_dir/git-$label-status.out" "$tmp_dir/skron-$label-status.out"
  if [[ -n "$path" ]]; then
    cmp "$git_case/$path" "$skron_case/$path"
  fi
...
init_merge_case_pair() {
  ...
  skron_case="$tmp_dir/skron-merge-$label"
  git init --quiet -b main "$git_case"
  git init --quiet -b main "$skron_case"
  for repo in "$git_case" "$skron_case"; do
    git -C "$repo" config user.name Stress
    git -C "$repo" config user.email stress@example.test
    git -C "$repo" config commit.gpgsign false
  done
...
run_conflicting_merge_pair() {
  ...
  git_status="$?"
  (cd "$skron_case" && "$skron_bin" merge feature >/dev/null 2>&1)
  skron_status="$?"
  set -e
  if [[ "$git_status" != "$skron_status" ]]; then
    echo "merge status mismatch for $label: git=$git_status skron=$skron_status" >&2
    exit 1
  fi
}
...
(cd "$skron_case" && "$skron_bin" merge --no-ff feature >/dev/null)
git -C "$git_case" rev-list --parents -1 HEAD | cut -d' ' -f2- >"$tmp_dir/git-no-ff-parents.out"
git -C "$skron_case" rev-list --parents -1 HEAD | cut -d' ' -f2- >"$tmp_dir/skron-no-ff-parents.out"
diff -u "$tmp_dir/git-no-ff-parents.out" "$tmp_dir/skron-no-ff-parents.out"
...
(cd "$skron_case" && "$skron_bin" merge --no-commit feature >/dev/null 2>/dev/null)
diff -u "$git_case/.git/MERGE_HEAD" "$skron_case/.git/MERGE_HEAD"
...
(cd "$skron_case" && "$skron_bin" merge --squash feature >/dev/null 2>/dev/null)
if [[ -e "$skron_case/.git/MERGE_HEAD" ]]; then
  echo "squash unexpectedly wrote MERGE_HEAD" >&2
  exit 1
fi
test -e "$skron_case/.git/SQUASH_MSG"
