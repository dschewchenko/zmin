...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/zmin-clone-submodule-flags-oracle.XXXXXX")"
cleanup() {
  rm -rf "$tmpdir"
}
trap cleanup EXIT
...
run_case clone_no_recurse_submodules --no-recurse-submodules
run_case clone_recurse_submodules --recurse-submodules
run_case clone_recursive --recursive
run_case clone_remote_submodules --remote-submodules
run_case clone_shallow_submodules --shallow-submodules
