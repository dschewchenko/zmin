...
validation_root="${WINDOWS_WINE_VALIDATION_ROOT:-$HOME/Downloads/skron-windows-validation}"
...
remote_http_win="${repo_root_win//\//\\}\\target\\$target\\release\\skron-git-remote-http.exe"
...
cargo build -p skron-cli --target "$target" --release --bins
cargo build -p skron-git-remote-http --target "$target" --release
cargo test -p skron-cli --target "$target" --release --test compatibility_command --no-run

skron_exe="$repo_root/target/$target/release/skron.exe"
git_wrapper_exe="$repo_root/target/$target/release/skron-git.exe"
...
env Path="$path_win" "$wine_bin" "$skron_exe" --version
env Path="$path_win" "$wine_bin" "$git_wrapper_exe" --version
env Path="$path_win" SKRON_GIT_REMOTE_HTTP="$remote_http_win" "$wine_bin" "$compatibility_exe" --nocapture

smoke_dir="$(mktemp -d "$validation_root/windows-wine-smoke.XXXXXX")"
trap 'rm -rf "$smoke_dir"' EXIT

run_skron() {
  env Path="$path_win" "$wine_bin" "$skron_exe" "$@"
}
...
cd "$smoke_dir"
run_skron init -b main
run_skron config user.name Bench
run_skron config user.email bench@example.test
run_skron config commit.gpgsign false
printf 'hello from windows wine\n' > a.txt
run_skron add -A
run_skron_commit commit -m 'windows wine smoke'
run_skron status --porcelain=v1 --branch > status.out
run_skron log --oneline > log.out
