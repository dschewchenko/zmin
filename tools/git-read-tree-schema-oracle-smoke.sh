...
tmpdir="$(mktemp -d /tmp/zmin-read-tree-schema-oracle.XXXXXX)"
cleanup() {
  rm -rf "$tmpdir"
}
trap cleanup EXIT
...
run_case() {
  ...
  local git_tree_after="$tmpdir/${name}.git.tree-after"
  local zmin_tree_after="$tmpdir/${name}.zmin.tree-after"
  ...
  cp -R "$seed" "$git_work"
  cp -R "$seed" "$zmin_work"
  tree="$("$GIT_BIN" -C "$seed" rev-parse HEAD~1^{tree})"

  set +e
  "$GIT_BIN" -C "$git_work" read-tree "$tree" >"$git_out" 2>"$git_err"
  git_exit=$?
  "$ZMIN_BIN" -C "$zmin_work" read-tree "$tree" >"$zmin_out" 2>"$zmin_err"
  zmin_exit=$?
  set -e
  ...
  "$GIT_BIN" -C "$git_work" write-tree >"$git_tree_after"
  "$GIT_BIN" -C "$zmin_work" write-tree >"$zmin_tree_after"
  compare_files write_tree "$git_tree_after" "$zmin_tree_after"
...
run_case read_tree_positional_treeish
