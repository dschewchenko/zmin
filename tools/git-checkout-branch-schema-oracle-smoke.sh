...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/zmin-checkout-branch-schema-oracle.XXXXXX")"
cleanup() {
  rm -rf "$tmpdir"
}
trap cleanup EXIT
...
compare_repo_state() {
  ...
  "$GIT_BIN" -C "$git_work" rev-parse topic >"$tmpdir/${name}.git.topic"
  "$GIT_BIN" -C "$zmin_work" rev-parse topic >"$tmpdir/${name}.zmin.topic"
  compare_files topic-ref "$tmpdir/${name}.git.topic" "$tmpdir/${name}.zmin.topic"
...
run_case checkout_create_branch_short checkout -b topic HEAD~1
run_case checkout_create_reflog_branch checkout -l -b topic HEAD~1
