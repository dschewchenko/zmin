...
tmpdir="$(mktemp -d /tmp/zmin-name-rev-show-branch-oracle.XXXXXX)"
cleanup() {
  rm -rf "$tmpdir"
}
trap cleanup EXIT
...
dangling_commit() {
  local repo="$1"
  local tree
  tree="$("$GIT_BIN" -C "$repo" mktree </dev/null)"
...
run_dangling_case() {
  ...
  seed_repo "$git_work"
  commit="$(dangling_commit "$git_work")"
  cp -R "$git_work" "$zmin_work"
...
run_case show_branch_all_short show-branch -a
run_case show_branch_remotes_short show-branch -r
