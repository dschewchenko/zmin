...
fn merge_base_matches_stock_git_for_linear_history() {
    ...
    run_zmin(repo.path(), ["tag", "-a", "v2", "-m", "full", "HEAD~1"]);
    let tag_full = git(repo.path(), ["rev-parse", "v2"]);
    let tree_full = git(repo.path(), ["rev-parse", "HEAD^{tree}"]);

    assert_eq!(
        run_zmin(repo.path(), ["merge-base", "HEAD", "HEAD~1"]),
        git(repo.path(), ["merge-base", "HEAD", "HEAD~1"])
    );
    assert_eq!(
        run_zmin(repo.path(), ["merge-base", "HEAD", "HEAD"]),
        git(repo.path(), ["merge-base", "HEAD", "HEAD"])
    );
    assert_eq!(
        run_zmin_status(repo.path(), ["merge-base", "--is-ancestor", "HEAD", "HEAD"]),
...
fn merge_base_uses_commit_graph_without_changing_results() {
    ...
    assert_eq!(
        run_zmin(repo.path(), ["merge-base", "HEAD", "HEAD~4"]),
...
fn merge_base_multi_commit_modes_match_stock_git() {
    ...
    let plain = run_zmin(repo.path(), ["merge-base", "a", "b", "c"]);
    let octopus = run_zmin(repo.path(), ["merge-base", "--octopus", "a", "b", "c"]);
...
fn merge_ff_only_matches_stock_git_state_and_exit_codes() {
    ...
    fs::write(zmin_repo.path().join("feature.txt"), b"feature\n").expect("write zmin feature");
    ...
    run_zmin(zmin_repo.path(), ["merge", "--ff-only", "feature"]);
    assert_eq!(
    ...
    assert_eq!(
        fs::read(zmin_repo.path().join("feature.txt")).expect("read merged feature"),
        b"feature\n"
    );
    ...
    fs::write(zmin_repo.path().join("main.txt"), b"main\n").expect("write zmin main");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(git_repo.path(), ["commit", "-m", "main"]);
    run_zmin_with_env(zmin_repo.path(), ["commit", "-m", "main"]);

    assert_eq!(
        run_zmin_status(zmin_repo.path(), ["merge", "--ff-only", "feature"]),
...
fn merge_no_ff_creates_merge_commit_for_fast_forwardable_branch() {
    ...
    let zmin_out = run_zmin_args(zmin_repo.path(), &["merge", "--no-ff", "feature"]);
    assert_eq!(zmin_out.lines().next(), git_out.lines().next());
    assert_eq!(
    ...
    assert_eq!(
        git(zmin_repo.path(), ["rev-parse", "HEAD^{tree}"]),
        git(zmin_repo.path(), ["rev-parse", "feature^{tree}"])
    );
    assert_eq!(
...
fn merge_no_commit_leaves_merge_state_without_advancing_head() {
    ...
    let zmin_head_before = git(zmin_repo.path(), ["rev-parse", "HEAD"]);
    git(git_repo.path(), ["merge", "--no-commit", "feature"]);
    run_zmin(zmin_repo.path(), ["merge", "--no-commit", "feature"]);
    ...
    assert_eq!(
        fs::read_to_string(zmin_repo.path().join(".git/MERGE_HEAD")).expect("zmin MERGE_HEAD"),
...
fn merge_squash_stages_result_without_merge_head() {
    ...
    let zmin_out = run_zmin_args(zmin_repo.path(), &["merge", "--squash", "feature"]);
    ...
    assert!(!zmin_repo.path().join(".git/MERGE_HEAD").exists());
    assert!(zmin_repo.path().join(".git/SQUASH_MSG").exists());
    assert_eq!(
        git(zmin_repo.path(), ["status", "--porcelain=v1"]),
        git(git_repo.path(), ["status", "--porcelain=v1"])
    );
...
#[test]
fn merge_stat_toggle_family_matches_stock_git_output_and_state() {
    for args in [
        ["merge", "--stat", "feature"].as_slice(),
        ["merge", "--summary", "feature"].as_slice(),
        ["merge", "--no-stat", "feature"].as_slice(),
        ["merge", "--no-summary", "feature"].as_slice(),
        ["merge", "-n", "feature"].as_slice(),
    ] {
        let git_repo = committed_repo();
        let zmin_repo = committed_repo();
        let default_branch = git(git_repo.path(), ["rev-parse", "--abbrev-ref", "HEAD"]);
...
fn merge_non_ff_clean_changes_creates_stock_compatible_merge_commit() {
    ...
    run_zmin_with_env(zmin_repo.path(), ["merge", "feature"]);
    assert_eq!(
        git(zmin_repo.path(), ["rev-parse", "HEAD^{tree}"]),
        git(git_repo.path(), ["rev-parse", "HEAD^{tree}"])
    );
    ...
    assert_eq!(
        fs::read(zmin_repo.path().join("feature.txt")).expect("read feature"),
        b"feature\n"
    );
    assert_eq!(
        fs::read(zmin_repo.path().join("main.txt")).expect("read main"),
        b"main\n"
    );
}
...
fn merge_strategy_output_matches_stock_git_for_default_ort_and_recursive() {
    for strategy in [None, Some("ort"), Some("recursive")] {
        ...
        let zmin_out = match strategy {
            Some(name) => run_zmin_args(zmin_repo.path(), &["merge", "-s", name, "feature"]),
            None => run_zmin_args(zmin_repo.path(), &["merge", "feature"]),
        };
        assert_eq!(
            zmin_out.lines().next(),
            git_out.lines().next(),
...
fn merge_conflict_leaves_unmerged_index_and_merge_state_like_stock_git() {
    ...
    let zmin_failure = run_zmin_failure_output(zmin_repo.path(), &["merge", "feature"]);
    ...
    assert_eq!(
        fs::read_to_string(zmin_repo.path().join("a.txt")).expect("zmin conflicted file"),
        fs::read_to_string(git_repo.path().join("a.txt")).expect("git conflicted file")
    );

    write_file(git_repo.path(), "a.txt", "resolved\n");
    write_file(zmin_repo.path(), "a.txt", "resolved\n");
    ...
    run_zmin_with_env(zmin_repo.path(), ["merge", "--continue"]);
    assert_eq!(
        git(zmin_repo.path(), ["rev-parse", "HEAD^{tree}"]),
        git(git_repo.path(), ["rev-parse", "HEAD^{tree}"])
    );
...
fn merge_abort_during_conflict_restores_head_like_stock_git() {
    ...
    run_zmin(zmin_repo.path(), ["merge", "--abort"]);
    ...
    assert_eq!(
        zmin_repo.path().join("feature.txt").exists(),
...
fn merge_ours_strategy_matches_stock_git_tree_and_parents() {
    ...
    run_zmin_with_env(zmin_repo.path(), ["merge", "-s", "ours", "feature"]);
    assert_eq!(
        git(zmin_repo.path(), ["rev-parse", "HEAD^{tree}"]),
        git(git_repo.path(), ["rev-parse", "HEAD^{tree}"])
    );
    assert_eq!(
        git(zmin_repo.path(), ["rev-parse", "HEAD^{tree}"]),
        git(zmin_repo.path(), ["rev-parse", "HEAD^1^{tree}"])
    );
    assert_eq!(
        git(zmin_repo.path(), ["rev-list", "--parents", "-1", "HEAD"])
            .split_whitespace()
...
fn merge_abort_and_continue_without_merge_match_stock_git_failures() {
    ...
    assert_eq!(
        run_zmin_failure_output(zmin_repo.path(), &["merge", "--abort"]),
        git_failure_output(git_repo.path(), &["merge", "--abort"])
    );
    assert_eq!(
        run_zmin_failure_output(zmin_repo.path(), &["merge", "--continue"]),
