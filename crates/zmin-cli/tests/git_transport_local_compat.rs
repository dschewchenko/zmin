...
fn sorted_shallow_file(path: &Path) -> Vec<String> {
    let mut lines = fs::read_to_string(path)
        .expect("read shallow file")
        .lines()
        .map(str::to_owned)
        .collect::<Vec<_>>();
    lines.sort();
...
#[test]
fn pull_merge_commit_mode_flags_match_stock_git_for_explicit_local_branch() {
    for (label, args, expect_head_stable, expect_merge_head, expect_squash_msg) in [
        (
            "pull --commit",
            ["pull", "--commit", "--no-rebase", ".", "side"],
            false,
            false,
            false,
        ),
        ...
        assert_eq!(
            git(&zmin_repo, ["write-tree"]),
            git(&git_repo, ["write-tree"]),
            "{label} write-tree"
        );
        assert_eq!(
        ...
        let zmin_merge_head = zmin_repo.join(".git/MERGE_HEAD");
        let git_merge_head = git_repo.join(".git/MERGE_HEAD");
        assert_eq!(
            zmin_merge_head.exists(),
            expect_merge_head,
            "{label} zmin MERGE_HEAD presence"
        );
        assert_eq!(
            git_merge_head.exists(),
            expect_merge_head,
            "{label} git MERGE_HEAD presence"
        );
        if expect_merge_head {
            assert_eq!(
                fs::read_to_string(&zmin_merge_head).expect("zmin MERGE_HEAD"),
                fs::read_to_string(&git_merge_head).expect("git MERGE_HEAD"),
                "{label} MERGE_HEAD"
            );
        }

        let zmin_squash_msg = zmin_repo.join(".git/SQUASH_MSG");
        let git_squash_msg = git_repo.join(".git/SQUASH_MSG");
        assert_eq!(
            zmin_squash_msg.exists(),
            expect_squash_msg,
            "{label} zmin SQUASH_MSG presence"
        );
        assert_eq!(
            git_squash_msg.exists(),
            expect_squash_msg,
            "{label} git SQUASH_MSG presence"
        );
        if expect_squash_msg {
            assert_eq!(
                fs::read_to_string(&zmin_squash_msg).expect("zmin SQUASH_MSG"),
                fs::read_to_string(&git_squash_msg).expect("git SQUASH_MSG"),
                "{label} SQUASH_MSG"
            );
        }
    }
...
#[test]
fn pull_merge_stat_toggle_family_matches_stock_git_for_explicit_local_branch() {
    for (label, args) in [
        (
            "pull --stat",
            ["pull", "--stat", "--no-rebase", ".", "side"],
        ),
        (
            "pull --summary",
            ["pull", "--summary", "--no-rebase", ".", "side"],
        ),
        (
            "pull --no-stat",
            ["pull", "--no-stat", "--no-rebase", ".", "side"],
        ),
        (
            "pull --no-summary",
            ["pull", "--no-summary", "--no-rebase", ".", "side"],
        ),
        ("pull -n", ["pull", "-n", "--no-rebase", ".", "side"]),
    ] {
        let dir = TempDir::new().expect("temp dir");
        let git_repo = dir.path().join("git-repo");
        let zmin_repo = dir.path().join("zmin-repo");
...
fn pull_merge_allow_unrelated_histories_matches_stock_git_for_explicit_local_branch() {
    ...
    fs::write(remote.join("remote.txt"), b"remote\n").expect("write remote");
    git(&remote, ["add", "-A"]);
    git_with_env(&remote, ["commit", "-m", "remote"]);

    for repo in [&git_repo, &zmin_repo] {
        fs::write(repo.join("local.txt"), b"local\n").expect("write local");
        git(repo, ["add", "-A"]);
...
#[test]
fn pull_merge_strategy_option_ours_and_theirs_match_stock_git_for_explicit_local_branch() {
    for strategy in ["ours", "theirs"] {
        ...
        for repo in [&git_repo, &zmin_repo] {
            ...
            git_with_env(repo, ["commit", "-m", "base"]);
            git(repo, ["switch", "-c", "side"]);
            fs::write(repo.join("f.txt"), b"side\n").expect("write side");
            git(repo, ["add", "-A"]);
            git_with_env(repo, ["commit", "-m", "side"]);
            git(repo, ["switch", "main"]);
            fs::write(repo.join("f.txt"), b"main\n").expect("write main");
            git(repo, ["add", "-A"]);
            git_with_env(repo, ["commit", "-m", "main"]);
        }

        let args = ["pull", "-X", strategy, "--no-rebase", ".", "side"];
        let git_output = command_any_output("git", &git_repo, &args, "pull -X git");
        let zmin_output = command_any_output(zmin_bin(), &zmin_repo, &args, "pull -X zmin");
        assert_eq!(zmin_output, git_output, "pull -X {strategy} output");
        assert_eq!(
            git(&zmin_repo, ["rev-parse", "HEAD^{tree}"]),
            git(&git_repo, ["rev-parse", "HEAD^{tree}"]),
            "pull -X {strategy} tree"
        );
        assert_eq!(
            git(&zmin_repo, ["rev-list", "--parents", "-1", "HEAD"])
                .split_whitespace()
            ...
            git(&git_repo, ["rev-list", "--parents", "-1", "HEAD"])
                .split_whitespace()
                .count(),
            "pull -X {strategy} parents"
        );
        assert_eq!(
            git(&zmin_repo, ["log", "-1", "--pretty=%B"]),
            git(&git_repo, ["log", "-1", "--pretty=%B"]),
            "pull -X {strategy} message"
        );
        assert_eq!(
            fs::read_to_string(zmin_repo.join("f.txt")).expect("read zmin file"),
            fs::read_to_string(git_repo.join("f.txt")).expect("read git file"),
            "pull -X {strategy} worktree"
        );
        assert_eq!(
            fs::read_to_string(zmin_repo.join(".git/FETCH_HEAD")).expect("zmin FETCH_HEAD"),
            fs::read_to_string(git_repo.join(".git/FETCH_HEAD")).expect("git FETCH_HEAD"),
            "pull -X {strategy} FETCH_HEAD"
        );
    }
}
...
#[test]
fn fetch_tags_named_local_remote_branch_follows_tags_like_stock_git() {
    let dir = TempDir::new().expect("temp dir");
    let source = dir.path().join("source");
    let git_client = dir.path().join("git-client");
    let zmin_client = dir.path().join("zmin-client");
    ...
    let args = ["fetch", "--tags", "origin", "main"];
    let git_output = command_any_output("git", &git_client, &args, "git fetch tags origin main");
    let zmin_output = command_any_output(
        zmin_bin(),
        &zmin_client,
        &args,
        "zmin fetch tags origin main",
    );
...
#[test]
fn fetch_source_only_refspec_explicit_location_branch_matches_stock_git() {
    let dir = TempDir::new().expect("temp dir");
    let source = dir.path().join("source");
    let git_client = dir.path().join("git-client");
    let zmin_client = dir.path().join("zmin-client");
    ...
    let args = ["fetch", source.to_str().expect("source path"), "main:"];
    let git_output = command_any_output("git", &git_client, &args, "git fetch path main colon");
    let zmin_output = command_any_output(
        zmin_bin(),
        &zmin_client,
        &args,
        "zmin fetch path main colon",
    );
...
#[test]
fn fetch_empty_colon_refspec_explicit_location_head_matches_stock_git() {
    let dir = TempDir::new().expect("temp dir");
    let source = dir.path().join("source");
    let git_client = dir.path().join("git-client");
    let zmin_client = dir.path().join("zmin-client");
    ...
    let args = ["fetch", source.to_str().expect("source path"), ":"];
    let git_output = command_any_output("git", &git_client, &args, "git fetch path colon");
    let zmin_output = command_any_output(zmin_bin(), &zmin_client, &args, "zmin fetch path colon");
...
fn fetch_prune_direct_location_one_to_one_refspec_matches_stock_git() {
    ...
    let refspec = "main:refs/remotes/origin/main";
    git(
        &git_client,
        [
            "fetch",
...
#[test]
fn fetch_prune_direct_location_head_fetch_head_matches_stock_git() {
    let dir = TempDir::new().expect("temp dir");
    let source = dir.path().join("source");
    let git_client = dir.path().join("git-client");
    let zmin_client = dir.path().join("zmin-client");
    ...
    git(
        &git_client,
        ["fetch", "--prune", source.to_str().expect("source path")],
    );
    run_zmin(
        &zmin_client,
        ["fetch", "--prune", source.to_str().expect("source path")],
...
#[test]
fn fetch_no_tags_direct_location_head_skips_tags_like_stock_git() {
    let dir = TempDir::new().expect("temp dir");
    let source = dir.path().join("source");
    let git_client = dir.path().join("git-client");
    let zmin_client = dir.path().join("zmin-client");
    ...
    let args = ["fetch", "--no-tags", source.to_str().expect("source path")];
    assert_eq!(
        command_any_output(zmin_bin(), &zmin_client, &args, "zmin fetch --no-tags path"),
        command_any_output("git", &git_client, &args, "git fetch --no-tags path")
    );
    assert_eq!(
        fs::read_to_string(zmin_client.join(".git/FETCH_HEAD")).expect("zmin FETCH_HEAD"),
        fs::read_to_string(git_client.join(".git/FETCH_HEAD")).expect("git FETCH_HEAD")
    ...
    assert_eq!(
        git_status(
            &zmin_client,
            ["rev-parse", "--verify", "refs/tags/reachable-tag"]
        ),
        git_status(
            &git_client,
            ["rev-parse", "--verify", "refs/tags/reachable-tag"]
...
#[test]
fn fetch_no_tags_direct_location_branch_skips_tags_like_stock_git() {
    let dir = TempDir::new().expect("temp dir");
    let source = dir.path().join("source");
    let git_client = dir.path().join("git-client");
    let zmin_client = dir.path().join("zmin-client");
    ...
    assert_eq!(
        command_any_output(
            ...
            &zmin_client,
            &args,
            "zmin fetch --no-tags path branch"
        ),
        command_any_output("git", &git_client, &args, "git fetch --no-tags path branch")
    );
    assert_eq!(
        fs::read_to_string(zmin_client.join(".git/FETCH_HEAD")).expect("zmin FETCH_HEAD"),
        fs::read_to_string(git_client.join(".git/FETCH_HEAD")).expect("git FETCH_HEAD")
...
#[test]
fn fetch_update_shallow_multiple_refspecs_local_file_forms_match_stock_git() {
    for (case_label, use_remote, remote_url_from_source) in [
        ...
        let dir = TempDir::new().expect("temp dir");
        let source = dir.path().join("source");
        let shallow_remote = dir.path().join("shallow.git");
        let git_client = dir.path().join(format!(
            "git-client-update-shallow-{}",
            case_label.replace(' ', "-")
        ));
        let zmin_client = dir.path().join(format!(
            "zmin-client-update-shallow-{}",
            case_label.replace(' ', "-")
        ));
        ...
        assert_eq!(
            git(&shallow_remote, ["rev-parse", "--is-shallow-repository"]),
            "true"
        );

        let remote_url = remote_url_from_source(&shallow_remote);
        for client in [&git_client, &zmin_client] {
            git(
                dir.path(),
                ["init", "-b", "main", client.to_str().expect("client path")],
        ...
        let mut args = vec!["fetch", "--quiet", "--update-shallow"];
        if use_remote {
            args.push("origin");
        } else {
        ...
        assert_eq!(
            sorted_shallow_file(&zmin_client.join(".git/shallow")),
            sorted_shallow_file(&git_client.join(".git/shallow")),
            "{case_label}"
        );
        assert_eq!(
            fs::read_to_string(zmin_client.join(".git/FETCH_HEAD")).expect("zmin FETCH_HEAD"),
...
#[test]
fn fetch_update_shallow_explicit_location_head_matches_stock_git() {
    let dir = TempDir::new().expect("temp dir");
    let source = dir.path().join("source");
    let shallow_remote = dir.path().join("shallow");
    ...
    for (label, remote_url) in [
        ...
        let git_client = dir.path().join(format!("git-explicit-head-{label}"));
        let zmin_client = dir.path().join(format!("zmin-explicit-head-{label}"));
        for client in [&git_client, &zmin_client] {
            git(
                dir.path(),
                ["init", "-b", "main", client.to_str().expect("client path")],
        ...
        let args = ["fetch", "--quiet", "--update-shallow", remote_url];
        let git_output = command_any_output("git", &git_client, &args, "git");
        let zmin_output = command_any_output(zmin_bin(), &zmin_client, &args, "zmin");
...
#[test]
fn fetch_deepen_explicit_location_matches_stock_git() {
    for (transport_label, remote_url_from_source) in [
        (
            "local",
            Box::new(|source: &Path| source.display().to_string()) as Box<dyn Fn(&Path) -> String>,
        ),
        ...
        for (form_label, deepen_args) in [
            ...
            let git_client = dir
                .path()
                .join(format!("git-client-{transport_label}-{form_label}"));
            let zmin_client = dir
                .path()
                .join(format!("zmin-client-{transport_label}-{form_label}"));
            ...
            for (tool, client) in [("git", &git_client), (zmin_bin(), &zmin_client)] {
                command_any_output(
                    tool,
                    client,
                    &["fetch", "--quiet", "--depth=1", remote_url.as_str(), "main"],
                    tool,
                );
            }
            ...
            let mut args = vec!["fetch", "--quiet"];
            args.extend(deepen_args);
            args.extend([remote_url.as_str(), "main"]);
            let case_label = format!("{transport_label} {form_label}");
            let git_output = command_any_output("git", &git_client, &args, "git");
            let zmin_output = command_any_output(zmin_bin(), &zmin_client, &args, "zmin");
...
#[test]
fn fetch_deepen_explicit_location_head_matches_stock_git() {
    for (transport_label, remote_url_from_source) in [
        (
            "local",
            Box::new(|source: &Path| source.display().to_string()) as Box<dyn Fn(&Path) -> String>,
        ...
        for (form_label, deepen_args) in [
            ...
            let git_client = dir
                .path()
                .join(format!("git-client-head-{transport_label}-{form_label}"));
            let zmin_client = dir
                .path()
                .join(format!("zmin-client-head-{transport_label}-{form_label}"));
            ...
            for (tool, client) in [("git", &git_client), (zmin_bin(), &zmin_client)] {
                command_any_output(
                    tool,
                    client,
                    &["fetch", "--quiet", "--depth=1", remote_url.as_str()],
                    tool,
                );
            }
...
#[test]
fn fetch_unshallow_explicit_location_matches_stock_git() {
    for (transport_label, remote_url_from_source) in [
        (
            "local",
            Box::new(|source: &Path| source.display().to_string()) as Box<dyn Fn(&Path) -> String>,
        ...
        let dir = TempDir::new().expect("temp dir");
        let source = dir.path().join("source");
        let git_client = dir.path().join(format!("git-client-{transport_label}"));
        let zmin_client = dir.path().join(format!("zmin-client-{transport_label}"));
        ...
        assert_eq!(zmin_output.0, git_output.0, "{transport_label}");
        assert_eq!(zmin_output.1, git_output.1, "{transport_label}");
        assert_eq!(zmin_output.2, git_output.2, "{transport_label}");
        assert_eq!(
            git(&zmin_client, ["rev-parse", "--is-shallow-repository"]),
            git(&git_client, ["rev-parse", "--is-shallow-repository"]),
            "{transport_label}"
...
#[test]
fn fetch_unshallow_explicit_location_head_matches_stock_git() {
    for (transport_label, remote_url_from_source) in [
        (
            "local",
            Box::new(|source: &Path| source.display().to_string()) as Box<dyn Fn(&Path) -> String>,
        ...
        let git_client = dir
            .path()
            .join(format!("git-client-head-{transport_label}"));
        let zmin_client = dir
            .path()
            .join(format!("zmin-client-head-{transport_label}"));
        ...
        let args = vec!["fetch", "--quiet", "--unshallow", remote_url.as_str()];
        let git_output = command_any_output("git", &git_client, &args, "git");
        let zmin_output = command_any_output(zmin_bin(), &zmin_client, &args, "zmin");
...
#[test]
fn fetch_unshallow_multiple_refspecs_local_file_forms_match_stock_git() {
    for (case_label, use_remote, remote_url_from_source) in [
        (
            "named local remote",
            true,
        ...
        let source = dir.path().join("source");
        let git_client = dir.path().join(format!(
            "git-client-unshallow-{}",
            case_label.replace(' ', "-")
        ));
        let zmin_client = dir.path().join(format!(
            "zmin-client-unshallow-{}",
            case_label.replace(' ', "-")
        ));
        ...
        let mut unshallow_args = vec!["fetch", "--quiet", "--unshallow"];
        if use_remote {
            unshallow_args.push("origin");
        } else {
            unshallow_args.push(remote_url.as_str());
        }
        unshallow_args.extend([
            "refs/heads/main:refs/remotes/origin/main",
            "refs/heads/feature:refs/remotes/origin/feature",
        ]);

        let git_output = command_any_output("git", &git_client, &unshallow_args, "git");
        let zmin_output = command_any_output(zmin_bin(), &zmin_client, &unshallow_args, "zmin");
...
#[test]
fn fetch_shallow_since_explicit_location_matches_stock_git() {
    for (label, shallow_since_args) in [
        ("equals", vec!["--shallow-since=2020-01-03T00:00:00 +0000"]),
        (
            "separate",
        ...
        for (remote_label, remote_url) in [
            ...
            let git_client = dir.path().join(format!("git-client-{case_label}"));
            let zmin_client = dir.path().join(format!("zmin-client-{case_label}"));
            for client in [&git_client, &zmin_client] {
                git(
                    dir.path(),
                    ["init", "-b", "main", client.to_str().expect("client path")],
...
#[test]
fn fetch_shallow_exclude_explicit_location_matches_stock_git() {
    for (transport_label, remote_url_from_source) in [
        (
            "local",
            Box::new(|source: &Path| source.display().to_string()) as Box<dyn Fn(&Path) -> String>,
        ...
        for (form_label, shallow_exclude_args) in [
            ("equals", vec!["--shallow-exclude=refs/heads/base"]),
            ("separate", vec!["--shallow-exclude", "refs/heads/base"]),
        ] {
            let dir = TempDir::new().expect("temp dir");
...
#[test]
fn fetch_shallow_exclude_explicit_location_head_matches_stock_git() {
    for (transport_label, remote_url_from_source) in [
        (
            "local",
            Box::new(|source: &Path| source.display().to_string()) as Box<dyn Fn(&Path) -> String>,
...
#[test]
fn fetch_shallow_exclude_multiple_refspecs_local_file_forms_match_stock_git() {
    for (case_label, use_remote, remote_url_from_source) in [
        (
            "named local remote",
            true,
        ...
        git_with_env(&source, ["commit", "-m", "feature"]);
        git(&source, ["checkout", "main"]);
        git(&source, ["branch", "base", &base_tip]);
        ...
        let mut args = vec!["fetch", "--quiet", "--shallow-exclude=refs/heads/base"];
        if use_remote {
            args.push("origin");
        } else {
            args.push(remote_url.as_str());
...
#[test]
fn fetch_shallow_since_multiple_refspecs_local_file_forms_match_stock_git() {
    for (case_label, use_remote, remote_url_from_source) in [
        (
            "named local remote",
            true,
        ...
        let git_client = dir
            .path()
            .join(format!("git-client-since-{}", case_label.replace(' ', "-")));
        let zmin_client = dir.path().join(format!(
            "zmin-client-since-{}",
            case_label.replace(' ', "-")
        ));
        ...
        command_output_with_env(
            ...
            &["commit", "-m", "feature"],
            &[
                ("GIT_AUTHOR_DATE", "2020-01-02T01:00:00 +0000"),
                ("GIT_COMMITTER_DATE", "2020-01-02T01:00:00 +0000"),
            ],
            "git",
        );
        git(&source, ["checkout", "main"]);
        ...
        let mut args = vec![
            "fetch",
            "--quiet",
            "--shallow-since=2020-01-02T00:00:00 +0000",
        ];
        if use_remote {
            args.push("origin");
        } else {
