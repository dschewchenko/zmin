...
fn invalid_option_combinations_match_stock_git_failures() {
    ...
    assert_eq!(
        command_output(zmin_bin(), repo.path(), &["add"], "zmin"),
        command_output("git", repo.path(), &["add"], "git")
    );
    assert_eq!(
        command_output(zmin_bin(), repo.path(), &["tag", "-d"], "zmin"),
        command_output("git", repo.path(), &["tag", "-d"], "git")
    );
    assert_eq!(
        command_output(zmin_bin(), repo.path(), &["rev-parse"], "zmin"),
    ...
    assert_eq!(
        command_output(zmin_bin(), repo.path(), &["log", "--format=%Q"], "zmin"),
    ...
    assert_eq!(
        command_output(zmin_bin(), repo.path(), &["clean", "-x", "-n"], "zmin"),
    ...
    for args in [
        ...
        let zmin_failure = run_zmin_failure_output(repo.path(), args);
        let git_failure = git_failure_output(repo.path(), args);
        assert_failure_matches(args, zmin_failure, git_failure);
    }
}

fn assert_failure_matches(
    args: &[&str],
    zmin_failure: (i32, String, String),
    git_failure: (i32, String, String),
) {
    #[cfg(windows)]
    if args == ["remote", "set-head", "missing", "-a"] {
        assert_eq!(
            zmin_failure.1, git_failure.1,
            "failure stdout mismatch for {args:?}"
        );
        assert_eq!(
            zmin_failure.2, git_failure.2,
            "failure stderr mismatch for {args:?}"
        );
        assert!(
            matches!(zmin_failure.0, 128 | 143) && matches!(git_failure.0, 128 | 143),
            "failure exit mismatch for {args:?}: zmin={} git={}",
            zmin_failure.0,
            git_failure.0
        );
        return;
    ...
    assert_eq!(zmin_failure, git_failure, "failure mismatch for {args:?}");
}
...
fn commit_conflict_and_invalid_cleanup_failures_match_stock_git() {
    ...
    git(repo.path(), ["add", "-A"]);
    assert_eq!(
        run_zmin_failure_output(repo.path(), &["commit", "--cleanup", "bogus", "-m", "msg"]),
...
#[test]
fn cat_file_incompatible_modes_return_usage_status_like_stock_git() {
    let repo = git_init();
    let (zmin_code, zmin_stdout, zmin_stderr) =
        command_failure_output(zmin_bin(), repo.path(), &["cat-file", "-e", "-p"], "zmin");
    let (git_code, _git_stdout, git_stderr) =
        command_failure_output("git", repo.path(), &["cat-file", "-e", "-p"], "git");

    assert_eq!(zmin_code, git_code);
    assert_eq!(zmin_code, 129);
    assert!(zmin_stdout.is_empty());
    assert!(
        zmin_stderr.contains("cannot be used together"),
        "unexpected zmin stderr: {zmin_stderr}"
    );
    assert!(
        git_stderr.contains("cannot be used together"),
        "unexpected git stderr: {git_stderr}"
    );
}
...
fn run_zmin_failure_output(cwd: &std::path::Path, args: &[&str]) -> (i32, String, String) {
    command_failure_output(zmin_bin(), cwd, args, "zmin")
}
