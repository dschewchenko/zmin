...
fn run_scalar(home: &TempDir, cwd: &std::path::Path, args: &[&str]) -> (i32, String, String) {
    run_scalar_command(common::zmin_bin(), home, cwd, args)
}
...
fn scalar_conflicting_and_incomplete_options_match_stock_usage_shape() {
    ...
    for args in [
        ...
        let zmin_parent = TempDir::new().expect("zmin parent");
        let stock =
            run_stock_scalar(&stock_home, stock_parent.path(), &args).expect("stock scalar");
        let mut zmin_args = vec!["scalar"];
        zmin_args.extend(args.iter().copied());
        let zmin = run_scalar(&zmin_home, zmin_parent.path(), &zmin_args);
        assert_eq!(zmin, stock, "{args:?}");
    }
}
...
fn scalar_run_maintenance_tasks_match_stock_empty_repo_shape() {
    for task in [
        ...
        let zmin_repo = common::git_init();
        let Some(stock) = run_stock_scalar(&stock_home, stock_repo.path(), &["run", task]) else {
            continue;
        };
        assert_eq!(
            run_scalar(&zmin_home, zmin_repo.path(), &["scalar", "run", task]),
            stock,
            "scalar run {task}"
        );
        if task == "all" {
...
fn scalar_diagnose_matches_stock_scalar_output_location() {
    ...
    let zmin_diagnose = run_scalar(&zmin_home, zmin_repo.path(), &["scalar", "diagnose"]);
    assert_eq!(zmin_diagnose.0, stock_diagnose.0);
    assert!(stock_diagnose.1.contains("Collecting diagnostic info"));
    assert!(zmin_diagnose.1.contains("Collecting diagnostic info"));
    assert!(stock_diagnose.2.contains("Diagnostics complete."));
    assert!(zmin_diagnose.2.contains("Diagnostics complete."));

    let stock_archives = scalar_diagnostic_archives(stock_repo.path());
    let zmin_archives = scalar_diagnostic_archives(zmin_repo.path());
    assert_eq!(stock_archives.len(), 1);
    assert_eq!(zmin_archives.len(), 1);
    assert!(
        stock_archives[0]
            .file_name()
    ...
    let bytes = std::fs::read(&zmin_archives[0]).expect("read zmin diagnostics archive");
...
fn scalar_delete_matches_stock_scalar_shape() {
    ...
    let zmin_repo = zmin_parent.path().join("repo");
    std::fs::create_dir(&stock_repo).expect("stock repo dir");
    std::fs::create_dir(&zmin_repo).expect("zmin repo dir");
    common::git(&stock_repo, ["init"]);
    common::git(&zmin_repo, ["init"]);
    let stock_path = canonical_path(&stock_repo);
    let zmin_path = canonical_path(&zmin_repo);
    ...
    let zmin_register = run_scalar(
        &zmin_home,
        zmin_parent.path(),
        &["scalar", "register", "--no-maintenance", &zmin_path],
    );
    assert_eq!(
        (
            zmin_register.0,
            zmin_register.1.as_str(),
            zmin_register.2.as_str()
        ),
        (
            stock_register.0,
    ...
    let stock_delete = run_stock_scalar(&stock_home, stock_parent.path(), &["delete", &stock_path])
        .expect("stock scalar");
    let zmin_delete = run_scalar(
        &zmin_home,
        zmin_parent.path(),
        &["scalar", "delete", &zmin_path],
    );
    assert_eq!(zmin_delete.0, stock_delete.0);
    assert_eq!(zmin_delete.1, zmin_path);
    assert_eq!(stock_delete.1, stock_path);
    assert_eq!(zmin_delete.2, stock_delete.2);
    assert!(!stock_repo.exists());
    assert!(!zmin_repo.exists());
}
...
fn scalar_register_list_unregister_match_stock_scalar_shape() {
    ...
    assert_eq!(zmin_register, stock_register);

    let stock_list = run_stock_scalar(&stock_home, repo.path(), &["list"]).expect("stock scalar");
    let zmin_list = run_scalar(&zmin_home, repo.path(), &["scalar", "list"]);
    assert_eq!(zmin_list, stock_list);
    assert_eq!(zmin_list.1, registered);

    let stock_unregister = run_stock_scalar(&stock_home, repo.path(), &["unregister", &registered])
        .expect("stock scalar");
    let zmin_unregister = run_scalar(
        &zmin_home,
        repo.path(),
        &["scalar", "unregister", &registered],
    );
    assert_eq!(zmin_unregister, stock_unregister);
}
...
fn scalar_repeated_register_and_unregister_match_stock_shape() {
    ...
    let zmin_path = canonical_path(zmin_repo.path());
    ...
    let zmin_first_register = run_scalar(
        &zmin_home,
        zmin_repo.path(),
        &["scalar", "register", "--no-maintenance", &zmin_path],
    );
    assert_eq!(zmin_first_register.0, stock_first_register.0);
    assert_eq!(zmin_first_register.1, stock_first_register.1);
    assert_eq!(zmin_first_register.2, stock_first_register.2);
    ...
    .expect("stock scalar");
    let zmin_second_register = run_scalar(
        &zmin_home,
        zmin_repo.path(),
        &["scalar", "register", "--no-maintenance", &zmin_path],
    );
    assert_eq!(zmin_second_register.0, stock_second_register.0);
    assert_eq!(zmin_second_register.1, zmin_path);
    assert_eq!(stock_second_register.1, stock_path);
    assert_eq!(zmin_second_register.2, stock_second_register.2);

    let stock_first_unregister =
        run_stock_scalar(&stock_home, stock_repo.path(), &["unregister", &stock_path])
            .expect("stock scalar");
    let zmin_first_unregister = run_scalar(
        &zmin_home,
        zmin_repo.path(),
        &["scalar", "unregister", &zmin_path],
    );
    assert_eq!(zmin_first_unregister.0, stock_first_unregister.0);
    assert_eq!(zmin_first_unregister.1, zmin_path);
    assert_eq!(stock_first_unregister.1, stock_path);
    assert_eq!(zmin_first_unregister.2, stock_first_unregister.2);

    let stock_second_unregister =
        run_stock_scalar(&stock_home, stock_repo.path(), &["unregister", &stock_path])
            .expect("stock scalar");
    let zmin_second_unregister = run_scalar(
        &zmin_home,
        zmin_repo.path(),
        &["scalar", "unregister", &zmin_path],
    );
    assert_eq!(zmin_second_unregister, stock_second_unregister);
}
...
fn scalar_run_config_and_reconfigure_keep_match_stock_scalar_config_effects() {
    ...
    let zmin_run = run_scalar(
        &zmin_home,
        zmin_repo.path(),
        &["scalar", "run", "config", &zmin_path],
    );
    assert_eq!(
        normalize_scalar_platform_stderr(zmin_run),
        normalize_scalar_platform_stderr(stock_run)
    );
    assert_eq!(
        common::git_status_args(zmin_repo.path(), &["config", "--get", "core.fsmonitor"]),
    ...
    let zmin_reconfigure_repo = common::git_init();
    let stock_reconfigure_path = canonical_path(stock_reconfigure_repo.path());
    let zmin_reconfigure_path = canonical_path(zmin_reconfigure_repo.path());
    ...
    .expect("stock scalar");
    let zmin_reconfigure = run_scalar(
        &zmin_home,
        zmin_reconfigure_repo.path(),
        &[
            "scalar",
            "reconfigure",
            "--maintenance=keep",
            &zmin_reconfigure_path,
        ],
    );
    assert_eq!(
        normalize_scalar_platform_stderr(zmin_reconfigure),
...
fn scalar_reconfigure_single_enlistment_matches_stock_maintenance_effects() {
    for mode in ["enable", "disable", "keep"] {
        ...
        assert_eq!(zmin_register, stock_register, "register {mode}");
        ...
        assert_eq!(zmin, stock, "reconfigure {mode}");

        for key in ["core.fsmonitor", "index.version", "maintenance.strategy"] {
            assert_eq!(
                common::git_status_args(zmin_repo.path(), &["config", "--get", key]),
        ...
        let zmin_global = global_gitconfig(&zmin_home).replace(&zmin_path, "<repo>");
        assert_eq!(zmin_global, stock_global, "{mode}: global config");
    }
}
