...
pub fn ensure_remote_http_helper() -> &'static Path {
    REMOTE_HTTP_HELPER
        .get_or_init(|| {
            if let Ok(path) = std::env::var("ZMIN_GIT_REMOTE_HTTP") {
...
pub fn run_zmin_args(cwd: &std::path::Path, args: &[&str]) -> String {
    command_output(zmin_bin(), cwd, args, "zmin").1
}

pub fn run_zmin_with_stdin<const N: usize>(
    cwd: &std::path::Path,
    args: [&str; N],
    stdin: &str,
) -> String {
    run_zmin_with_stdin_args(cwd, &args, stdin)
}

pub fn run_zmin_with_stdin_args(cwd: &std::path::Path, args: &[&str], stdin: &str) -> String {
    command_with_stdin(zmin_bin(), cwd, args, stdin, "zmin")
}

pub fn run_zmin_with_stdin_bytes<const N: usize>(
    cwd: &std::path::Path,
    args: [&str; N],
    stdin: &[u8],
) -> String {
    command_with_stdin_bytes(zmin_bin(), cwd, &args, stdin, "zmin")
}

pub fn run_zmin_with_env<const N: usize>(cwd: &std::path::Path, args: [&str; N]) -> String {
...
pub fn run_zmin_status_args(cwd: &std::path::Path, args: &[&str]) -> i32 {
    command_status(zmin_bin(), cwd, args, "zmin")
}
...
pub fn run_zmin_status_with_stdin<const N: usize>(
    cwd: &std::path::Path,
    args: [&str; N],
    stdin: &str,
) -> i32 {
    command_status_with_stdin(zmin_bin(), cwd, &args, stdin, "zmin")
}
...
pub fn run_zmin_failure_output(cwd: &Path, args: &[&str]) -> (i32, String, String) {
