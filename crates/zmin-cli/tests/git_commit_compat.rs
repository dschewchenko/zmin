...
fn commit_amend_matches_stock_git_state() {
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"initial\n").expect("write zmin a");
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"amended\n").expect("amend zmin a");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(git_repo.path(), ["commit", "--amend", "-m", "amended"]);
    run_zmin_with_env(zmin_repo.path(), ["commit", "--amend", "-m", "amended"]);
    ...
    assert_eq!(
        run_zmin(zmin_repo.path(), ["rev-list", "--parents", "HEAD"]),
        git(git_repo.path(), ["rev-list", "--parents", "HEAD"])
    );
    assert_eq!(
        run_zmin(zmin_repo.path(), ["rev-list", "--count", "HEAD"]),
    ...
    fs::write(zmin_repo.path().join("b.txt"), b"no edit\n").expect("write zmin b");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(git_repo.path(), ["commit", "--amend", "--no-edit"]);
    run_zmin_with_env(zmin_repo.path(), ["commit", "--amend", "--no-edit"]);
    ...
    fs::write(git_repo.path().join("c.txt"), b"reuse message\n").expect("write git c");
    fs::write(zmin_repo.path().join("c.txt"), b"reuse message\n").expect("write zmin c");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
...
#[test]
fn commit_dot_pathspec_matches_stock_git_state() {
    let git_repo = git_init();
    let zmin_repo = git_init();
    configure_identity(git_repo.path());
    configure_identity(zmin_repo.path());
    ...
    for repo in [git_repo.path(), zmin_repo.path()] {
        fs::write(repo.join(".gitattributes"), b"\n").expect("write attributes");
    }
    git(git_repo.path(), ["add", ".gitattributes"]);
    run_zmin(zmin_repo.path(), ["add", ".gitattributes"]);
    git_with_env(git_repo.path(), ["commit", "-m", "add attrs", "."]);
    run_zmin_with_env(zmin_repo.path(), ["commit", "-m", "add attrs", "."]);
    ...
    assert_eq!(
        run_zmin(zmin_repo.path(), ["status", "--porcelain"]),
...
fn commit_pathspec_only_and_fixup_match_stock_git_state() {
    ...
    run_zmin(zmin_repo.path(), ["add", "b.txt"]);
    git_with_env(git_repo.path(), ["commit", "-m", "path", "--", "a.txt"]);
    run_zmin_with_env(zmin_repo.path(), ["commit", "-m", "path", "--", "a.txt"]);
    assert_eq!(
        git(zmin_repo.path(), ["cat-file", "-p", "HEAD"]),
        git(git_repo.path(), ["cat-file", "-p", "HEAD"])
    );
    ...
    run_zmin_with_env(zmin_repo.path(), ["commit", "--fixup", "HEAD"]);
    assert_eq!(
        git(zmin_repo.path(), ["cat-file", "-p", "HEAD"]),
        git(git_repo.path(), ["cat-file", "-p", "HEAD"])
    );
    ...
    run_zmin(zmin_repo.path(), ["add", "c.txt"]);
    assert_eq!(
        command_output_with_env(
            zmin_bin(),
            zmin_repo.path(),
    ...
    assert_eq!(
        fs::read_to_string(zmin_repo.path().join(".git/editor-input.txt"))
            .expect("read zmin fixup amend editor input"),
    ...
    run_zmin(zmin_repo.path(), ["add", "d.txt"]);
    assert_eq!(
        command_output_with_env(
            zmin_bin(),
            zmin_repo.path(),
    ...
    assert_eq!(
        fs::read_to_string(zmin_repo.path().join(".git/editor-input.txt"))
            .expect("read zmin fixup reword editor input"),
        fs::read_to_string(git_repo.path().join(".git/editor-input.txt"))
...
fn commit_messages_match_stock_git_object() {
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"multi\n").expect("write zmin a");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(git_repo.path(), ["commit", "-m", "subject", "-m", "body"]);
    run_zmin_with_env(zmin_repo.path(), ["commit", "-m", "subject", "-m", "body"]);
    assert_eq!(
        git(zmin_repo.path(), ["cat-file", "-p", "HEAD"]),
        git(git_repo.path(), ["cat-file", "-p", "HEAD"])
    );
    ...
    fs::write(zmin_repo.path().join("message.txt"), b"from file\n\nbody")
        .expect("write zmin message");
    fs::write(git_repo.path().join("file-message.txt"), b"file message\n")
        .expect("write git file message");
    fs::write(zmin_repo.path().join("file-message.txt"), b"file message\n")
        .expect("write zmin file message");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(git_repo.path(), ["commit", "-F", "message.txt"]);
    run_zmin_with_env(zmin_repo.path(), ["commit", "-F", "message.txt"]);
    assert_eq!(
        git(zmin_repo.path(), ["cat-file", "-p", "HEAD"]),
        git(git_repo.path(), ["cat-file", "-p", "HEAD"])
    );
    ...
    fs::write(zmin_repo.path().join("empty.txt"), b"empty\n").expect("write zmin empty");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(
        git_repo.path(),
    ...
    fs::write(zmin_repo.path().join("amend.txt"), b"amend\n").expect("write zmin amend");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(
        git_repo.path(),
...
#[test]
fn commit_long_message_option_matches_stock_git_object() {
    let git_repo = git_init();
    configure_identity(git_repo.path());
...
fn commit_reuse_message_matches_stock_git_object() {
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"base\n").expect("write zmin a");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(git_repo.path(), ["commit", "-m", "base", "-m", "body"]);
    run_zmin_with_env(zmin_repo.path(), ["commit", "-m", "base", "-m", "body"]);

    fs::write(git_repo.path().join("b.txt"), b"reuse\n").expect("write git b");
    fs::write(zmin_repo.path().join("b.txt"), b"reuse\n").expect("write zmin b");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(git_repo.path(), ["commit", "-C", "HEAD"]);
    run_zmin_with_env(zmin_repo.path(), ["commit", "-C", "HEAD"]);
    assert_eq!(
        git(zmin_repo.path(), ["cat-file", "-p", "HEAD"]),
        git(git_repo.path(), ["cat-file", "-p", "HEAD"])
    );
    ...
    fs::write(zmin_repo.path().join("c.txt"), b"override\n").expect("write zmin c");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(
        git_repo.path(),
    ...
    fs::write(zmin_repo.path().join("d.txt"), b"reedit\n").expect("write zmin d");
    install_capture_commit_editor(git_repo.path());
    install_capture_commit_editor(zmin_repo.path());
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    ...
    fs::write(zmin_repo.path().join("e.txt"), b"reedit unchanged\n").expect("write zmin e");
...
fn commit_edit_and_no_edit_message_sources_match_stock_git() {
    ...
    run_zmin(zmin_repo.path(), ["add", "base.txt"]);
    git_with_env(
        git_repo.path(),
    ...
    .enumerate()
    {
        ...
        assert_eq!(
            command_output_with_env(zmin_bin(), zmin_repo.path(), &args, &zmin_env, "zmin"),
        ...
        let zmin_editor_input = zmin_repo.path().join(".git/editor-input.txt");
        let git_editor_input = git_repo.path().join(".git/editor-input.txt");
        assert_eq!(
            zmin_editor_input.exists(),
            git_editor_input.exists(),
            "editor invocation mismatch: {args:?}"
        );
        if zmin_editor_input.exists() {
            assert_eq!(
                fs::read_to_string(zmin_editor_input).expect("read zmin editor input"),
...
fn commit_author_matches_stock_git_object() {
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"author\n").expect("write zmin a");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(
        git_repo.path(),
...
fn commit_date_matches_stock_git_object() {
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"date\n").expect("write zmin a");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(
        git_repo.path(),
...
fn commit_amend_author_date_options_match_stock_git_object() {
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"init\n").expect("write zmin a");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(git_repo.path(), ["commit", "-m", "init"]);
    run_zmin_with_env(zmin_repo.path(), ["commit", "-m", "init"]);

    fs::write(git_repo.path().join("a.txt"), b"amend\n").expect("write git amend");
    fs::write(zmin_repo.path().join("a.txt"), b"amend\n").expect("write zmin amend");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(
        git_repo.path(),
...
fn commit_reset_author_matches_stock_git_object() {
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"reset\n").expect("write zmin reset");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(
        git_repo.path(),
...
fn commit_summary_and_quiet_output_match_stock_git() {
    ...
    run_zmin(zmin_repo.path(), ["rm", "dir/b.txt"]);
    assert_eq!(
        command_output_with_env(
            zmin_bin(),
            zmin_repo.path(),
...
fn commit_hooks_match_stock_git_flow() {
    ...
    install_commit_hook_set(zmin_repo.path());

    fs::write(git_repo.path().join("a.txt"), b"hooks\n").expect("write git hooks");
    fs::write(zmin_repo.path().join("a.txt"), b"hooks\n").expect("write zmin hooks");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    assert_eq!(
        command_output_with_env(
    ...
    assert_eq!(
        fs::read_to_string(zmin_repo.path().join("hook.log")).expect("read zmin hook log"),
    ...
    fs::write(zmin_repo.path().join("b.txt"), b"skip\n").expect("write zmin skip");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    assert_eq!(
        command_output_with_env(
...
fn commit_prepare_and_post_rewrite_hooks_match_stock_git_flow() {
    ...
    install_prepare_commit_msg_hook(zmin_repo.path());

    fs::write(git_repo.path().join("a.txt"), b"prepare\n").expect("write git prepare");
    fs::write(zmin_repo.path().join("a.txt"), b"prepare\n").expect("write zmin prepare");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    assert_eq!(
        command_output_with_env(
    ...
    fs::write(zmin_repo.path().join("b.txt"), b"amend\n").expect("write zmin amend");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    assert_eq!(
        command_output_with_env(
    ...
    assert_eq!(
        fs::read_to_string(zmin_repo.path().join("rewrite.log")).expect("read zmin rewrite log"),
...
fn commit_hook_failures_match_stock_git_flow() {
    for hook_name in ["pre-commit", "commit-msg"] {
        ...
        install_failing_commit_hook(zmin_repo.path(), hook_name);
        fs::write(git_repo.path().join("a.txt"), b"fail\n").expect("write git fail");
        fs::write(zmin_repo.path().join("a.txt"), b"fail\n").expect("write zmin fail");
        git(git_repo.path(), ["add", "-A"]);
        run_zmin(zmin_repo.path(), ["add", "-A"]);
        assert_eq!(
    ...
    install_failing_commit_hook(zmin_repo.path(), "post-commit");
    fs::write(git_repo.path().join("a.txt"), b"post\n").expect("write git post");
    fs::write(zmin_repo.path().join("a.txt"), b"post\n").expect("write zmin post");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    assert_eq!(
        command_output_with_env(
...
fn commit_template_editor_matches_stock_git_object() {
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"template\n").expect("write zmin template");
    fs::write(
    ...
    fs::write(
        zmin_repo.path().join("template.txt"),
        b"template subject\n\n# comment\n",
    )
    .expect("write zmin template message");
    fs::write(
    ...
    let zmin_editor = zmin_repo.path().join("editor.sh");
    let env = [
        ("GIT_AUTHOR_NAME", "Bench"),
        ("GIT_AUTHOR_EMAIL", "bench@example.test"),
        ("GIT_AUTHOR_DATE", "1700000000 +0000"),
    ...
    let zmin_editor_path = zmin_editor.to_str().expect("zmin editor path");
    let mut git_env = env.to_vec();
    git_env.push(("GIT_EDITOR", git_editor_path));
    let mut zmin_env = env.to_vec();
    zmin_env.push(("GIT_EDITOR", zmin_editor_path));
    assert_eq!(
        command_output_with_env(
            zmin_bin(),
            zmin_repo.path(),
    ...
    fs::write(zmin_repo.path().join("b.txt"), b"config template\n").expect("write zmin b");
    fs::write(
    ...
    fs::write(
        zmin_repo.path().join("config-template.txt"),
        b"config template subject\n",
    )
    .expect("write zmin config template");
    git(
        git_repo.path(),
    ...
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    assert_eq!(
        command_output_with_env(zmin_bin(), zmin_repo.path(), &["commit"], &zmin_env, "zmin").0,
...
fn commit_editor_without_message_matches_stock_git_buffer() {
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"normal editor\n").expect("write zmin a");
    git(git_repo.path(), ["add", "a.txt"]);
    run_zmin(zmin_repo.path(), ["add", "a.txt"]);
    ...
    assert_eq!(
        command_output_with_env(zmin_bin(), zmin_repo.path(), &["commit"], &zmin_env, "zmin"),
...
fn commit_status_option_and_config_match_stock_git_buffer() {
    for (config_status, args) in [
        ...
        if let Some(value) = config_status {
            ...
            run_zmin(zmin_repo.path(), ["config", "commit.status", value]);
        }

        fs::write(git_repo.path().join("a.txt"), b"status option\n").expect("write git a");
        fs::write(zmin_repo.path().join("a.txt"), b"status option\n").expect("write zmin a");
        git(git_repo.path(), ["add", "a.txt"]);
        run_zmin(zmin_repo.path(), ["add", "a.txt"]);
...
fn commit_editor_empty_and_unchanged_abort_like_stock_git() {
    for editor_body in [
        ...
        install_custom_commit_editor(zmin_repo.path(), editor_body);

        fs::write(git_repo.path().join("a.txt"), b"abort editor\n").expect("write git a");
        fs::write(zmin_repo.path().join("a.txt"), b"abort editor\n").expect("write zmin a");
        git(git_repo.path(), ["add", "a.txt"]);
        run_zmin(zmin_repo.path(), ["add", "a.txt"]);
...
fn commit_template_requires_editor_change_like_stock_git() {
    ...
    fs::write(git_repo.path().join("template.txt"), b"template subject\n")
        .expect("write git template message");
    fs::write(zmin_repo.path().join("template.txt"), b"template subject\n")
        .expect("write zmin template message");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
...
fn commit_cleanup_modes_match_stock_git_object() {
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"cleanup\n").expect("write zmin cleanup");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    ...
    fs::write(
        zmin_repo.path().join("message.txt"),
        b"subject\n\n body  \n# comment\n\n \n  \n",
    )
    .expect("write zmin commit cleanup message");
    ...
    .enumerate()
    {
        ...
        fs::write(
            zmin_repo.path().join("a.txt"),
            format!("cleanup {idx}\n").as_bytes(),
        )
        .expect("rewrite zmin cleanup target");
        git(git_repo.path(), ["add", "-A"]);
        run_zmin(zmin_repo.path(), ["add", "-A"]);
        git_with_env(git_repo.path(), *args);
        run_zmin_with_env(zmin_repo.path(), *args);
        assert_eq!(
            git(zmin_repo.path(), ["cat-file", "-p", "HEAD"]),
            git(git_repo.path(), ["cat-file", "-p", "HEAD"])
        );
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"cleanup 3\n").expect("rewrite zmin cleanup target");
...
fn commit_signoff_matches_stock_git_object() {
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"signoff\n").expect("write zmin signoff");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
...
fn commit_trailers_match_stock_git_object_and_editor_buffer() {
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"trailer\n").expect("write zmin trailer");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(
        git_repo.path(),
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"trailer\nfile\n")
        .expect("rewrite zmin file trailer");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(
        git_repo.path(),
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"trailer\nmulti\n")
        .expect("rewrite zmin multi trailer");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(
        git_repo.path(),
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"trailer\nsignoff\n")
        .expect("rewrite zmin signoff trailer");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(
        git_repo.path(),
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"trailer\neditor\n")
        .expect("rewrite zmin editor trailer");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    let git_editor = git_repo.path().join(".git/editor.sh");
    let zmin_editor = zmin_repo.path().join(".git/editor.sh");
    ...
    assert!(zmin_output.1.contains("Final subject"));
    assert!(git_output.1.contains("Final subject"));
    assert_eq!(
        fs::read_to_string(zmin_repo.path().join(".git/editor-input.txt"))
            .expect("read zmin trailer editor input"),
        fs::read_to_string(git_repo.path().join(".git/editor-input.txt"))
...
fn commit_squash_matches_stock_git_object() {
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"squash\n").expect("write zmin squash base");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(git_repo.path(), ["commit", "-m", "base"]);
    run_zmin_with_env(zmin_repo.path(), ["commit", "-m", "base"]);
    ...
    fs::write(zmin_repo.path().join("a.txt"), b"squash target\n")
        .expect("write zmin squash target");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(git_repo.path(), ["commit", "-m", "target"]);
    run_zmin_with_env(zmin_repo.path(), ["commit", "-m", "target"]);

    fs::write(git_repo.path().join("a.txt"), b"squash work\n").expect("write git squash work");
    fs::write(zmin_repo.path().join("a.txt"), b"squash work\n").expect("write zmin squash work");
    git(git_repo.path(), ["add", "-A"]);
    run_zmin(zmin_repo.path(), ["add", "-A"]);
    git_with_env(
        git_repo.path(),
