...
fn stash_list_formats_match_stock_git() {
    ...
    for args in [
        ...
        ["stash", "list", "--format=%gN|%gE|%gn|%ge"].as_slice(),
        ["stash", "list", "--format=%GS|%GG"].as_slice(),
...
#[test]
fn stash_list_missing_prefix_format_atoms_match_stock_git_literals() {
    let git_repo = stash_fixture_repo();
    let zmin_repo = clone_repo_fixture(git_repo.path());
    configure_identity(zmin_repo.path());
    ...
    write_file(git_repo.path(), "a.txt", "one\nstashed\n");
    write_file(zmin_repo.path(), "a.txt", "one\nstashed\n");
    git_with_env(git_repo.path(), ["stash", "push", "-m", "prefix"]);
    run_zmin_with_env(zmin_repo.path(), ["stash", "push", "-m", "prefix"]);

    for args in [
        ["stash", "list", "--format=%C"].as_slice(),
        ["stash", "list", "--format=%G"].as_slice(),
        ["stash", "list", "--format=%g"].as_slice(),
        ["stash", "list", "--format=%a"].as_slice(),
        ["stash", "list", "--format=%c"].as_slice(),
    ] {
        assert_eq!(
            run_zmin_args(zmin_repo.path(), args),
            git_args(git_repo.path(), args),
            "missing prefix format atom should stay literal for {args:?}",
        );
    }
}

#[test]
fn stash_list_unterminated_color_format_atoms_match_stock_git_literals() {
    let git_repo = stash_fixture_repo();
    let zmin_repo = clone_repo_fixture(git_repo.path());
    configure_identity(zmin_repo.path());
    ...
    write_file(git_repo.path(), "a.txt", "one\nstashed\n");
    write_file(zmin_repo.path(), "a.txt", "one\nstashed\n");
    git_with_env(git_repo.path(), ["stash", "push", "-m", "color"]);
    run_zmin_with_env(zmin_repo.path(), ["stash", "push", "-m", "color"]);

    for args in [
        ["stash", "list", "--format=%C("].as_slice(),
        ["stash", "list", "--format=%C(red"].as_slice(),
        ["stash", "list", "--format=%C(always,red"].as_slice(),
    ] {
        assert_eq!(
            run_zmin_args(zmin_repo.path(), args),
            git_args(git_repo.path(), args),
            "unterminated %C color atom should stay literal for {args:?}",
        );
    }
}

#[test]
fn stash_list_invalid_forced_color_format_atom_matches_stock_git() {
    let git_repo = stash_fixture_repo();
    let zmin_repo = clone_repo_fixture(git_repo.path());
    configure_identity(zmin_repo.path());
    ...
    assert_eq!(
        run_zmin_failure_output(
            zmin_repo.path(),
            &["stash", "list", "--format=%C(always,bad)%h"]
        ),
        git_failure_output(
            git_repo.path(),
            &["stash", "list", "--format=%C(always,bad)%h"]
...
#[test]
fn stash_list_width_format_atoms_without_target_match_stock_git() {
    let git_repo = stash_fixture_repo();
    let zmin_repo = clone_repo_fixture(git_repo.path());
    configure_identity(zmin_repo.path());
    ...
    for args in [
        ["stash", "list", "--format=%<(10)"].as_slice(),
        ["stash", "list", "--format=%>(10)"].as_slice(),
        ["stash", "list", "--format=%<(10,trunc)"].as_slice(),
        ["stash", "list", "--format=%>(10,trunc)"].as_slice(),
    ] {
        assert_eq!(
            run_zmin_args(zmin_repo.path(), args),
            git_args(git_repo.path(), args),
            "width atom without a target should match stock Git for {args:?}",
        );
    }
}
