...
#[cfg(unix)]
fn write_delayed_clean_filter_helper(path: &std::path::Path) {
    fs::write(
        path,
        r#"use strict;
use warnings;
...
"#,
    ...
    .expect("write delayed clean filter helper");
}
...
#[cfg(unix)]
#[test]
fn add_process_filter_delayed_clean_status_matches_stock_git() {
    let git_repo = git_init();
    let zmin_repo = git_init();
    let helper_dir = TempDir::new().expect("helper tempdir");
    let helper = helper_dir.path().join("delayed-clean-filter.pl");
    write_delayed_clean_filter_helper(&helper);
    let command = format!("perl {}", helper.display());
    ...
    assert_eq!(
        command_any_output(
            ...
            zmin_repo.path(),
            &["add", "a.bad"],
            "zmin add delayed clean process filter",
        ),
        command_any_output(
            "git",
            git_repo.path(),
            &["add", "a.bad"],
            "git add delayed clean process filter",
        )
    );
    assert_eq!(
        run_zmin(zmin_repo.path(), ["status", "--porcelain=v1"]),
