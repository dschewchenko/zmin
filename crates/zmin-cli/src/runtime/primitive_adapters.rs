...
use zmin_git_core::{TreeEntry, TreeMode, decode_commit, encode_tree};
...
impl GitTransport for CliTransportAdapter {
    fn discover_refs(&self, remote: &GitEndpoint) -> PrimitiveResult<RefDiscovery> {
        ...
        let common_dir =
            read_common_git_dir(&source.git_dir).map_err(|error| PrimitiveError::Storage {
                details: format!("read local common dir for {remote}: {error:?}"),
            })?;
        let common_refs = RefStore::new(&common_dir, GitHashAlgorithm::Sha1);
        ...
        common_refs
            .for_each_resolved_ref("refs/heads/", |name, id| {
        ...
        common_refs
            .for_each_resolved_ref("refs/tags/", |name, id| {
                discovered.insert(name.to_owned(), id.to_hex());
                if let Some(peeled) =
    ...
    fn upload_pack_request(
        ...
        run_upload_pack_request_service(&repo, stdin, false).map_err(Self::map_cli_error)
    }
    ...
    fn receive_pack_request(
        ...
        let repo = Self::local_source(&request.remote)?;
        run_receive_pack_request_service(&repo, stdin).map_err(Self::map_cli_error)
    }
...
mod transport_tests {
    ...
    use zmin_primitives::git_runtime::GitTransportService;
    ...
    fn upload_pack_request_from_local_remote_returns_pack_response() {
        ...
        let object_id =
            zmin_git_core::CommitBuilder::new(tree_id.clone(), signature.clone(), signature)
...
impl CoreGitObjectStore for OwnedCliObjectStoreAdapter {
    fn read_object(&self, id: &ObjectId) -> std::io::Result<zmin_git_core::LooseObject> {
...
impl OwnedCliRefsStoreAdapter {
    ...
    pub(crate) fn objects_dir(&self) -> PathBuf {
        self.common_refs.git_dir().join("objects")
    }
    ...
    pub(crate) fn read_ref_oid(&self, name: &str) -> super::Result<Option<ObjectId>> {
        match self.common_refs.read_ref(name) {
            Ok(RefTarget::Direct(id)) => Ok(Some(id)),
            Ok(RefTarget::Symbolic(target)) => Ok(self.common_refs.resolve(&target).ok()),
            Err(error) if error.kind() == io::ErrorKind::NotFound => Ok(None),
    ...
    pub(crate) fn resolve_ref(&self, name: &str) -> super::Result<Option<ObjectId>> {
        match self.common_refs.resolve(name) {
            Ok(id) => Ok(Some(id)),
            Err(error) if error.kind() == io::ErrorKind::NotFound => Ok(None),
            Err(error) => Err(CliError::Io(error)),
        }
    ...
    {
        self.common_refs.for_each_server_info_ref(visitor)
    }
...
impl OwnedCliRefsStoreAdapter {
    ...
    fn receive_allows_deleting_current_branch(&self) -> super::Result<bool> {
        let entries = read_config_file(&self.common_refs.git_dir().join("config"))?;
        let value = entries
            .into_iter()
            .rev()
            .find(|entry| {
...
impl GitRefsStore for OwnedCliRefsStoreAdapter {
    fn read_ref(&self, name: &GitRefName) -> PrimitiveResult<Option<String>> {
        if name == "HEAD" {
            let target = self.refs.read_head();
            return match target {
                Ok(RefTarget::Direct(id)) => Ok(Some(id.to_hex())),
                Ok(RefTarget::Symbolic(target)) => self
                    .common_refs
                    .resolve(&target)
                    .map(|resolved| Some(resolved.to_hex()))
                    .map_err(|error| PrimitiveError::Storage {
                        details: format!("failed to resolve symbolic ref HEAD: {error}"),
                    }),
                Err(error) if error.kind() == io::ErrorKind::NotFound => Ok(None),
                Err(error) => Err(PrimitiveError::Storage {
                    details: format!("read ref {name}: {error}"),
    ...
    fn write_ref(&self, name: &GitRefName, value: &String) -> PrimitiveResult<()> {
        ...
        CliRefsStoreAdapter::new(refs).write_ref(name, value)
    }
    ...
    fn write_symbolic_ref(&self, name: &GitRefName, target: &String) -> PrimitiveResult<()> {
        ...
        CliRefsStoreAdapter::new(refs).write_symbolic_ref(name, target)
    }
    ...
    fn apply_ref_updates(&self, updates: &[RefUpdate]) -> PrimitiveResult<()> {
        CliRefsStoreAdapter::new(&self.common_refs).apply_ref_updates(updates)
    }

    fn begin_transaction(&self) -> PrimitiveResult<()> {
        CliRefsStoreAdapter::new(&self.common_refs).begin_transaction()
    }

    fn commit_transaction(&self) -> PrimitiveResult<()> {
        CliRefsStoreAdapter::new(&self.common_refs).commit_transaction()
    }

    fn rollback_transaction(&self) -> PrimitiveResult<()> {
        CliRefsStoreAdapter::new(&self.common_refs).rollback_transaction()
    }
}
