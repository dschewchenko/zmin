...
fn diff_entry_visible_with_options(
    ...
    let row = match diff_stat_row_with_whitespace(context, entry, options) {
        Ok(row) => row,
...
pub(crate) fn parse_diff_input(
    ...
    if !cached && let Some(input) = parse_blob_pair_diff_input(repo, store, &args)? {
        return Ok(input);
    }
...
fn write_format_patch_attach_body<W: Write, S: GitObjectStore + ?Sized>(
    ...
    let filename = if context.numbered_files {
        ...
        format_patch_filename_with_suffix(entry.number, &subject, context.suffix)
    };
...
pub(crate) fn write_format_patch_cover_letter<W: Write, S: GitObjectStore + ?Sized>(
    ...
    writeln!(
        out,
        "From: {} <{}>",
        signature_name(cover_signature),
...
fn write_format_patch_header<W: Write>(
    ...
    if context.attach || context.inline {
        ...
        writeln!(
            out,
            "Content-Type: multipart/mixed; boundary=\"------------0.1.0.zmin\""
        )?;
        writeln!(out)?;
        return Ok(());
    }
...
fn hunk_header(
    ...
    hunk_line_header(old_lines, old_start.saturating_sub(1)).filter(|header| {
        !header
            .bytes()
