...
pub(crate) fn patch_id_generate(input: &[u8], mode: PatchIdMode) -> Vec<([u8; 20], String)> {
...
pub(crate) fn patch_id_normalize_line(line: &[u8], mode: PatchIdMode) -> Vec<u8> {
    if mode == PatchIdMode::Verbatim {
        return line.to_vec();
    }
    line.iter()
        .copied()
