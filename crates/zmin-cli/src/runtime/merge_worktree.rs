...
pub(crate) fn rm_path_matches(
    ...
    if relative.is_empty() {
        if !recursive {
            return Err(CliError::Fatal {
                code: 128,
                message: "not removing recursively without -r".into(),
            });
        }
        return Ok(index
            .entries()
...
pub(crate) fn fast_forward_to_cached(
    ...
    let current_id = {
        let _trace = phase_trace("fast_forward.resolve_current");
        match resolve_commitish_io(repo, store, "HEAD") {
            Ok(id) => Some(id),
    ...
    let target_id = {
        let _trace = phase_trace("fast_forward.resolve_target");
    ...
    {
        let _trace = phase_trace("fast_forward.checkout");
        checkout_clean_worktree_transition_after_clean_check_with_metadata(
            repo,
            store,
            &target_id,
            &checkout_metadata,
    ...
    {
        let _trace = phase_trace("fast_forward.update_refs");
        if let Some(current_id) = &current_id {
    ...
    {
        let _trace = phase_trace("fast_forward.render");
        if let Some(current_id) = &current_id {
            println!(
                "Updating {}..{}",
