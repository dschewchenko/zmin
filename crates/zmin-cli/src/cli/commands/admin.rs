use crate::runtime;

pub(crate) fn dispatch(
    command: runtime::Command,
    raw_args: &[String],
) -> std::result::Result<(), runtime::CliError> {
    match command {
        runtime::Command::ForEachRepo {
            config,
            keep_going,
            arguments,
        } => super::admin_commands::for_each_repo_command(&config, keep_going, arguments),
        runtime::Command::UpdateIndex {
            add,
            remove,
            force_remove,
            replace,
            again,
            quiet,
            refresh,
            really_refresh,
            ignore_submodules,
            ignore_missing,
            unmerged,
            ignore_skip_worktree_entries,
            no_ignore_skip_worktree_entries,
            unresolve,
            info_only,
            cacheinfo,
            index_info_mode,
            index_version,
            show_index_version,
            split_index,
            no_split_index,
            untracked_cache,
            no_untracked_cache,
            force_untracked_cache,
            test_untracked_cache,
            fsmonitor,
            no_fsmonitor,
            fsmonitor_valid,
            no_fsmonitor_valid,
            verbose,
            chmod,
            assume_unchanged,
            no_assume_unchanged,
            skip_worktree,
            no_skip_worktree,
            stdin,
            nul_terminated,
            paths,
        } => super::admin_commands::update_index_command({
            if stdin > 1 {
                return Err(runtime::CliError::Stderr {
                    code: 129,
                    text: "error: option 'stdin' must be the last argument\n".into(),
                });
            }
            if index_info_mode > 1 {
                return Err(runtime::CliError::Stderr {
                    code: 129,
                    text: "error: option 'index-info' must be the last argument\n".into(),
                });
            }
            super::admin_commands::UpdateIndexCommandOptions {
                add: add > 0,
                remove: remove > 0,
                force_remove: force_remove > 0,
                replace: replace > 0,
                again: again > 0,
                quiet: quiet > 0,
                refresh: refresh > 0 || really_refresh > 0,
                ignore_submodules: ignore_submodules > 0,
                ignore_missing: ignore_missing > 0,
                unmerged: unmerged > 0,
                ignore_skip_worktree_entries: ignore_skip_worktree_entries > 0,
                no_ignore_skip_worktree_entries: no_ignore_skip_worktree_entries > 0,
                unresolve: unresolve > 0,
                info_only: info_only > 0,
                cacheinfo,
                index_info: index_info_mode > 0,
                index_version,
                show_index_version: show_index_version > 0,
                split_index: split_index > 0,
                no_split_index: no_split_index > 0,
                untracked_cache: untracked_cache > 0,
                no_untracked_cache: no_untracked_cache > 0,
                force_untracked_cache: force_untracked_cache > 0,
                test_untracked_cache: test_untracked_cache > 0,
                fsmonitor: fsmonitor > 0,
                no_fsmonitor: no_fsmonitor > 0,
                fsmonitor_valid: fsmonitor_valid > 0,
                no_fsmonitor_valid: no_fsmonitor_valid > 0,
                verbose: verbose > 0,
                chmod,
                assume_unchanged: assume_unchanged > 0,
                no_assume_unchanged: no_assume_unchanged > 0,
                skip_worktree: skip_worktree > 0,
                no_skip_worktree: no_skip_worktree > 0,
                stdin: stdin > 0,
                nul_terminated: nul_terminated > 0,
                paths,
            }
        }),
        runtime::Command::Bugreport {
            output_directory,
            suffix,
            no_suffix,
            diagnose,
            no_diagnose,
        } => super::admin_commands::bugreport_command(
            output_directory,
            suffix.as_deref(),
            no_suffix,
            resolve_bugreport_diagnose(raw_args, diagnose.as_deref(), no_diagnose),
        ),
        runtime::Command::Diagnose {
            output_directory,
            suffix,
            mode,
        } => super::admin_commands::diagnose_command_entry(
            output_directory,
            suffix.as_deref(),
            &mode,
        ),
        runtime::Command::Backfill {
            min_batch_size,
            sparse,
            no_sparse,
            revs,
        } => super::admin_commands::backfill_command(min_batch_size, sparse, no_sparse, revs),
        runtime::Command::Gitk { args } => {
            super::admin_commands::not_ready_current_git_command("gitk", args)
        }
        runtime::Command::Gitweb { args } => {
            super::admin_commands::not_ready_current_git_command("gitweb", args)
        }
        runtime::Command::Scalar {
            directories,
            configs,
            help,
            command,
        } => super::scalar_commands::scalar_command(directories, configs, help, command),
        runtime::Command::Hook { command } => run_hook(command),
        runtime::Command::Hooks { command } => super::admin_commands::managed_hooks(command),
        runtime::Command::ShI18n { args } => super::admin_commands::sh_i18n_command(args),
        runtime::Command::ShSetup { args } => super::admin_commands::sh_setup_command(args),
        runtime::Command::Cvsserver { args } => super::admin_commands::cvsserver_command(args),
        runtime::Command::Cvsexportcommit { args } => {
            super::admin_commands::cvsexportcommit_command(args)
        }
        runtime::Command::Cvsimport { args } => super::admin_commands::cvsimport_command(args),
        runtime::Command::Archimport { args } => super::admin_commands::archimport_command(args),
        runtime::Command::P4 { args } => super::admin_commands::p4_command(args),
        runtime::Command::Svn { args } => super::admin_commands::svn_command(args),
        runtime::Command::Instaweb {
            start,
            stop,
            restart,
            local,
            port,
            httpd,
            module_path,
            browser,
            daemon_internal,
            git_dir,
            work_tree,
        } => {
            super::admin_commands::instaweb_command(super::admin_commands::InstawebCommandOptions {
                start,
                stop,
                restart,
                local,
                port,
                httpd,
                module_path,
                browser,
                daemon_internal,
                git_dir,
                work_tree,
            })
        }
        _ => unreachable!("non-admin command dispatched to admin"),
    }
}

fn resolve_bugreport_diagnose<'a>(
    raw_args: &[String],
    diagnose: Option<&'a str>,
    no_diagnose: bool,
) -> Option<&'a str> {
    if diagnose.is_none() && !no_diagnose {
        return None;
    }
    let mut mode = diagnose;
    for arg in raw_args.iter().skip(1).map(String::as_str) {
        if arg == "--no-diagnose" {
            mode = None;
        } else if arg == "--diagnose" || arg.starts_with("--diagnose=") {
            mode = diagnose;
        }
    }
    mode
}

pub(crate) fn run_hook(
    command: runtime::HookCommand,
) -> std::result::Result<(), runtime::CliError> {
    super::admin_commands::hook(command)
}
