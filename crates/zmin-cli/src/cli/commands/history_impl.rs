...
fn log_with_options(options: LogOptions<'_>) -> Result<()> {
    ...
    let (revs, max_count) = split_log_revs_and_count(options.revs.clone(), options.max_count)?;
    ...
    let abbrev_len = {
        let _trace = phase_trace("log.default_abbrev_len");
...
fn split_log_revs_and_count(
    ...
    while let Some(rev) = iter.next() {
        if let Some(value) = rev.strip_prefix('-')
            && !value.is_empty()
    ...
    Ok((parsed_revs, parsed_max_count))
}
...
impl LogDecorations {
    ...
    fn load(
        ...
        ref_rows.sort_by(|left, right| left.0.cmp(&right.0));
...
fn decorate_ref_name(ref_name: &str, mode: LogDecorationMode, head_target: bool) -> String {
    if matches!(mode, LogDecorationMode::Full) || ref_name.starts_with("refs/notes/") {
...
impl<'a> LogFormat<'a> {
    ...
    pub(crate) fn render_with_context(
        ...
        match self {
            ...
            Self::Custom { pattern, .. } => {
                render_log_format(pattern, id, commit, abbrev_len, notes)
            }
        }
    }
...
fn log_showroot_enabled(repo: &GitRepo) -> Result<bool> {
    ...
    entry.bool_value().ok_or_else(|| CliError::Fatal {
        code: 128,
        message: format!("bad boolean config value '{}' for 'log.showroot'", entry.value),
    })
}
