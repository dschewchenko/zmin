...
fn fetch_multiple_refspecs_from_http_remote(
    ...
    for refspec in refspecs {
        if let Some(row) = http_refspec_source_row(&rows, refspec)? {
            resolved.push((refspec.clone(), row));
        }
    }
    let destination_refs = refs_adapter_from_git_dir(&repo.git_dir);
    let store = object_adapter_from_objects_dir(repo.objects_dir.clone());
...
fn http_refspec_source_row(rows: &[LsRemoteRow], refspec: &str) -> Result<Option<LsRemoteRow>> {
    let source = refspec
        .trim_start_matches('+')
        .split_once(':')
        .map(|(source, _)| source)
    ...
    if source.contains('*') {
        return Err(CliError::Fatal {
            code: 128,
            message: "HTTP fetch with wildcard refspecs is not supported yet".into(),
        });
    }
    if source.is_empty() {
        return Ok(None);
...
fn http_refspec_destination(refspec: &str, source_ref: &str) -> Result<Option<String>> {
    ...
    if let Some((_, destination)) = refspec.split_once(':') {
