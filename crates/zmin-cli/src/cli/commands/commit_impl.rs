...
pub(crate) fn write_tree_command_entry(prefix: Option<&str>, missing_ok: bool) -> Result<()> {
    write_tree_command(prefix, missing_ok)
}
...
fn use_hook_worktree_summary(repo: &GitRepo) -> Result<bool> {
    for hook_name in [
        "pre-commit",
        "prepare-commit-msg",
        "commit-msg",
        ...
        if commit_hook_path(repo, hook_name)?.is_some() {
            return Ok(true);
        }
    }
    Ok(false)
...
#[cfg(windows)]
fn post_commit_summary_new_source(use_hook_worktree_summary: bool) -> DiffSideSource {
    if use_hook_worktree_summary {
...
#[cfg(not(windows))]
fn post_commit_summary_new_source(_use_hook_worktree_summary: bool) -> DiffSideSource {
    DiffSideSource::Index
}
...
fn print_commit_summary(
    ...
    summary_new_source: DiffSideSource,
) -> Result<()> {
...
fn write_commit_verbose_status<W: Write>(
    ...
    staged_entries: &[zmin_git_core::IndexDiffEntry],
    worktree_entries: &[zmin_git_core::IndexDiffEntry],
...
fn worktree_commit_diff_entries(
    ...
    let mut entries = worktree_status(repo, index)?
        .into_iter()
        .map(|(path, status)| zmin_git_core::IndexDiffEntry {
            status: match status {
...
fn write_tree_prefix_index(index: &GitIndex, prefix: &str) -> Result<GitIndex> {
    ...
    let entries = index
        ...
        .iter()
        .filter_map(|entry| {
            let rest = entry.path.strip_prefix(prefix.as_slice())?;
            if rest.is_empty() {
                return None;
            }
            let mut entry = entry.clone();
            entry.path = rest.to_vec();
            Some(entry)
        })
        .collect::<Vec<_>>();
