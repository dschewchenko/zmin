...
fn validate_maintenance_schedule(schedule: Option<&str>, auto: bool) -> Result<()> {
    ...
    if auto {
        return Err(CliError::Fatal {
            code: 128,
            message: auto_schedule_failure_message().into(),
        });
    }
    Ok(())
}
...
#[cfg(windows)]
fn auto_schedule_failure_message() -> &'static str {
    "options '--auto' and '--schedule=' cannot be used together"
}
...
fn maintenance_repo_path(repo: &GitRepo) -> Result<String> {
    Ok(git_path_output(&fs::canonicalize(&repo.root)?))
}

fn git_path_output(path: &Path) -> String {
...
fn windows_test_unix_scheduler_home_dir() -> PathBuf {
    PathBuf::from("C:/Users/zmin-test")
}
...
mod maintenance_scheduler_tests {
    ...
    fn crontab_region_contains_all_frequencies() {
        let region = maintenance_crontab_region(Path::new("/tmp/zmin git"), 19);
        assert!(region.contains("# BEGIN GIT MAINTENANCE SCHEDULE"));
        assert!(region.contains("19 1-23 * * * '/tmp/zmin git' for-each-repo --keep-going --config=maintenance.repo maintenance run --schedule=hourly"));
        assert!(region.contains("19 0 * * 1-6 '/tmp/zmin git' for-each-repo --keep-going --config=maintenance.repo maintenance run --schedule=daily"));
        assert!(region.contains("19 0 * * 0 '/tmp/zmin git' for-each-repo --keep-going --config=maintenance.repo maintenance run --schedule=weekly"));
    }

    #[test]
    fn crontab_replace_region_overwrites_existing_git_block() {
        let original = "MAILTO=dev@example.test\n\n# BEGIN GIT MAINTENANCE SCHEDULE\nold\n# END GIT MAINTENANCE SCHEDULE\n";
        let replacement = maintenance_crontab_region(Path::new("/tmp/zmin"), 7);
        let updated = maintenance_crontab_replace_region(original, &replacement);
        assert!(
            updated.starts_with("MAILTO=dev@example.test\n\n# BEGIN GIT MAINTENANCE SCHEDULE\n")
        );
        assert!(!updated.contains("\nold\n"));
        assert!(updated.contains("7 1-23 * * * '/tmp/zmin'"));
    }
    ...
    fn systemd_service_template_uses_schedule_instance_and_security_flags() {
        let unit = maintenance_systemd_service_unit(Path::new("/tmp/zmin git"));
        assert!(unit.contains("ExecStart=\"/tmp/zmin git\" for-each-repo --keep-going --config=maintenance.repo maintenance run --schedule=%i"));
    ...
    fn systemd_service_template_escapes_executable_specifiers() {
        let unit = maintenance_systemd_service_unit(Path::new("/tmp/zmin %i \"git\""));
        assert!(unit.contains("ExecStart=\"/tmp/zmin %%i \\\"git\\\"\" for-each-repo --keep-going --config=maintenance.repo maintenance run --schedule=%i"));
        assert!(!unit.contains("ExecStart=\"/tmp/zmin %i \"git\"\""));
    }
    ...
    fn launchctl_plist_escapes_executable_path() {
        let plist = maintenance_launchctl_plist_for_executable(
            Path::new(r#"/tmp/Zmin & Co/<bin>"quoted"/zmin"#),
            "weekly",
            19,
        )
