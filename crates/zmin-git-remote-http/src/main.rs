...
impl Drop for PhaseTrace {
    fn drop(&mut self) {
        let Some(path) = std::env::var_os("ZMIN_REMOTE_HTTP_PHASE_TRACE_FILE") else {
            return;
        };
        ...
        let _ = writeln!(
            file,
            "zmin-remote-http-phase\t{}\tseconds={:.6}",
            self.name,
...
fn phase_trace(name: &'static str) -> Option<PhaseTrace> {
    std::env::var_os("ZMIN_REMOTE_HTTP_PHASE_TRACE_FILE").map(|_| PhaseTrace {
        name,
        start: Instant::now(),
    })
}
...
fn trace_event(message: impl std::fmt::Display) {
    ...
    let _ = writeln!(file, "zmin-remote-http-event\t{message}");
}

#[derive(Debug, Parser)]
#[command(
    name = "zmin-git-remote-http",
...
fn temp_response_file() -> io::Result<tempfile::NamedTempFile> {
    tempfile::Builder::new()
        .prefix("zmin-git-remote-http-response-")
        .suffix(".body")
        .tempfile()
}
...
fn temp_output_file(output_file: &Path) -> io::Result<tempfile::NamedTempFile> {
    ...
    tempfile::Builder::new()
        .prefix(".zmin-git-remote-http-output-")
        .tempfile_in(parent)
}
...
fn write_plain_http1_request_head(
    ...
    if !has_user_agent {
        head.extend_from_slice(b"\r\nUser-Agent: zmin-git-remote-http/");
...
fn plain_http1_default_user_agent_len(has_user_agent: bool) -> usize {
    if has_user_agent {
        ...
        "\r\nUser-Agent: zmin-git-remote-http/".len() + env!("CARGO_PKG_VERSION").len()
    }
}
...
fn auto_http3_failure_cache_file_name(origin: &Http3Origin) -> String {
    const PREFIX: &str = "zmin-http3-failed-";
...
mod tests {
    ...
    fn batch_request_reader_rejects_too_many_headers() {
        ...
        for index in 0..=BATCH_REQUEST_HEADER_LIMIT {
            bytes.extend_from_slice(format!("HEADER X-Zmin-{index}: value\n").as_bytes());
        }
        bytes.extend_from_slice(b"\n");
        let mut reader = io::Cursor::new(bytes);
        let mut line = String::new();
    ...
    fn response_header_collection_is_bounded() {
        ...
        for index in 0..(COLLECTED_RESPONSE_HEADER_LIMIT + 8) {
            let name = http::HeaderName::from_bytes(format!("x-zmin-{index}").as_bytes()).unwrap();
    ...
    fn plain_http1_request_head_does_not_duplicate_user_agent_header() {
        ...
        let request = TransportRequest {
            method: http::Method::GET,
            url: url.to_string(),
            headers: vec![parse_request_header("User-Agent: zmin-test/1").unwrap()],
            body: RequestBody::Empty,
            output_file: None,
        };
    ...
    fn auto_http3_failure_cache_file_name_encodes_host_without_intermediate_string() {
        ...
        assert_eq!(
            ...
            "zmin-http3-failed-617a415a30392d2e-443.cache"
        );
    }
