...
pub fn git_init_sha256() -> TempDir {
    let repo = TempDir::new().expect("temp repo");
    let output = Command::new(stock_git_bin())
        .arg("init")
        .arg("--quiet")
        ...
        .current_dir(repo.path())
        .output()
        .expect("run git init sha256");
    assert!(
        output.status.success(),
        "git init sha256 failed: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    repo
}

pub fn git_init_reftable() -> TempDir {
    let repo = TempDir::new().expect("temp repo");
    let output = Command::new(stock_git_bin())
        .arg("init")
        .arg("--quiet")
...
pub fn git_env(repo: &TempDir, args: &[&str]) {
    let output = git_env_output(repo, args);
    assert!(
        output.status.success(),
        "git failed: {}",
        String::from_utf8_lossy(&output.stderr)
...
pub fn git_expect_failure(repo: &TempDir, args: &[&str]) -> String {
    let output = git_output(repo, args);
    assert!(
        !output.status.success(),
...
pub fn git_with_stdin(repo: &TempDir, args: &[&str], stdin: &[u8]) -> String {
...
pub fn git_raw_with_stdin(repo: &TempDir, args: &[&str], stdin: &[u8]) -> Vec<u8> {
    let output = git_output_with_stdin_at(repo.path(), args, stdin);
    assert!(
        output.status.success(),
        "git failed: {}",
        String::from_utf8_lossy(&output.stderr)
...
pub fn git_output_with_stdin_at(cwd: &Path, args: &[&str], stdin: &[u8]) -> Output {
...
pub fn git_hash_object(kind: &str, content: &[u8]) -> String {
    use std::io::Write as _;
...
pub fn git_hash_object_write(repo: &TempDir, content: &[u8]) -> String {
    git_with_stdin(repo, &["hash-object", "-w", "--stdin"], content)
}
...
fn git_env_output(repo: &TempDir, args: &[&str]) -> Output {
    Command::new(stock_git_bin())
        .args(["-c", "commit.gpgsign=false"])
        .args(args)
        .current_dir(repo.path())
