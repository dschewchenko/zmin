pub const STRINGS: &[(&str, &str)] = &[
    (
        "login_connecting",
        "Verbindung zum Skron-Authentifizierungsdienst unter {url} wird hergestellt...",
    ),
    (
    ...
    (
        ...
        "Keine lokale Identität für {email} auf {server} gefunden; führe `skron login` aus.",
    ),
    (
        "publish_no_changes",
    ...
    (
        ...
        "Es wurde nichts übernommen. Nutze `skron stage <Datei>` und starte `skron publish` erneut.",
    ),
    (
    ...
    (
        ...
        "Starte `skron publish`, sobald du mit der Liste zufrieden bist.",
    ),
    (
        "sync_checking",
