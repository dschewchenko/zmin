...
#[cfg(feature = "cli")]
use crate::error::{Error, Result as SkronResult};
...
pub fn configure_blocking_client_builder(
    ...
    if std::env::var("SKRON_ACCEPT_INVALID_CERTS").is_ok() {
...
#[cfg(feature = "cli")]
pub fn parse_json_response<T>(response: Response, context: &str) -> SkronResult<T>
where
    T: DeserializeOwned,
{
...
#[cfg(feature = "cli")]
fn load_pinned_ca() -> SkronResult<Option<Certificate>> {
    let pem_source = if let Ok(path) = env::var("SKRON_PINNED_CA_PEM") {
        Some(fs::read(&path).map_err(|err| Error::Config {
            details: format!("load SKRON_PINNED_CA_PEM {path}: {err}"),
        })?)
    } else if let Some(inline) = option_env!("SKRON_PINNED_CA_PEM_INLINE") {
    ...
    let target_fingerprint = env::var("SKRON_PINNED_CA_SHA3_256")
        .ok()
        .map(|v| v.trim().to_owned())
        .or_else(|| option_env!("SKRON_PINNED_CA_SHA3_256").map(|v| v.trim().to_owned()))
        .or_else(|| {
    ...
    if let Some(expected) = target_fingerprint {
        ...
        println!(
            "warning: SKRON_PINNED_CA_PEM provided without SKRON_PINNED_CA_SHA3_256; pinning disabled"
        );
    }
...
#[cfg(feature = "cli")]
fn verify_certificate_fingerprint(der: &[u8], expected: &str) -> SkronResult<()> {
...
#[cfg(feature = "cli")]
fn load_mkcert_root() -> SkronResult<Option<Certificate>> {
...
fn detect_mkcert_caroot() -> Option<PathBuf> {
    if let Ok(path) = env::var("SKRON_MKCERT_CAROOT") {
...
#[cfg(feature = "cli")]
fn pem_to_der(pem: &[u8]) -> SkronResult<Vec<u8>> {
