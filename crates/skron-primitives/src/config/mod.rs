...
fn resolve_default_server_url() -> String {
    if let Ok(url) = env::var("SKRON_SERVER") {
    ...
    if let Some(url) = option_env!("SKRON_SERVER_DEFAULT") {
        let trimmed = url.trim();
        if !trimmed.is_empty() {
            return trimmed.to_owned();
        }
...
impl RuntimeProfile {
    pub fn default_data_dir(&self) -> Result<PathBuf> {
        match &self.kind {
            ...
            RuntimeKind::Wasm(_) => Ok(PathBuf::from("/skron/data")),
        }
    }
    ...
    pub fn default_cache_dir(&self, _data_dir: &Path) -> Result<PathBuf> {
        match &self.kind {
            ...
            RuntimeKind::Wasm(_) => Ok(PathBuf::from("/skron/data/cache")),
        }
    }
...
impl RuntimeEnv {
    pub fn from_env() -> Self {
        match std::env::var("SKRON_ENV").as_deref() {
...
impl CliProfile {
    pub fn detect() -> Result<Self> {
        ...
        let environment_scope =
            std::env::var("SKRON_PROFILE").unwrap_or_else(|_| "default".to_owned());
        Ok(Self {
            workspace_root,
            environment_scope,
        })
...
fn default_cli_root() -> Result<PathBuf> {
    if let Ok(custom) = env::var("SKRON_DATA_DIR") {
    ...
    if let Some(dir) = dirs::data_dir() {
        return Ok(dir.join("skron"));
    }
    if let Some(home) = dirs::home_dir() {
        return Ok(home.join(".skron"));
    }
    Err(Error::Config {
        details: "unable to determine data directory; set SKRON_DATA_DIR".into(),
    })
}
