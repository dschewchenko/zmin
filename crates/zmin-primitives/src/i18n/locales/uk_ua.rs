pub const STRINGS: &[(&str, &str)] = &[
    (
        "login_connecting",
        "Підключення до служби автентифікації Zmin за адресою {url}...",
    ),
    (
    ...
    (
        ...
        "Локальну ідентичність для {email} на {server} не знайдено; запустіть `zmin login`.",
    ),
    ...
    (
        ...
        "Зміни не додано. Використайте `zmin stage <файл>` і повторіть `zmin publish`.",
    ),
    (
    ...
    (
        ...
        "Коли все готово, запустіть `zmin publish`.",
    ),
    (
        "sync_checking",
