pub const STRINGS: &[(&str, &str)] = &[
    (
        "login_connecting",
        "Conectando con el servicio de autenticación de Zmin en {url}...",
    ),
    (
    ...
    (
        ...
        "No se encontró una identidad local para {email} en {server}; ejecuta `zmin login`.",
    ),
    (
        "publish_no_changes",
    ...
    (
        ...
        "No se añadió nada. Usa `zmin stage <archivo>` y vuelve a ejecutar `zmin publish`.",
    ),
    (
    ...
    (
        ...
        "Cuando esté listo, ejecuta `zmin publish`.",
    ),
    (
        "sync_checking",
