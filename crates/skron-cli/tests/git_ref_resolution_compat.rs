...
use common::{
    ...
    git_with_env, git_with_stdin, run_skron_args, run_skron_status_args, run_skron_with_stdin,
...
fn name_rev_matches_stock_git_for_refs_tags_and_stdin_annotation() {
    ...
    assert_eq!(
        run_skron_with_stdin(repo.path(), ["name-rev", "--annotate-stdin"], &input),
...
fn replace_matches_stock_git_for_list_create_delete() {
    ...
    assert_eq!(git(skron_repo.path(), ["rev-parse", "HEAD~1"]), one);
    assert_eq!(git(skron_repo.path(), ["rev-parse", "HEAD"]), two);

    git_args(git_repo.path(), &["replace", &two, &one]);
    run_skron_args(skron_repo.path(), &["replace", &two, &one]);
    assert_eq!(
        fs::read_to_string(git_repo.path().join(".git/refs/replace").join(&two))
            .expect("read git replace ref"),
        fs::read_to_string(skron_repo.path().join(".git/refs/replace").join(&two))
            .expect("read skron replace ref")
    );
    ...
    assert_eq!(
        run_skron_args(skron_repo.path(), &["replace", "-d", &two[..12]]),
        git_args(git_repo.path(), &["replace", "-d", &two[..12]])
    );
    assert_eq!(
        run_skron_args(skron_repo.path(), &["replace"]),
...
fn replace_graft_matches_stock_git_for_reparenting_commit() {
    ...
    assert_eq!(git(skron_repo.path(), ["rev-parse", "HEAD~1"]), two);
    assert_eq!(git(skron_repo.path(), ["rev-parse", "HEAD~2"]), one);

    git_args(git_repo.path(), &["replace", "--graft", &two]);
    run_skron_args(skron_repo.path(), &["replace", "--graft", &two]);
    assert_eq!(
        git_args(git_repo.path(), &["replace", "--format=long"]),
        run_skron_args(skron_repo.path(), &["replace", "--format=long"])
    );
    let git_replacement = fs::read_to_string(git_repo.path().join(".git/refs/replace").join(&two))
        .expect("git replacement");
    let skron_replacement =
        fs::read_to_string(skron_repo.path().join(".git/refs/replace").join(&two))
            .expect("skron replacement");
    assert_eq!(git_replacement, skron_replacement);
    assert_eq!(
        git_args(git_repo.path(), &["cat-file", "-p", git_replacement.trim()]),
        git_args(
            skron_repo.path(),
            &["cat-file", "-p", skron_replacement.trim()],
        )
    );

    assert_eq!(
        run_skron_status_args(skron_repo.path(), &["replace", "--graft", &two, &one]),
...
fn replace_edit_matches_stock_git_for_commit_message_edit() {
    ...
    let skron_oid = git(skron_repo.path(), ["rev-parse", "HEAD"]);

    let git_editor = git_repo.path().join("replace-edit.sh");
    let skron_editor = skron_repo.path().join("replace-edit.sh");
    ...
    #[cfg(unix)]
    {
        ...
        fs::set_permissions(&skron_editor, fs::Permissions::from_mode(0o755))
            .expect("chmod skron editor");
    }
    ...
    let git_replacement =
        fs::read_to_string(git_repo.path().join(".git/refs/replace").join(&git_oid))
            .expect("git replacement");
    let skron_replacement =
        fs::read_to_string(skron_repo.path().join(".git/refs/replace").join(&skron_oid))
            .expect("skron replacement");
    assert_eq!(
        git_args(git_repo.path(), &["cat-file", "-p", git_replacement.trim()]),
        git_args(
...
fn replace_convert_graft_file_matches_stock_git_for_reparenting_entries() {
    ...
    let skron_two = git(skron_repo.path(), ["rev-parse", "main"]);
    let skron_side = git(skron_repo.path(), ["rev-parse", "side"]);
    fs::create_dir_all(git_repo.path().join(".git/info")).expect("git info dir");
    fs::create_dir_all(skron_repo.path().join(".git/info")).expect("skron info dir");
    fs::write(
    ...
    fs::write(
        skron_repo.path().join(".git/info/grafts"),
        format!("{skron_two} {skron_side}\n"),
    )
    .expect("write skron grafts");
    ...
    assert!(!skron_repo.path().join(".git/info/grafts").exists());
    let git_replacement =
        fs::read_to_string(git_repo.path().join(".git/refs/replace").join(&git_two))
            .expect("git replacement");
    let skron_replacement =
        fs::read_to_string(skron_repo.path().join(".git/refs/replace").join(&skron_two))
            .expect("skron replacement");
    assert_eq!(
        git_args(git_repo.path(), &["cat-file", "-p", git_replacement.trim()]),
        git_args(
