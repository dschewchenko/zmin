...
use common::{
    command_stdout_bytes, git_status_with_stdin, git_with_stdin, run_skron_status_with_stdin,
...
fn credential_matches_stock_git_for_basic_protocol_flows() {
    ...
    assert_eq!(
        run_skron_with_stdin(repo.path(), ["credential", "fill"], complete),
        git_with_stdin(repo.path(), ["credential", "fill"], complete)
    );
    assert_eq!(
        run_skron_with_stdin(repo.path(), ["credential", "approve"], complete),
        git_with_stdin(repo.path(), ["credential", "approve"], complete)
    );
    assert_eq!(
        run_skron_with_stdin(repo.path(), ["credential", "reject"], complete),
        git_with_stdin(repo.path(), ["credential", "reject"], complete)
    );
    let missing = "protocol=https\nhost=example.com\n\n";
    assert_eq!(
        run_skron_status_with_stdin(repo.path(), ["credential", "fill"], missing),
...
fn credential_store_matches_stock_git_for_store_get_and_erase() {
    ...
    assert_eq!(
        fs::read_to_string(skron_home.path().join(".git-credentials"))
            .expect("read skron credentials"),
    ...
    assert_eq!(
        fs::read_to_string(skron_home.path().join(".git-credentials"))
            .expect("read skron credentials after erase"),
...
fn credential_cache_matches_stock_git_for_store_get_and_erase() {
    ...
    let skron_socket = dir.path().join("skron.sock");
    let git_socket_arg = format!("--socket={}", git_socket.display());
    let skron_socket_arg = format!("--socket={}", skron_socket.display());
    let complete = "protocol=https\nhost=example.com\nusername=u\npassword=p\n\n";
    let query = "protocol=https\nhost=example.com\n\n";
    let erase = "protocol=https\nhost=example.com\nusername=u\n\n";
    ...
    assert_eq!(
        command_with_stdin(
            skron_bin(),
            dir.path(),
            &["credential-cache", skron_socket_arg.as_str(), "store"],
            complete,
            "skron credential-cache store",
        ),
        command_with_stdin(
            "git",
            dir.path(),
    ...
    assert_eq!(
        command_with_stdin(
            skron_bin(),
            dir.path(),
            &["credential-cache", skron_socket_arg.as_str(), "get"],
            query,
            "skron credential-cache get",
        ),
        command_with_stdin(
            "git",
            dir.path(),
    ...
    assert_eq!(
        command_with_stdin(
            skron_bin(),
            dir.path(),
            &["credential-cache", skron_socket_arg.as_str(), "erase"],
            erase,
            "skron credential-cache erase",
        ),
        command_with_stdin(
            "git",
            dir.path(),
    ...
    assert_eq!(
        command_with_stdin(
            ...
            &["credential-cache", skron_socket_arg.as_str(), "get"],
            query,
            "skron credential-cache get after erase",
        ),
        command_with_stdin(
            "git",
            dir.path(),
    ...
    command_stdout_bytes(
        skron_bin(),
        dir.path(),
        &["credential-cache", skron_socket_arg.as_str(), "exit"],
    );
    command_stdout_bytes(
        "git",
        dir.path(),
