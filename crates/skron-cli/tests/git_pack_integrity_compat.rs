...
use common::{
    ...
    git_with_stdin, git_with_stdin_args, git_with_stdin_bytes, run_skron, run_skron_args,
    run_skron_status, run_skron_with_env, run_skron_with_stdin_args, run_skron_with_stdin_bytes,
...
fn fsck_matches_stock_git_for_healthy_repo_and_detects_corrupt_object() {
    ...
    assert_eq!(run_skron(repo.path(), ["fsck"]), git(repo.path(), ["fsck"]));
    assert_eq!(
        run_skron(repo.path(), ["fsck", "--name-objects"]),
    ...
    assert_eq!(
        run_skron_status(repo.path(), ["fsck"]),
...
fn fsck_missing_email_severity_config_matches_stock_git() {
    ...
    assert_eq!(
        command_any_output(skron_bin(), repo.path(), &["fsck"], "skron"),
...
fn fsck_lost_found_writes_dangling_objects_like_stock_git() {
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["fsck", "--lost-found"]),
        git(git_repo.path(), ["fsck", "--lost-found"])
    );
    assert_eq!(
        fs::read(skron_repo.path().join(".git/lost-found/other").join(&blob)).expect("skron blob"),
...
fn verify_pack_matches_stock_git_for_default_and_stats() {
    ...
    assert_eq!(
        run_skron(repo.path(), ["verify-pack", idx]),
        git(repo.path(), ["verify-pack", idx])
    );
    assert_eq!(
        run_skron(repo.path(), ["verify-pack", "-v", idx]),
        git(repo.path(), ["verify-pack", "-v", idx])
    );
    assert_eq!(
        run_skron(repo.path(), ["verify-pack", "-s", idx]),
    ...
    let skron_corrupt = command_any_output(
        skron_bin(),
        corrupt_repo.path(),
    ...
    assert_eq!(skron_corrupt.0, git_corrupt.0);
    assert_ne!(skron_corrupt.0, 0);
...
fn verify_commit_and_tag_match_stock_git_for_unsigned_and_signed_fixtures() {
    ...
    assert_eq!(
        run_skron_status(repo.path(), ["verify-commit", "HEAD"]),
        git_status(repo.path(), ["verify-commit", "HEAD"])
    );
    assert_eq!(
        run_skron_status(repo.path(), ["verify-tag", "v1"]),
        git_status(repo.path(), ["verify-tag", "v1"])
    );
    assert_eq!(
        run_skron_status(repo.path(), ["verify-tag", "lightweight"]),
    ...
    let skron_bad = command_any_output(
        skron_bin(),
        repo.path(),
        &["verify-tag", "--raw", "signed-fixture"],
        "skron",
    ...
    assert_eq!(skron_bad, git_bad);
    assert_ne!(skron_bad.0, 0);
    ...
    let skron_bad_commit = command_any_output(
        skron_bin(),
        repo.path(),
        &["verify-commit", "--raw", "signed-commit-fixture"],
        "skron",
    ...
    assert_eq!(skron_bad_commit, git_bad_commit);
    assert_ne!(skron_bad_commit.0, 0);
}
...
fn unpack_objects_writes_pack_objects_readable_by_stock_git() {
    ...
    let target = git_init();
    run_skron_with_stdin_bytes(target.path(), ["unpack-objects", "-q"], &pack);
...
fn unpack_objects_strict_accepts_stock_git_pack() {
    ...
    let skron_target = git_init();
    git_with_stdin_bytes(
        git_target.path(),
        ["unpack-objects", "--strict", "-q"],
        &pack,
    );
    run_skron_with_stdin_bytes(
        skron_target.path(),
        ["unpack-objects", "--strict", "-q"],
        &pack,
    );
    ...
    for line in objects.lines() {
        let id = line.split_whitespace().next().expect("object id");
        assert_eq!(
            git(skron_target.path(), ["cat-file", "-t", id]),
            git(git_target.path(), ["cat-file", "-t", id]),
            "object type for {id}"
        );
        assert_eq!(
            command_stdout_bytes("git", skron_target.path(), &["cat-file", "-p", id]),
...
fn pack_objects_window_depth_writes_stock_readable_delta_pack() {
    ...
    let pack_id = run_skron_with_stdin_args(
        source.path(),
        &[
            "pack-objects",
            "--window=10",
            "--depth=10",
            ".git/objects/pack/pack-skron",
        ],
        &pack_input,
    );
    let idx = source
        ...
        .join(format!("pack-skron-{pack_id}.idx"));
...
fn pack_objects_base_name_writes_stock_readable_pack_and_index() {
    ...
    let pack_id = run_skron_with_stdin_args(
        source.path(),
        &["pack-objects", ".git/objects/pack/pack-skron"],
        &objects,
    );
    let pack_path = source
        .path()
        .join(format!(".git/objects/pack/pack-skron-{pack_id}.pack"));
    let index_path = source
        .path()
        .join(format!(".git/objects/pack/pack-skron-{pack_id}.idx"));
    ...
    git_with_stdin_bytes(
        target.path(),
        ["unpack-objects", "-q"],
        &fs::read(pack_path).expect("read skron pack"),
    );
    for line in objects.lines() {
        let id = line.split_whitespace().next().expect("object id");
        assert_eq!(
...
fn unpack_objects_recover_accepts_valid_stock_git_pack() {
    ...
    run_skron_with_stdin_bytes(skron_target.path(), ["unpack-objects", "-r", "-q"], &pack);
...
fn index_pack_stdin_writes_stock_readable_pack_index() {
    ...
    let skron_output = run_skron_with_stdin_bytes(
        skron_repo.path(),
        ["index-pack", "--stdin", "--no-rev-index"],
        &pack,
    );
    ...
    assert_eq!(
        fs::read(skron_repo.path().join(".git/objects/pack").join(&idx_name))
            .expect("read skron idx"),
    ...
    let skron_v1_output = run_skron_with_stdin_bytes(
        skron_v1_repo.path(),
        [
            "index-pack",
            "--stdin",
    ...
    assert_eq!(skron_v1_output, git_v1_output);
    ...
    let skron_rev_repo = git_init();
    let git_rev_output =
        git_with_stdin_bytes(git_rev_repo.path(), ["index-pack", "--stdin"], &pack);
    let skron_rev_output =
        run_skron_with_stdin_bytes(skron_rev_repo.path(), ["index-pack", "--stdin"], &pack);
    assert_eq!(skron_rev_output, git_rev_output);
    ...
    let skron_explicit_rev_repo = git_init();
    let git_explicit_rev_output = git_with_stdin_bytes(
        git_explicit_rev_repo.path(),
        ["index-pack", "--stdin", "--rev-index"],
        &pack,
    );
    let skron_explicit_rev_output = run_skron_with_stdin_bytes(
        skron_explicit_rev_repo.path(),
        ["index-pack", "--stdin", "--rev-index"],
        &pack,
    );
    assert_eq!(skron_explicit_rev_output, git_explicit_rev_output);
    ...
    assert_eq!(
        fs::read(
            skron_explicit_rev_repo
                .path()
                .join(".git/objects/pack")
                .join(&explicit_rev_name)
        )
        .expect("read skron explicit rev"),
        fs::read(
    ...
    let skron_explicit_pack = skron_explicit_rev_repo
        .path()
        .join(".git/objects/pack")
        .join(format!("pack-{explicit_rev_pack_id}.pack"));
    flip_last_byte(&skron_explicit_pack.with_extension("rev"));
    assert_eq!(
        command_any_output(
            skron_bin(),
            skron_explicit_rev_repo.path(),
            &[
                "index-pack",
                "--verify",
                skron_explicit_pack.to_str().expect("skron pack path")
            ],
            "skron",
        ),
        command_any_output(
    ...
    let skron_corrupt_idx_repo = git_init();
    let git_corrupt_idx_output = git_with_stdin_bytes(
        git_corrupt_idx_repo.path(),
        ["index-pack", "--stdin"],
        &pack,
    );
    let skron_corrupt_idx_output = run_skron_with_stdin_bytes(
        skron_corrupt_idx_repo.path(),
        ["index-pack", "--stdin"],
        &pack,
    );
    assert_eq!(skron_corrupt_idx_output, git_corrupt_idx_output);
    let corrupt_idx_pack_id = git_corrupt_idx_output
        .strip_prefix("pack\t")
        .expect("index-pack corrupt idx output pack id");
    let skron_corrupt_idx_pack = skron_corrupt_idx_repo
        .path()
        .join(".git/objects/pack")
        .join(format!("pack-{corrupt_idx_pack_id}.pack"));
    flip_last_byte(&skron_corrupt_idx_pack.with_extension("idx"));
    assert_eq!(
        command_any_output(
            skron_bin(),
            skron_corrupt_idx_repo.path(),
            &[
                "index-pack",
                "--verify",
                skron_corrupt_idx_pack
                    .to_str()
                    .expect("skron corrupt idx pack path")
            ],
            "skron",
        ),
        command_any_output(
...
fn index_pack_version_1_matches_stock_git_output_and_writes_v1_index() {
    ...
    let pack_id = git_output
        .strip_prefix("keep\t")
        .expect("index-pack keep output");
    let skron_idx = skron_repo
        .path()
        .join(format!(".git/objects/pack/pack-{pack_id}.idx"));
    let git_idx = git_repo
        .path()
        .join(format!(".git/objects/pack/pack-{pack_id}.idx"));
    let skron_bytes = fs::read(&skron_idx).expect("read skron v1 idx");
    let git_bytes = fs::read(&git_idx).expect("read git v1 idx");
    assert_eq!(skron_bytes, git_bytes);
    assert_eq!(&skron_bytes[..4], &[0, 0, 0, 0]);
    assert_eq!(git_status(skron_repo.path(), ["fsck", "--strict"]), 0);
}
...
fn pack_objects_index_version_1_writes_stock_compatible_v1_index() {
    ...
    let skron_prefix = skron_repo.path().join("out/pack");
    fs::create_dir_all(git_prefix.parent().expect("git parent")).expect("mkdir git out");
    fs::create_dir_all(skron_prefix.parent().expect("skron parent")).expect("mkdir skron out");
    ...
    let skron_output = run_skron_with_stdin_args(
        skron_repo.path(),
        &[
            "pack-objects",
            skron_prefix.to_str().expect("skron prefix"),
            "--revs",
            "--index-version=1",
        ],
        "HEAD\n",
    ...
    let git_idx = fs::read_dir(git_repo.path().join("out"))
        ...
        .expect("git idx path");
    let skron_idx = fs::read_dir(skron_repo.path().join("out"))
        .expect("read skron out dir")
        .find_map(|entry| {
            let path = entry.expect("skron out entry").path();
            (path.extension().and_then(|value| value.to_str()) == Some("idx")).then_some(path)
        })
        .expect("skron idx path");
    let skron_pack = skron_idx.with_extension("pack");
    assert_eq!(
        &fs::read(&git_idx).expect("read git idx")[..4],
        &[0, 0, 0, 0]
    );
    assert_eq!(
        &fs::read(&skron_idx).expect("read skron idx")[..4],
        &[0, 0, 0, 0]
    );
    assert_eq!(
        command_any_output(
            ...
            &[
                "verify-pack",
                "-v",
                skron_idx.to_str().expect("skron idx path")
            ],
            "git",
        )
        .0,
    ...
    git_with_stdin_bytes(
        ...
        &fs::read(&skron_pack).expect("read skron pack"),
    );
    for line in git(source.path(), ["rev-list", "--objects", "HEAD"]).lines() {
        let id = line.split_whitespace().next().expect("object id");
        assert_eq!(
...
fn index_pack_strict_rejects_malformed_commit_like_stock_git() {
    ...
    for args in [
        ...
        let skron_pack = skron_repo.path().join("bad.pack");
        fs::write(&git_pack, &pack).expect("write git malformed pack");
        fs::write(&skron_pack, &pack).expect("write skron malformed pack");

        let mut git_args = args.to_vec();
        let mut skron_args = args.to_vec();
        git_args.push(git_pack.to_str().expect("git pack path"));
        skron_args.push(skron_pack.to_str().expect("skron pack path"));
        assert_eq!(
            command_any_output(skron_bin(), skron_repo.path(), &skron_args, "skron"),
...
fn index_pack_fix_thin_repairs_stock_git_thin_pack() {
    ...
    git_args(skron_target.path(), &["fetch", source_path, &fetch_ref]);
    ...
    assert!(skron_output.starts_with("pack\t"));
    let skron_pack_id = skron_output
        .strip_prefix("pack\t")
        .expect("skron index-pack output pack id");

    let skron_idx = skron_target
        .path()
        .join(".git/objects/pack")
        .join(format!("pack-{skron_pack_id}.idx"));
    assert_eq!(
        command_any_output(
            "git",
            skron_target.path(),
            &["verify-pack", skron_idx.to_str().expect("idx path")],
            "git"
        )
        .0,
        0
...
fn bundle_create_list_heads_and_unbundle_are_stock_readable() {
    ...
    assert_eq!(
        run_skron_args(source.path(), &["bundle", "list-heads", bundle]),
    ...
    assert_eq!(
        ...
        0,
        "stock git should verify a skron-created bundle"
    );

    let target = git_init();
    assert_eq!(
        run_skron_args(target.path(), &["bundle", "unbundle", bundle]),
        git_args(source.path(), &["bundle", "list-heads", bundle])
    );
...
fn bundle_unbundle_accepts_prerequisite_bundles() {
    ...
    let skron_target = clone_repo_fixture(source.path());
    ...
    assert_eq!(
        run_skron_args(skron_target.path(), &["bundle", "unbundle", bundle]),
        git_args(git_target.path(), &["bundle", "unbundle", bundle])
    );
    assert_eq!(
        git(skron_target.path(), ["cat-file", "-p", &head]),
        git(source.path(), ["cat-file", "-p", &head])
    );

    let missing_base = git_init();
    let skron_missing = command_any_output(
        skron_bin(),
        missing_base.path(),
    ...
    assert_eq!(skron_missing.0, git_missing.0);
    assert_ne!(skron_missing.0, 0);
}
