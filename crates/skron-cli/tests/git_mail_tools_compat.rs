...
use common::{
    ...
    read_named_files, run_skron, run_skron_args, run_skron_with_stdin, run_skron_with_stdin_args,
    write_file,
};
...
fn interpret_trailers_matches_stock_git_for_common_modes() {
    ...
    for args in [
        ...
        assert_eq!(
            run_skron_with_stdin_args(repo.path(), args, fixture),
    ...
    for args in [
        ...
        assert_eq!(
            run_skron_with_stdin_args(repo.path(), args, folded),
    ...
    for args in [
        ...
        assert_eq!(
            run_skron_with_stdin_args(repo.path(), args, patch),
...
fn interpret_trailers_matches_stock_git_in_place() {
    let repo = git_init();
    fs::write(repo.path().join("skron-msg.txt"), "Subject\n\nBody\n").expect("write skron msg");
    ...
    assert_eq!(
        fs::read_to_string(repo.path().join("skron-msg.txt")).expect("read skron msg"),
...
fn mailsplit_matches_stock_git_for_mbox_and_maildir() {
    let repo = git_init();
    fs::create_dir(repo.path().join("skron-out")).expect("create skron out");
    ...
    assert_eq!(
        run_skron(
            repo.path(),
            ["mailsplit", "-d4", "-f3", "-oskron-out", "mbox"]
        ),
        git(
            repo.path(),
    ...
    assert_eq!(
        read_named_files(&repo.path().join("skron-out")),
    ...
    fs::create_dir(repo.path().join("skron-maildir-out")).expect("create skron maildir out");
    fs::create_dir(repo.path().join("git-maildir-out")).expect("create git maildir out");

    assert_eq!(
        run_skron(repo.path(), ["mailsplit", "-oskron-maildir-out", "maildir"]),
        git(repo.path(), ["mailsplit", "-ogit-maildir-out", "maildir"])
    );
    assert_eq!(
        read_named_files(&repo.path().join("skron-maildir-out")),
...
fn mailinfo_matches_stock_git_for_common_patch_mail() {
    ...
    for args in [
        ["mailinfo", "skron-msg", "skron-patch"].as_slice(),
        ["mailinfo", "-k", "skron-msg", "skron-patch"].as_slice(),
        ["mailinfo", "-b", "skron-msg", "skron-patch"].as_slice(),
        ["mailinfo", "-m", "skron-msg", "skron-patch"].as_slice(),
    ] {
        let git_args = args
            .iter()
            .map(|arg| match *arg {
                "skron-msg" => "git-msg",
                "skron-patch" => "git-patch",
                other => other,
            })
            .collect::<Vec<_>>();
        assert_eq!(
            run_skron_with_stdin_args(repo.path(), args, mail),
            git_with_stdin_args(repo.path(), &git_args, mail),
            "args: {args:?}"
        );
        assert_eq!(
            fs::read_to_string(repo.path().join("skron-msg")).expect("read skron msg"),
            fs::read_to_string(repo.path().join("git-msg")).expect("read git msg"),
            "msg args: {args:?}"
        );
        assert_eq!(
            fs::read_to_string(repo.path().join("skron-patch")).expect("read skron patch"),
...
fn fmt_merge_msg_matches_stock_git_for_fetch_head_titles() {
    ...
    for args in [
        ...
        let skron_output = if args.contains(&"-F") {
            run_skron_args(work.path(), args)
        } else {
            run_skron_with_stdin_args(work.path(), args, &fetch_head)
        };
...
fn request_pull_matches_stock_git_for_local_pushed_branch() {
    ...
    let url = remote.path().to_str().expect("remote path");

    assert_eq!(
        run_skron_args(repo.path(), &["request-pull", &start, url, "HEAD"]),
        git_args(repo.path(), &["request-pull", &start, url, "HEAD"])
    );
}
...
fn send_email_alias_modes_match_stock_git() {
    ...
    assert_eq!(
        run_skron(repo.path(), ["send-email", "--dump-aliases"]),
...
fn send_email_alias_file_types_match_stock_git() {
    for (alias_type, content) in [
        ...
        assert_eq!(
            run_skron_with_stdin(repo.path(), ["send-email", "--translate-aliases"], "dev\n"),
...
fn send_email_sends_patch_to_configured_smtp_server() {
    ...
    git(repo.path(), ["add", "-A"]);
    git_with_env(repo.path(), ["commit", "-m", "two"]);
    let patch = run_skron(repo.path(), ["format-patch", "-1"]);
...
fn imap_send_appends_mbox_messages_to_plain_imap_server() {
    ...
    run_skron_with_stdin(repo.path(), ["imap-send", "--no-curl"], mbox);
...
fn quiltimport_applies_series_like_stock_git() {
    ...
    assert_eq!(run_skron(&skron_repo, args), git(&git_repo, args));
    assert_eq!(
        git(
            &skron_repo,
    ...
    assert_eq!(
        git(&skron_repo, ["rev-parse", "HEAD^{tree}"]),
