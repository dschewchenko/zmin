...
use common::{
    ...
    git_with_env, git_with_stdin, run_skron, run_skron_failure_output, run_skron_with_stdin,
    visible_worktree_files, write_file,
};
...
fn sparse_checkout_set_list_disable_matches_stock_git_files() {
    ...
    run_skron(skron_repo.path(), ["sparse-checkout", "set", "docs"]);
    assert_eq!(
        visible_worktree_files(skron_repo.path()),
        visible_worktree_files(git_repo.path())
    );
    assert_eq!(
        git(skron_repo.path(), ["ls-files", "-t"]),
        git(git_repo.path(), ["ls-files", "-t"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["sparse-checkout", "list"]),
        "docs"
    );

    git(git_repo.path(), ["sparse-checkout", "add", "src"]);
    run_skron(skron_repo.path(), ["sparse-checkout", "add", "src"]);
    assert_eq!(
        visible_worktree_files(skron_repo.path()),
        visible_worktree_files(git_repo.path())
    );
    ...
    fs::remove_file(skron_repo.path().join("docs/guide.md")).expect("remove skron sparse file");
    fs::remove_file(git_repo.path().join("docs/guide.md")).expect("remove git sparse file");
    git(git_repo.path(), ["sparse-checkout", "reapply"]);
    run_skron(skron_repo.path(), ["sparse-checkout", "reapply"]);
    assert_eq!(
        visible_worktree_files(skron_repo.path()),
        visible_worktree_files(git_repo.path())
    );
    ...
    run_skron(skron_repo.path(), ["sparse-checkout", "disable"]);
    assert_eq!(
        visible_worktree_files(skron_repo.path()),
        visible_worktree_files(git_repo.path())
    );
...
fn sparse_checkout_pathspec_edge_cases_match_stock_git() {
    ...
    assert_eq!(
        run_skron_failure_output(skron_repo.path(), &["sparse-checkout", "set", ".."]),
        git_failure_output(git_repo.path(), &["sparse-checkout", "set", ".."])
    );

    git(git_repo.path(), ["sparse-checkout", "set", "a\\b"]);
    run_skron(skron_repo.path(), ["sparse-checkout", "set", "a\\b"]);
    assert_eq!(
        run_skron(skron_repo.path(), ["sparse-checkout", "list"]),
    ...
    assert_eq!(
        run_skron_failure_output(skron_repo.path(), &["sparse-checkout", "add", ".."]),
...
fn sparse_checkout_stdin_and_config_options_match_stock_git() {
    ...
    for key in [
        ...
        assert_eq!(
            git_config_get(skron_init_repo.path(), key),
