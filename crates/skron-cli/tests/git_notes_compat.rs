...
use common::{
    ...
    git_with_stdin, git_with_stdin_args, run_skron, run_skron_failure_output, run_skron_status,
    run_skron_with_env, run_skron_with_stdin_args, skron_bin, write_file,
};
...
fn notes_add_list_show_remove_match_stock_git() {
    ...
    let skron_repo = notes_base_repo();
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["notes", "get-ref"]),
        git(git_repo.path(), ["notes", "get-ref"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["notes", "list"]),
        git(git_repo.path(), ["notes", "list"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["notes", "show", "HEAD"]),
        git(git_repo.path(), ["notes", "show", "HEAD"])
    );

    assert_eq!(
        run_skron(skron_repo.path(), ["notes", "remove", "HEAD"]),
        git(git_repo.path(), ["notes", "remove", "HEAD"])
    );
    assert_eq!(
        run_skron_status(skron_repo.path(), ["notes", "show", "HEAD"]),
        git_status(git_repo.path(), ["notes", "show", "HEAD"])
    );

    write_file(git_repo.path(), "note.txt", "from file\nsecond\n");
    write_file(skron_repo.path(), "note.txt", "from file\nsecond\n");
    ...
    run_skron_with_env(skron_repo.path(), ["notes", "remove", "HEAD"]);
    write_file(git_repo.path(), "append-note.txt", "fresh\nfile\n");
    write_file(skron_repo.path(), "append-note.txt", "fresh\nfile\n");
    git_with_env(
        git_repo.path(),
    ...
    write_file(skron_repo.path(), "second.txt", "second\n");
    ...
    let skron_previous = git(skron_repo.path(), ["rev-parse", "HEAD~1"]);
    let git_current = git(git_repo.path(), ["rev-parse", "HEAD"]);
    let skron_current = git(skron_repo.path(), ["rev-parse", "HEAD"]);
    assert_ne!(skron_previous, skron_current);
    assert_eq!(
        run_skron(skron_repo.path(), ["notes", "list"]),
        git(git_repo.path(), ["notes", "list"])
    );
    assert!(!run_skron(skron_repo.path(), ["notes", "list"]).contains(&skron_current));
    git_with_env(
        git_repo.path(),
    ...
    run_skron_with_env(
        skron_repo.path(),
        ["notes", "copy", &skron_previous, &skron_current],
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["notes", "show", "HEAD"]),
        git(git_repo.path(), ["notes", "show", "HEAD"])
    );
    let skron_duplicate = run_skron_failure_output(
        skron_repo.path(),
        &["notes", "copy", &skron_previous, &skron_current],
    );
    ...
    assert_eq!(skron_duplicate, git_duplicate);
    assert_eq!(
        command_output(
            skron_bin(),
            skron_repo.path(),
            &["notes", "copy", "-f", &skron_previous, &skron_current],
            "skron",
        )
        .0,
        command_output(
...
fn notes_unknown_subcommand_matches_stock_git_usage() {
    ...
    assert_eq!(
        run_skron_failure_output(skron_repo.path(), &["notes", "frobnicate"]),
...
fn notes_edit_matches_stock_git_for_update_and_empty_remove() {
    ...
    run_skron_with_env(skron_repo.path(), ["notes", "add", "-m", "old", "HEAD"]);

    let git_editor = git_repo.path().join("edit-note.sh");
    let skron_editor = skron_repo.path().join("edit-note.sh");
    fs::write(&git_editor, "#!/bin/sh\nprintf 'edited\\n' > \"$1\"\n").expect("write git editor");
    fs::write(&skron_editor, "#!/bin/sh\nprintf 'edited\\n' > \"$1\"\n")
        .expect("write skron editor");
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        for editor in [&git_editor, &skron_editor] {
    ...
    assert_eq!(
        command_output_with_env(
            ...
            skron_repo.path(),
            &["notes", "edit", "HEAD"],
            &[("GIT_EDITOR", skron_editor.to_str().expect("editor path"))],
            "skron",
        ),
        command_output_with_env(
            "git",
    ...
    fs::write(&skron_editor, "#!/bin/sh\n: > \"$1\"\n").expect("write skron empty editor");
    assert_eq!(
        command_output_with_env(
            skron_bin(),
            skron_repo.path(),
...
fn notes_reuse_message_matches_stock_git_for_add_and_append() {
    ...
    command_output(
        skron_bin(),
        skron_repo.path(),
        &["notes", "add", "-m", "literal", "-C", &skron_blob, "HEAD"],
        "skron",
    );
    assert_eq!(
        command_stdout_bytes(skron_bin(), skron_repo.path(), &["notes", "show", "HEAD"]),
    ...
    command_output(
        skron_bin(),
        skron_repo.path(),
        &["notes", "append", "--reuse-message", &skron_blob, "HEAD"],
        "skron",
    );
    assert_eq!(
        command_stdout_bytes(skron_bin(), skron_repo.path(), &["notes", "show", "HEAD"]),
...
fn notes_reedit_message_matches_stock_git_for_add_and_append() {
    ...
    let skron_editor = skron_repo.path().join("reedit-note.sh");
    fs::write(&git_editor, "#!/bin/sh\nprintf 'edited\\n' > \"$1\"\n").expect("write git editor");
    fs::write(&skron_editor, "#!/bin/sh\nprintf 'edited\\n' > \"$1\"\n")
        .expect("write skron editor");
    #[cfg(unix)]
    ...
    assert_eq!(
        command_output_with_env(
            skron_bin(),
            skron_repo.path(),
            &["notes", "append", "--reedit-message", &skron_blob, "HEAD"],
            &[("GIT_EDITOR", skron_editor.to_str().expect("editor path"))],
            "skron",
        ),
        command_output_with_env(
...
fn notes_copy_stdin_matches_stock_git_for_pair_stream() {
    ...
    run_skron_with_env(skron_repo.path(), ["notes", "add", "-m", "note", "HEAD"]);

    write_file(git_repo.path(), "b.txt", "two\n");
    write_file(skron_repo.path(), "b.txt", "two\n");
    git(git_repo.path(), ["add", "-A"]);
    run_skron(skron_repo.path(), ["add", "-A"]);
    git_with_env(git_repo.path(), ["commit", "-m", "second"]);
    run_skron_with_env(skron_repo.path(), ["commit", "-m", "second"]);
    ...
    let skron_from = git(skron_repo.path(), ["rev-parse", "HEAD~1"]);
    let skron_to = git(skron_repo.path(), ["rev-parse", "HEAD"]);
    assert_eq!(skron_from, git_from);
    assert_eq!(skron_to, git_to);

    assert_eq!(
        run_skron_with_stdin_args(
            skron_repo.path(),
            &["notes", "copy", "--stdin"],
            &format!("{skron_from} {skron_to}\n"),
        ),
        git_with_stdin_args(
            git_repo.path(),
            &["notes", "copy", "--stdin"],
...
fn notes_list_show_extra_arguments_match_stock_git_failures() {
    ...
    for args in [
        ...
        let skron = run_skron_failure_output(skron_repo.path(), args);
        let git = git_failure_output(git_repo.path(), args);
        assert_eq!(skron.0, git.0, "exit status should match for {args:?}");
        assert_eq!(skron.1, git.1, "stdout should match for {args:?}");
    }
}
...
fn notes_edit_flag_matches_stock_git_for_add_and_append() {
    ...
    let skron_editor = skron_repo.path().join("edit-flag-note.sh");
    fs::write(&git_editor, "#!/bin/sh\nprintf 'edited\\n' > \"$1\"\n").expect("write git editor");
    fs::write(&skron_editor, "#!/bin/sh\nprintf 'edited\\n' > \"$1\"\n")
        .expect("write skron editor");
    #[cfg(unix)]
...
fn notes_separator_options_match_stock_git_for_message_blocks() {
    ...
    for args in [
        ...
        command_output(skron_bin(), skron_repo.path(), args, "skron");
        assert_eq!(
            command_stdout_bytes(skron_bin(), skron_repo.path(), &["notes", "show", "HEAD"]),
            command_stdout_bytes("git", git_repo.path(), &["notes", "show", "HEAD"]),
    ...
    run_skron_with_env(skron_repo.path(), ["notes", "add", "-m", "base", "HEAD"]);
    command_output(
        "git",
        git_repo.path(),
        &[
...
fn notes_stripspace_options_match_stock_git_for_text_and_reused_messages() {
    ...
    for (git_args, skron_args) in [
        ...
        (
            ...
            vec!["notes", "add", "-C", &skron_blob, "HEAD"],
        ),
        (
            vec!["notes", "add", "--stripspace", "-C", &git_blob, "HEAD"],
            vec!["notes", "add", "--stripspace", "-C", &skron_blob, "HEAD"],
        ),
    ] {
        command_output("git", git_repo.path(), &git_args, "git");
        command_output(skron_bin(), skron_repo.path(), &skron_args, "skron");
        assert_eq!(
            command_stdout_bytes(skron_bin(), skron_repo.path(), &["notes", "show", "HEAD"]),
            command_stdout_bytes("git", git_repo.path(), &["notes", "show", "HEAD"]),
...
fn notes_long_message_and_file_options_match_stock_git() {
    ...
    write_file(skron_repo.path(), "long-note.txt", "from long file\n");
    assert_eq!(
        command_output(
            skron_bin(),
            skron_repo.path(),
...
fn notes_other_no_option_toggles_match_stock_git() {
    ...
    run_skron_with_env(skron_repo.path(), ["notes", "add", "-m", "first", &first]);
    git_with_env(git_repo.path(), ["notes", "add", "-m", "second", &second]);
    run_skron_with_env(skron_repo.path(), ["notes", "add", "-m", "second", &second]);
    assert_eq!(
    ...
    let skron_merge_repo = notes_base_repo();
...
fn notes_remove_ignore_missing_and_stdin_match_stock_git() {
    ...
    assert_eq!(
        run_skron_failure_output(skron_repo.path(), &["notes", "remove", "HEAD"]),
    ...
    assert_eq!(
        ...
        run_skron_with_stdin_args(skron_repo.path(), &["notes", "remove", "--stdin"], "HEAD\n",)
    );
    assert_eq!(
        run_skron_status(skron_repo.path(), ["notes", "show", "HEAD"]),
        git_status(git_repo.path(), ["notes", "show", "HEAD"])
...
fn notes_ref_selection_matches_stock_git_env_and_explicit_refs() {
    ...
    for (envs, args) in [
        ...
        assert_eq!(
            command_output_with_env(skron_bin(), skron_repo.path(), &args, &envs, "skron"),
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["notes", "--ref=core", "show", "HEAD"]),
        git(git_repo.path(), ["notes", "--ref=core", "show", "HEAD"])
    );

    git(git_repo.path(), ["config", "core.notesRef", "core"]);
    git(skron_repo.path(), ["config", "core.notesRef", "core"]);
    assert_eq!(
        run_skron(skron_repo.path(), ["notes", "get-ref"]),
        git(git_repo.path(), ["notes", "get-ref"])
    );
    assert_eq!(
        run_skron_failure_output(skron_repo.path(), &["notes", "add", "-m", "bad", "HEAD"]),
        git_failure_output(git_repo.path(), &["notes", "add", "-m", "bad", "HEAD"])
    );
    git(git_repo.path(), ["config", "--unset", "core.notesRef"]);
    git(skron_repo.path(), ["config", "--unset", "core.notesRef"]);
    ...
    for args in [
        ...
        assert_eq!(
            command_failure_output_with_env(skron_bin(), skron_repo.path(), &args, &envs, "skron"),
...
fn notes_tree_fanout_matches_stock_git_after_growth() {
    ...
    for index in 1..=64 {
        ...
        write_file(skron_repo.path(), &file, &content);
        git(git_repo.path(), ["add", "-A"]);
        run_skron(skron_repo.path(), ["add", "-A"]);
        git_with_env(git_repo.path(), ["commit", "-m", &message]);
        run_skron_with_env(skron_repo.path(), ["commit", "-m", &message]);
        git_with_env(git_repo.path(), ["notes", "add", "-m", &note, "HEAD"]);
        run_skron_with_env(skron_repo.path(), ["notes", "add", "-m", &note, "HEAD"]);
    }

    assert_eq!(
        git(skron_repo.path(), ["ls-tree", "refs/notes/commits"]),
        git(git_repo.path(), ["ls-tree", "refs/notes/commits"])
    );
    assert!(!git(skron_repo.path(), ["ls-tree", "refs/notes/commits"]).contains(" tree "));
...
fn notes_merge_matches_stock_git_for_clean_and_strategy_merges() {
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["notes", "--ref=left", "list"]),
    ...
    for strategy in ["ours", "theirs", "union", "cat_sort_uniq"] {
        ...
        assert_eq!(
            run_skron(skron_repo.path(), ["notes", "--ref=left", "show", &object]),
    ...
    let skron_left_before = git(skron_repo.path(), ["rev-parse", "refs/notes/left"]);
    assert_eq!(
        run_skron_failure_output(
            skron_repo.path(),
            &["notes", "--ref=left", "merge", "right"]
    ...
    assert_eq!(
        fs::read_to_string(skron_repo.path().join(".git").join("NOTES_MERGE_REF"))
            .expect("read skron state"),
        fs::read_to_string(git_repo.path().join(".git").join("NOTES_MERGE_REF"))
            .expect("read git state"),
    );
    let skron_partial =
        fs::read_to_string(skron_repo.path().join(".git").join("NOTES_MERGE_PARTIAL"))
            .expect("read skron partial");
    let git_partial = fs::read_to_string(git_repo.path().join(".git").join("NOTES_MERGE_PARTIAL"))
        .expect("read git partial");
    assert_eq!(
        git(skron_repo.path(), ["cat-file", "-t", skron_partial.trim()]),
    ...
    assert_eq!(
        ...
        .expect("read skron conflict file"),
        fs::read_to_string(
            git_repo
                .path()
                .join(".git")
    ...
    assert_eq!(
        git(skron_repo.path(), ["rev-parse", "refs/notes/left"]),
        skron_left_before
    );
    assert_eq!(
    ...
    let _ = run_skron_failure_output(
        skron_repo.path(),
        &["notes", "--ref=left", "merge", "right"],
    );
    assert_eq!(
...
fn notes_prune_matches_stock_git_for_missing_objects() {
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["notes", "prune", "-v"]),
