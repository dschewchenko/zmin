...
use common::{
    ...
    run_skron, run_skron_args, run_skron_status, skron_bin,
};
...
fn clean_matches_stock_git_for_dry_run_force_and_paths() {
    ...
    let skron_repo = clean_fixture_repo();

    assert_eq!(
        run_skron(skron_repo.path(), ["clean", "-n"]),
        git(git_repo.path(), ["clean", "-n"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["clean", "-n", "-d"]),
        git(git_repo.path(), ["clean", "-n", "-d"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["clean", "-n", "-d", "dir"]),
        git(git_repo.path(), ["clean", "-n", "-d", "dir"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["clean", "-X", "-n"]),
        git(git_repo.path(), ["clean", "-X", "-n"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["clean", "-X", "-n", "-d"]),
    ...
    assert_eq!(
        run_skron_status(skron_repo.path(), ["clean"]),
        git_status(git_repo.path(), ["clean"])
    );

    assert_eq!(
        run_skron(skron_repo.path(), ["clean", "-f", "-d"]),
    ...
    assert!(skron_repo.path().join("ignored.log").exists());
    assert!(skron_repo.path().join("ignored_dir/ignored.txt").exists());
    assert!(skron_repo.path().join("tracked_dir/tracked.txt").exists());
    assert!(!skron_repo.path().join("dir").exists());
    assert!(!skron_repo.path().join("tracked_dir/untracked.txt").exists());
    assert!(!skron_repo.path().join("untracked.txt").exists());
}
...
fn clean_quiet_matches_stock_git_output_and_state() {
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["clean", "-f", "-q", "-d"]),
...
fn clean_exclude_patterns_match_stock_git_modes() {
    for args in [
        ...
        fs::write(skron_repo.path().join("keep.tmp"), b"keep\n").expect("write skron keep");
    ...
    fs::write(skron_repo.path().join("keep.tmp"), b"keep\n").expect("write skron keep");
    assert_eq!(
        run_skron(skron_repo.path(), ["clean", "-f", "-x", "-e", "keep.tmp"]),
        git(git_repo.path(), ["clean", "-f", "-x", "-e", "keep.tmp"])
    );
    assert!(skron_repo.path().join("keep.tmp").exists());
    assert!(!skron_repo.path().join("ignored.log").exists());
}
...
fn clean_no_option_toggles_match_stock_git_order() {
    for args in [
        ...
        assert_eq!(
            skron_repo.path().join("keep.tmp").exists(),
            git_repo.path().join("keep.tmp").exists(),
            "keep.tmp existence should match for {args:?}"
        );
        assert_eq!(
            skron_repo.path().join("untracked.txt").exists(),
...
fn clean_nested_git_directories_require_double_force_like_stock_git() {
    for args in [
        ...
        fs::create_dir_all(skron_repo.path().join("nested")).expect("create skron nested");
        git(git_repo.path().join("nested").as_path(), ["init"]);
        git(skron_repo.path().join("nested").as_path(), ["init"]);
        fs::write(git_repo.path().join("nested/file.txt"), b"nested\n").expect("write git nested");
        fs::write(skron_repo.path().join("nested/file.txt"), b"nested\n")
        ...
        assert_eq!(
            skron_repo.path().join("nested/.git").exists(),
...
fn clean_require_force_false_matches_stock_git() {
    ...
    git(skron_repo.path(), ["config", "clean.requireForce", "false"]);

    assert_eq!(
        command_any_output(skron_bin(), skron_repo.path(), &["clean"], "skron"),
    ...
    assert_eq!(
        skron_repo.path().join("dir").exists(),
...
fn clean_invalid_require_force_bool_matches_stock_git() {
    ...
    git(skron_repo.path(), ["config", "clean.requireForce", "maybe"]);
