...
use common::{
    ...
    git_with_stdin, git_with_stdin_args, run_skron, run_skron_failure_output, skron_bin,
    write_file,
};
...
fn maintenance_unknown_subcommand_failure_matches_stock_git() {
    ...
    assert_eq!(
        run_skron_failure_output(skron_repo.path(), &["maintenance", "frobnicate"]),
...
fn commit_graph_write_creates_stock_verifiable_graph() {
    ...
    assert_eq!(
        run_skron(repo.path(), ["commit-graph", "write", "--reachable"]),
        ""
    );
    assert!(repo.path().join(".git/objects/info/commit-graph").exists());
    assert_eq!(run_skron(repo.path(), ["commit-graph", "verify"]), "");
...
fn multi_pack_index_write_creates_stock_verifiable_index() {
    ...
    assert_eq!(run_skron(repo.path(), ["multi-pack-index", "write"]), "");
    assert!(
        repo.path()
            .join(".git/objects/pack/multi-pack-index")
            .exists()
    );
    assert_eq!(run_skron(repo.path(), ["multi-pack-index", "verify"]), "");
...
fn multi_pack_index_write_empty_repo_failure_matches_stock_git() {
    ...
    assert_eq!(
        run_skron_failure_output(skron_repo.path(), &["multi-pack-index", "write"]),
...
fn repack_write_midx_empty_repo_noops_like_stock_git() {
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["repack", "-m", "-q"]),
...
fn multi_pack_index_progress_flags_are_accepted_like_stock_git() {
    ...
    assert_eq!(
        run_skron(repo.path(), ["multi-pack-index", "write", "--no-progress"]),
        ""
    );
    assert_eq!(
        run_skron(repo.path(), ["multi-pack-index", "verify", "--progress"]),
        ""
    );
    assert_eq!(
        run_skron(repo.path(), ["multi-pack-index", "expire", "--no-progress"]),
        ""
    );
    assert_eq!(
        run_skron(
...
fn multi_pack_index_repack_batch_size_one_noops_like_stock_git() {
    ...
    let skron_repo = two_pack_midx_fixture();
    git(git_repo.path(), ["multi-pack-index", "write"]);
    assert_eq!(
        run_skron(skron_repo.path(), ["multi-pack-index", "write"]),
        ""
    );
    let git_before = pack_file_names(git_repo.path());
    let skron_before = pack_file_names(skron_repo.path());
    ...
    assert_eq!(pack_file_names(skron_repo.path()), skron_before);
    assert_eq!(
        run_skron(skron_repo.path(), ["multi-pack-index", "verify"]),
        ""
    );
    assert_eq!(
        git_status(skron_repo.path(), ["multi-pack-index", "verify"]),
        0
    );
}
...
fn multi_pack_index_repack_and_expire_consolidate_like_stock_git() {
    ...
    assert_eq!(
        pack_file_count(skron_repo.path()),
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["multi-pack-index", "expire"]),
        ""
    );
    assert_eq!(
        pack_file_count(skron_repo.path()),
    ...
    assert_eq!(pack_file_count(skron_repo.path()), 1);
    assert_eq!(
        run_skron(skron_repo.path(), ["multi-pack-index", "verify"]),
        ""
    );
    ...
    assert_eq!(git_status(skron_repo.path(), ["fsck", "--strict"]), 0);
}

#[test]
fn pack_refs_matches_stock_git_for_default_all_and_no_prune() {
    let git_repo = pack_refs_fixture_repo();
    let skron_repo = pack_refs_fixture_repo();

    git(git_repo.path(), ["pack-refs"]);
    run_skron(skron_repo.path(), ["pack-refs"]);
    assert_eq!(
        fs::read_to_string(skron_repo.path().join(".git/packed-refs"))
            .expect("read skron packed refs"),
        fs::read_to_string(git_repo.path().join(".git/packed-refs")).expect("read git packed refs")
    );
    assert_eq!(
        ref_file_list(skron_repo.path()),
    ...
    run_skron(skron_repo.path(), ["pack-refs", "--all", "--no-prune"]);
    assert_eq!(
        fs::read_to_string(skron_repo.path().join(".git/packed-refs"))
            .expect("read skron packed refs"),
        fs::read_to_string(git_repo.path().join(".git/packed-refs")).expect("read git packed refs")
    ...
    run_skron(skron_repo.path(), ["pack-refs", "--all", "--prune"]);
    assert_eq!(
        fs::read_to_string(skron_repo.path().join(".git/packed-refs"))
            .expect("read skron packed refs"),
        fs::read_to_string(git_repo.path().join(".git/packed-refs")).expect("read git packed refs")
...
fn prune_packed_matches_stock_git_for_dry_run_and_prune() {
    ...
    let skron_head = duplicate_packed_head_as_loose(skron_repo.path());
    assert_eq!(git_head, skron_head);

    assert_eq!(
        run_skron(skron_repo.path(), ["prune-packed", "-n"]),
        git(git_repo.path(), ["prune-packed", "-n"])
    );
    assert!(loose_object_exists(skron_repo.path(), &skron_head));

    git(git_repo.path(), ["prune-packed"]);
    run_skron(skron_repo.path(), ["prune-packed"]);
    assert_eq!(
        loose_object_exists(skron_repo.path(), &skron_head),
        loose_object_exists(git_repo.path(), &git_head)
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["cat-file", "-t", &skron_head]),
...
fn repack_packs_objects_and_prunes_loose_duplicates() {
    ...
    assert_eq!(run_skron(repo.path(), ["repack", "-adq"]), "");
    assert_eq!(run_skron(repo.path(), ["repack", "-adq"]), "");
...
fn repack_all_keeps_unreachable_loose_objects_out_of_pack() {
    ...
    let skron_dangling = git_with_stdin(
        skron_repo.path(),
        ["hash-object", "-w", "--stdin"],
        "dangling\n",
    );
    assert_eq!(skron_dangling, git_dangling);

    git(git_repo.path(), ["repack", "-adq"]);
    assert_eq!(run_skron(skron_repo.path(), ["repack", "-adq"]), "");
    assert!(loose_object_exists(skron_repo.path(), &skron_dangling));
    assert_eq!(
        loose_object_exists(skron_repo.path(), &skron_dangling),
        loose_object_exists(git_repo.path(), &git_dangling)
    );
    assert_eq!(
        packed_object_ids(skron_repo.path()).contains(&skron_dangling),
...
fn repack_loosens_packed_unreachable_objects_like_stock_git() {
    ...
    let skron_unreachable = &unreachable_commits[1];
    assert_eq!(skron_unreachable, git_unreachable);

    git(git_repo.path(), ["repack", "-Adq"]);
    assert_eq!(run_skron(skron_repo.path(), ["repack", "-Adq"]), "");
    assert!(loose_object_exists(skron_repo.path(), skron_unreachable));
    assert_eq!(
        loose_object_exists(skron_repo.path(), skron_unreachable),
        loose_object_exists(git_repo.path(), git_unreachable)
    );
    assert_eq!(
        packed_object_ids(skron_repo.path()).contains(skron_unreachable),
        packed_object_ids(git_repo.path()).contains(git_unreachable)
    );
    assert_eq!(
        git(skron_repo.path(), ["cat-file", "-t", skron_unreachable]),
...
fn repack_write_midx_creates_stock_verifiable_multi_pack_index() {
    ...
    assert_eq!(run_skron(repo.path(), ["repack", "-adqm"]), "");
    assert!(
        repo.path()
            .join(".git/objects/pack/multi-pack-index")
            .is_file()
    ...
    git(repo.path(), ["add", "-A"]);
    git_with_env(repo.path(), ["commit", "-m", "two"]);
    assert_eq!(run_skron(repo.path(), ["repack", "-m", "-n", "-q"]), "");
    assert_eq!(git_status(repo.path(), ["multi-pack-index", "verify"]), 0);
}
...
fn repack_negated_bitmap_and_midx_flags_match_stock_git_effects() {
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["repack", "-adqm", "--no-write-midx"]),
        ""
    );
    assert_eq!(
        skron_repo
    ...
    assert_eq!(
        git_status(skron_repo.path(), ["fsck", "--strict"]),
...
fn repack_delete_redundant_removes_stale_multi_pack_index_like_stock_git() {
    ...
    assert_eq!(run_skron(skron_repo.path(), ["repack", "-adqm"]), "");
...
fn repack_keep_pack_preserves_named_pack_like_stock_git() {
    ...
    let git_keep = first_pack_index(git_repo.path())
        ...
        .to_string_lossy()
        .into_owned();
    let skron_keep = first_pack_index(skron_repo.path())
        .with_extension("pack")
        .file_name()
        .expect("skron keep pack")
        .to_string_lossy()
        .into_owned();
    assert_eq!(skron_keep, git_keep);
    ...
    assert_eq!(
        run_skron(
            skron_repo.path(),
            ["repack", "-adq", "--keep-pack", &skron_keep]
        ),
        ""
    );
    let git_packs = pack_file_names(git_repo.path());
    let skron_packs = pack_file_names(skron_repo.path());
    assert_eq!(
        skron_packs.contains(&skron_keep),
    ...
    assert_eq!(
        git(skron_repo.path(), ["cat-file", "-p", "HEAD:b.txt"]),
...
fn repack_no_reuse_flags_write_stock_readable_pack() {
    ...
    assert_eq!(run_skron(repo.path(), ["repack", "-adqfF"]), "");
    assert!(first_pack_index(repo.path()).is_file());
...
fn repack_window_depth_writes_stock_readable_delta_pack() {
    ...
    assert_eq!(
        run_skron(repo.path(), ["repack", "--window=10", "--depth=10", "-q"]),
        ""
    );
...
fn repack_local_keeps_alternate_objects_out_of_local_pack() {
    ...
    let skron_clone = dir.path().join("skron-shared");
    git(
        dir.path(),
    ...
    for repo in [&git_clone, &skron_clone] {
        configure_identity(repo);
    ...
    assert_eq!(run_skron(&skron_clone, ["repack", "-adlq"]), "");
    assert_eq!(
        packed_object_ids(&skron_clone),
        packed_object_ids(&git_clone)
    );
    assert_eq!(
        git(&skron_clone, ["cat-file", "-p", "HEAD:a.txt"]),
...
fn gc_packs_reachable_objects_and_keeps_head_readable() {
    ...
    assert_eq!(run_skron(repo.path(), ["gc", "--prune=now", "--quiet"]), "");
    assert!(first_pack_index(repo.path()).is_file());
    for id in &objects {
        assert!(
            !loose_object_exists(repo.path(), id),
...
fn gc_aggressive_writes_stock_readable_delta_pack() {
    ...
    let (skron_base, skron_changed) = seed(skron_repo.path());
    assert_eq!(skron_base, git_base);
    assert_eq!(skron_changed, git_changed);

    assert_eq!(git(git_repo.path(), ["gc", "--aggressive", "--quiet"]), "");
    assert_eq!(
        run_skron(skron_repo.path(), ["gc", "--aggressive", "--quiet"]),
        ""
    );
    ...
    let skron_verify = git(
        skron_repo.path(),
        [
            "verify-pack",
            "-v",
            first_pack_index(skron_repo.path())
                .to_str()
                .expect("idx path"),
        ],
    );
...
fn maintenance_run_gc_reuses_repository_gc() {
    ...
    assert_eq!(
        run_skron(repo.path(), ["maintenance", "run", "--task=gc", "--quiet"]),
        ""
    );
    assert!(first_pack_index(repo.path()).is_file());
    assert!(git(repo.path(), ["cat-file", "-p", &head]).contains("\n\none"));
...
fn maintenance_register_and_unregister_match_stock_git_config_effects() {
    ...
    command_output_with_env(
        ...
        skron_repo.path(),
        &["maintenance", "register"],
        &[("HOME", skron_home.path().to_str().expect("skron home path"))],
        "skron",
    );
    assert_eq!(
        git(
    ...
    let skron_registered = command_output_with_env(
        "git",
        skron_repo.path(),
    ...
    assert_eq!(
        skron_registered,
        fs::canonicalize(skron_repo.path())
            .expect("canonical skron repo")
            .display()
            .to_string()
    );
    ...
    assert_eq!(
        fs::read_to_string(skron_home.path().join(".gitconfig")).expect("skron global config"),
...
fn maintenance_register_is_idempotent_like_stock_git() {
    ...
    .1;
    assert_eq!(
        skron_registered.lines().count(),
        git_registered.lines().count()
    );
    assert_eq!(skron_registered.lines().count(), 1);
}
...
fn maintenance_register_and_unregister_with_config_file_match_stock_git() {
    ...
    let skron_config = skron_home.path().join("custom.gitconfig");
    ...
    command_output_with_env(
        ...
        &[
            ...
            "register",
            "--config-file",
            skron_config.to_str().expect("skron config path"),
        ],
        &[("HOME", skron_home.path().to_str().expect("skron home path"))],
        "skron",
    );
    ...
    assert_eq!(
        normalize_repo_path(
            fs::read_to_string(&skron_config).expect("read skron config"),
            skron_repo.path(),
        ),
        normalize_repo_path(
...
fn maintenance_start_and_stop_auto_manage_launch_agents_like_stock_git() {
    ...
    let skron_start = command_output_with_env(
        skron_bin(),
        skron_repo.path(),
        &["maintenance", "start", "--scheduler=auto"],
        &[("HOME", skron_home.path().to_str().expect("skron home path"))],
    ...
    assert_eq!(
        skron_start.0, 0,
        "skron launchctl start should succeed when stock git launchctl runtime is available: {skron_start:?}"
    );
    assert_eq!(skron_start.0, git_start.0);
    assert_eq!(
        normalize_repo_path(skron_start.1, skron_repo.path()),
        normalize_repo_path(git_start.1, git_repo.path())
    );
    assert_eq!(skron_start.2, git_start.2);
    ...
    assert_eq!(skron_registered.0, git_registered.0);
    assert_eq!(
        normalize_repo_path(skron_registered.1, skron_repo.path()),
        normalize_repo_path(git_registered.1, git_repo.path())
    );
    assert_eq!(skron_registered.2, git_registered.2);
    ...
    let skron_stop = command_output_with_env(
        skron_bin(),
        skron_repo.path(),
        &["maintenance", "stop"],
        &[("HOME", skron_home.path().to_str().expect("skron home path"))],
    ...
    assert_eq!(skron_stop.0, git_stop.0);
...
fn maintenance_start_and_stop_schtasks_match_stock_git_when_enabled() {
    if std::env::var_os("SKRON_ENABLE_WINDOWS_MAINTENANCE_RUNTIME_TESTS").is_none() {
        eprintln!(
            "skipping schtasks runtime test; set SKRON_ENABLE_WINDOWS_MAINTENANCE_RUNTIME_TESTS=1 to enable"
        );
        return;
    }
    ...
    for (task, schedule) in tasks {
        assert!(
            ...
            "expected scheduled task {task} after skron start"
        );
        assert_schtasks_task_xml_matches_expected_shape(task, schedule, "skron");
    }
    ...
    for (task, _) in tasks {
        assert!(
            !schtasks_task_exists(task),
            "expected scheduled task {task} to be removed after skron stop"
        );
    }
    ...
    assert_eq!(skron_stop.0, git_stop.0);
    assert_eq!(skron_stop.1, git_stop.1);
    assert_eq!(skron_stop.2, git_stop.2);
...
fn maintenance_start_and_stop_crontab_match_stock_git_when_enabled() {
    if std::env::var_os("SKRON_ENABLE_LINUX_MAINTENANCE_RUNTIME_TESTS").is_none() {
        eprintln!(
            "skipping linux maintenance runtime test; set SKRON_ENABLE_LINUX_MAINTENANCE_RUNTIME_TESTS=1 to enable"
        );
        return;
    }
    ...
    let skron_crontab = crontab_contents();
    let skron_registered = command_output_with_env(
        "git",
        skron_repo.path(),
        &["config", "--global", "--get-all", "maintenance.repo"],
    ...
    let skron_crontab_after_stop = crontab_contents();
    ...
    assert_eq!(
        normalize_crontab(skron_crontab, skron_repo.path()),
    ...
    assert_eq!(skron_stop.1, git_stop.1);
    assert_eq!(skron_stop.2, git_stop.2);
    assert_eq!(skron_crontab_after_stop, git_crontab_after_stop);
...
fn maintenance_run_schedule_matches_stock_git_strategy_selection() {
    ...
    assert_eq!(git_status(skron_repo.path(), ["commit-graph", "verify"]), 0);
    assert_eq!(
        git_status(skron_repo.path(), ["multi-pack-index", "verify"]),
        0
    );
...
fn maintenance_run_schedule_task_and_auto_failures_match_stock_git() {
    ...
    for args in [
        ...
        assert_eq!(
            run_skron_failure_output(skron_repo.path(), args),
...
fn maintenance_prefetch_noops_without_remotes_like_stock_git() {
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["maintenance", "run", "--task=prefetch"]),
...
fn maintenance_prefetch_local_remote_writes_prefetch_refs_like_stock_git() {
    ...
    assert_eq!(git(skron_repo.path(), ["fsck", "--strict"]), "");
}
...
fn prune_matches_stock_git_for_unreachable_loose_objects() {
    ...
    let (skron_pruned, skron_kept, skron_staged) = prune_fixture(skron_repo.path());
    assert_eq!(skron_pruned, git_pruned);
    assert_eq!(skron_kept, git_kept);
    assert_eq!(skron_staged, git_staged);

    assert_eq!(
        run_skron(
            skron_repo.path(),
            ["prune", "-n", "--expire=now", &skron_kept]
        ),
        git(git_repo.path(), ["prune", "-n", "--expire=now", &git_kept])
    );
    assert!(loose_object_exists(skron_repo.path(), &skron_pruned));

    git(git_repo.path(), ["prune", "--expire=now", &git_kept]);
    run_skron(skron_repo.path(), ["prune", "--expire=now", &skron_kept]);
    assert_eq!(
        loose_object_exists(skron_repo.path(), &skron_pruned),
        loose_object_exists(git_repo.path(), &git_pruned)
    );
    assert_eq!(
        loose_object_exists(skron_repo.path(), &skron_kept),
        loose_object_exists(git_repo.path(), &git_kept)
    );
    assert_eq!(
        loose_object_exists(skron_repo.path(), &skron_staged),
...
fn prune_preserves_objects_reachable_from_reflogs() {
    ...
    let skron_reflog_commit = reflog_prune_fixture(skron_repo.path());
    assert_eq!(skron_reflog_commit, git_reflog_commit);

    git(git_repo.path(), ["prune", "--expire=now"]);
    run_skron(skron_repo.path(), ["prune", "--expire=now"]);
    assert_eq!(
        loose_object_exists(skron_repo.path(), &skron_reflog_commit),
...
fn prune_matches_stock_git_for_negated_options() {
    ...
    let (skron_pruned, skron_kept, _) = prune_fixture(skron_repo.path());
    assert_eq!(skron_pruned, git_pruned);
    assert_eq!(skron_kept, git_kept);
