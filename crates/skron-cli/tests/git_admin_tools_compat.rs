...
fn for_each_repo_matches_stock_git_for_configured_repositories() {
    ...
    assert_eq!(
        command_with_home(skron_bin(), home.path(), &args),
...
fn for_each_repo_missing_repo_failures_match_stock_git() {
    ...
    assert_eq!(
        command_failure_with_home(skron_bin(), home.path(), &args),
    ...
    assert_eq!(
        command_failure_with_home(skron_bin(), home.path(), &keep_going_args),
...
fn bugreport_creates_report_file_in_output_directory() {
    ...
    let skron_output = command_any_with_git_editor(
        skron_bin(),
        repo.path(),
        &[
            "bugreport",
    ...
    assert_eq!(skron_output.0, 0);
    assert_eq!(skron_output.1, "");
    assert!(skron_output.2.contains("Created new report at '"));
...
fn bugreport_suffix_modes_match_stock_git_files() {
    ...
    let skron_output = TempDir::new().expect("skron bugreport output");
    ...
    for (label, args, expected_file) in [
        ...
        let skron_args = args
            ...
            .map(|arg| {
                if *arg == "__OUT__" {
                    skron_output.path().to_str().expect("skron output path")
                } else {
                    arg
                }
            })
        ...
        let skron_result = command_any_with_git_editor(skron_bin(), skron_repo.path(), &skron_args);
        assert_eq!(skron_result.0, git_result.0, "{label} exit mismatch");
        assert_eq!(skron_result.1, git_result.1, "{label} stdout mismatch");
        assert!(
            git_output.path().join(&expected_file).is_file(),
            "stock Git did not create expected file for {label}",
        );
        assert!(
            skron_output.path().join(&expected_file).is_file(),
            "skron did not create expected file for {label}",
        );
    }
}
...
fn repo_info_reports_repository_metadata() {
    ...
    let keys = command_any_with_git_editor(skron_bin(), repo.path(), &["repo", "info", "--keys"]);
    assert_eq!(keys.0, 0);
...
fn backfill_promisor_remote_recovers_missing_local_objects() {
    ...
    let skron_repo = init_promisor_work_repo(remote.path(), &source_head);
    ...
    let skron_object_path = loose_object_path(skron_repo.path(), &blob);
    assert!(
    ...
    assert!(
        skron_object_path.is_file(),
        "expected loose object at {}",
        skron_object_path.display()
    );
    fs::remove_file(&git_object_path).expect("remove git blob");
    fs::remove_file(&skron_object_path).expect("remove skron blob");
    assert!(!git_object_path.exists());
    assert!(!skron_object_path.exists());

    let skron_backfill =
        command_any_with_git_editor(skron_bin(), skron_repo.path(), &["backfill", "HEAD"]);
    let git_backfill = command_any_with_git_editor("git", git_repo.path(), &["backfill", "HEAD"]);
    assert_eq!(skron_backfill.0, 0);
    assert_eq!(git_backfill.0, 0);
    assert_eq!(skron_backfill.1, git_backfill.1);

    assert_eq!(git(git_repo.path(), ["cat-file", "-t", &blob]), "blob");
    assert_eq!(
        run_skron(skron_repo.path(), ["cat-file", "-t", &blob]),
        "blob"
    );
}
...
fn hook_run_matches_stock_git_for_missing_and_executable_hooks() {
    ...
    assert_eq!(
        command_any_with_git_editor(skron_bin(), repo.path(), &["hook", "run", "pre-commit"]),
...
fn version_command_reports_git_compatible_version_shape() {
    ...
    let version = command_any_with_git_editor(skron_bin(), repo.path(), &["version"]);
    assert_eq!(version.0, 0);
    assert!(version.1.starts_with("git version "));
    assert!(version.1.contains(".skron"));
    assert_eq!(version.2, "");

    let build_options =
        command_any_with_git_editor(skron_bin(), repo.path(), &["version", "--build-options"]);
...
fn config_get_set_and_list_match_stock_git() {
    ...
    run_skron(skron_repo.path(), ["config", "user.name", "Bench"]);
    git(
        git_repo.path(),
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["config", "--get", "user.name"]),
        git(git_repo.path(), ["config", "--get", "user.name"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["config", "branch.main.remote"]),
        git(git_repo.path(), ["config", "branch.main.remote"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["config", "--list"]),
...
fn config_include_origin_and_scope_match_stock_git() {
    ...
    for args in [
        ...
        assert_eq!(
            command_with_isolated_config(skron_bin(), skron_repo.path(), home.path(), args),
    ...
    assert_eq!(
        command_with_isolated_config(skron_bin(), skron_repo.path(), home.path(), &global_args),
...
fn config_include_hasconfig_matches_stock_git() {
    ...
    for home in [git_home.path(), skron_home.path()] {
        fs::write(
            home.join(".gitconfig"),
    ...
    let skron_forbidden = command_any_with_isolated_config_and_env(
        skron_bin(),
        skron_repo.path(),
        skron_home.path(),
        &[],
    ...
    assert_eq!(skron_forbidden.0, git_forbidden.0);
    assert_eq!(skron_forbidden.1, git_forbidden.1);
    assert_eq!(
        skron_forbidden.2, git_forbidden.2,
...
fn config_type_bool_matches_stock_git_for_get_set_and_failures() {
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["config", "--get", "feature.enabled"]),
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["config", "--get", "feature.disabled"]),
    ...
    run_skron(skron_repo.path(), ["config", "feature.raw", "maybe"]);
    assert_eq!(
        run_skron_failure_output(skron_repo.path(), &["config", "--type=bool", "feature.raw"]),
...
fn config_type_int_and_bool_or_int_match_stock_git() {
    ...
    for (key, value) in [
        ...
        run_skron(skron_repo.path(), ["config", "--type=int", key, value]);
        assert_eq!(
            run_skron(skron_repo.path(), ["config", "--get", key]),
            git(git_repo.path(), ["config", "--get", key]),
            "raw stored value for {key}"
        );
        assert_eq!(
            run_skron(skron_repo.path(), ["config", "--type=int", key]),
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["config", "--get", "demo.mix"]),
...
fn config_type_bool_or_str_matches_stock_git() {
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["config", "--get", "demo.flag"]),
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["config", "--get", "demo.label"]),
...
fn config_legacy_type_flags_match_stock_git() {
    ...
    for (key, value, args) in [
        ...
        run_skron(skron_repo.path(), ["config", key, value]);
        assert_eq!(
...
fn config_type_path_expiry_and_color_match_stock_git() {
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["config", "--get", "demo.home"]),
        git(git_repo.path(), ["config", "--get", "demo.home"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["config", "--type=path", "demo.home"]),
    ...
    assert_eq!(
        run_skron(
            skron_repo.path(),
            ["config", "--type=path", "demo.relative"]
        ),
    ...
    for (key, value) in [("demo.never", "never"), ("demo.now", "now")] {
        ...
        assert_eq!(
            run_skron(skron_repo.path(), ["config", "--type=expiry-date", key]),
    ...
    run_skron(skron_repo.path(), ["config", "demo.badexpiry", "bad"]);
    assert_eq!(
        run_skron_failure_output(
            skron_repo.path(),
    ...
    for (key, value) in [
        ...
        run_skron(skron_repo.path(), ["config", "--type=color", key, value]);
        assert_eq!(
            run_skron(skron_repo.path(), ["config", "--type=color", key]).into_bytes(),
...
fn var_identity_matches_stock_git() {
    ...
    assert_eq!(
        run_skron_with_env(repo.path(), ["var", "GIT_AUTHOR_IDENT"]),
        git_with_env(repo.path(), ["var", "GIT_AUTHOR_IDENT"])
    );
    assert_eq!(
        run_skron_with_env(repo.path(), ["var", "GIT_COMMITTER_IDENT"]),
        git_with_env(repo.path(), ["var", "GIT_COMMITTER_IDENT"])
    );
    assert_eq!(
        run_skron(repo.path(), ["var", "GIT_DEFAULT_BRANCH"]),
...
fn shell_pure_helpers_match_git_232_direct_execution() {
    ...
    assert_eq!(run_skron(repo.path(), ["sh-i18n"]), "");
    assert_eq!(run_skron(repo.path(), ["sh-i18n", "-h"]), "");
    assert_eq!(run_skron(repo.path(), ["sh-i18n", "ignored"]), "");
    assert_eq!(run_skron(repo.path(), ["sh-setup"]), "");
    assert_eq!(
        run_skron(repo.path(), ["sh-setup", "-h"]),
        "usage: git sh-setup "
    );
    assert_eq!(run_skron(repo.path(), ["sh-setup", "ignored"]), "");
}
...
fn remote_config_commands_match_stock_git() {
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["remote"]),
        git(git_repo.path(), ["remote"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["remote", "-v"]),
        git(git_repo.path(), ["remote", "-v"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["remote", "get-url", "origin"]),
    ...
    assert_eq!(
        git(skron_repo.path(), ["config", "--local", "--list"]),
        git(git_repo.path(), ["config", "--local", "--list"])
    );

    git(git_repo.path(), ["remote", "remove", "upstream"]);
    run_skron(skron_repo.path(), ["remote", "remove", "upstream"]);
...
fn remote_show_and_prune_match_stock_git_for_local_remote() {
    ...
    let skron_clone = fixture.path().join("skron-local");
    ...
    for destination in [&git_clone, &skron_clone] {
        let output = Command::new("git")
            .args([
                "clone",
    ...
    assert_eq!(
        run_skron(&skron_clone, ["remote", "show", "-n", "origin"]),
        git(&git_clone, ["remote", "show", "-n", "origin"])
    );
    assert_eq!(
        run_skron(&skron_clone, ["remote", "prune", "-n", "origin"]),
        git(&git_clone, ["remote", "prune", "-n", "origin"])
    );
    assert_eq!(
        run_skron(&skron_clone, ["remote", "prune", "origin"]),
        git(&git_clone, ["remote", "prune", "origin"])
    );
    assert_eq!(
        git(&skron_clone, ["branch", "-r"]),
        git(&git_clone, ["branch", "-r"])
    );
    assert_eq!(
        run_skron_failure_output(&skron_clone, &["remote", "prune", "missing"]),
...
fn remote_set_branches_matches_stock_git_config() {
    ...
    run_skron(skron_repo.path(), ["remote", "set-branches", "origin"]);
    assert_eq!(
        git(skron_repo.path(), ["config", "--local", "--list"]),
        git(git_repo.path(), ["config", "--local", "--list"])
    );
...
fn remote_update_matches_stock_git_for_local_remotes() {
    ...
    assert_eq!(
        run_skron(&skron_clone, ["remote", "update"]),
    ...
    assert_eq!(
        run_skron(&skron_clone, ["remote", "update", "-p"]),
    ...
    assert_eq!(
        run_skron_failure_output(&skron_clone, &["remote", "update", "missing"]),
    ...
    assert_eq!(
        run_skron(&skron_clone, ["remote", "update", "team"]),
    ...
    assert_eq!(
        run_skron_failure_output(&skron_clone, &["remote", "update", "broken"]),
