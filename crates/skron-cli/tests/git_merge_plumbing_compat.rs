...
use common::{
    ...
    run_skron, run_skron_args, run_skron_status, skron_bin, write_file,
};
...
fn run_skron_output<const N: usize>(cwd: &Path, args: [&str; N]) -> CommandTextOutput {
    command_text_output(skron_bin(), cwd, &args)
}
...
fn merge_file_matches_stock_git_for_clean_and_conflict_cases() {
    ...
    let skron_conflict = run_skron_output(
        dir.path(),
        [
            "merge-file",
            "-p",
    ...
    assert_eq!(skron_conflict.status, git_conflict.status);
    assert_eq!(skron_conflict.stdout, git_conflict.stdout);

    write_file(dir.path(), "git-inplace.txt", "a\nours\nc\n");
    write_file(dir.path(), "skron-inplace.txt", "a\nours\nc\n");
    ...
    let skron_status = run_skron_status(
        dir.path(),
        [
            "merge-file",
            "-L",
    ...
    assert_eq!(skron_status, git_status);
    assert_eq!(
        fs::read_to_string(dir.path().join("skron-inplace.txt")).expect("read skron merge file"),
...
fn merge_one_file_matches_stock_git_for_clean_text_merge() {
    ...
    let skron_repo = merge_one_file_fixture();
    ...
    assert_eq!(
        fs::read_to_string(skron_repo.path().join("file.txt")).expect("read skron merge result"),
...
fn merge_one_file_matches_stock_git_for_identical_add_add() {
    ...
    assert_eq!(
        fs::read_to_string(skron_repo.path().join("added.txt")).expect("read skron add/add"),
        fs::read_to_string(git_repo.path().join("added.txt")).expect("read git add/add")
    );
    assert_eq!(
        git(skron_repo.path(), ["ls-files", "--stage", "added.txt"]),
...
fn mergetool_runs_configured_tool_and_stages_resolution_like_stock_git() {
    ...
    let skron_repo = mergetool_conflict_fixture();
    ...
    for repo in [git_repo.path(), skron_repo.path()] {
        git(repo, ["config", "mergetool.skrontest.cmd", command]);
        git(
            repo,
            ["config", "mergetool.skrontest.trustExitCode", "true"],
        );
    }

    let args = ["mergetool", "--tool=skrontest", "--no-prompt", "f.txt"];
    assert_eq!(
        run_skron_args(skron_repo.path(), &args),
        git_args(git_repo.path(), &args)
    );
    assert_eq!(
        fs::read_to_string(skron_repo.path().join("f.txt")).expect("read skron merged file"),
...
fn mergetool_uses_configured_default_tool_like_stock_git() {
    ...
    let command = "printf 'B:'; cat \"$BASE\"; printf 'L:'; cat \"$LOCAL\"; printf 'R:'; cat \"$REMOTE\"; printf 'resolved\\n' > \"$MERGED\"";
    for repo in [git_repo.path(), skron_repo.path()] {
        git(repo, ["config", "merge.tool", "skrontest"]);
        git(repo, ["config", "mergetool.skrontest.cmd", command]);
        git(
            repo,
            ["config", "mergetool.skrontest.trustExitCode", "true"],
...
fn rerere_status_and_remaining_match_stock_git_for_unmerged_paths() {
    ...
    let skron_repo = rerere_conflict_fixture();

    for args in [["rerere", "status"], ["rerere", "remaining"]] {
        assert_eq!(
            run_skron(skron_repo.path(), args),
    ...
    assert_eq!(run_skron(disabled.path(), ["rerere", "status"]), "");
    assert_eq!(run_skron(disabled.path(), ["rerere", "remaining"]), "");
}
...
fn rerere_diff_default_forget_and_invalid_record_match_stock_git() {
    ...
    assert_eq!(
        command_all_output(skron_bin(), skron_repo.path(), &["rerere", "diff"]),
        command_all_output("git", git_repo.path(), &["rerere", "diff"])
    );

    write_file(git_repo.path(), "f.txt", "resolved\n");
    write_file(skron_repo.path(), "f.txt", "resolved\n");
    assert_eq!(
        command_all_output(skron_bin(), skron_repo.path(), &["rerere"]),
    ...
    assert_eq!(
        command_all_output(skron_bin(), skron_repo.path(), &["rerere", "record"]),
        command_all_output("git", git_repo.path(), &["rerere", "record"])
    );
    assert_eq!(
        command_all_output(skron_bin(), skron_repo.path(), &["rerere", "gc"]),
...
fn rerere_forget_matches_stock_git_for_pathspecs_and_duplicate_preimages() {
    ...
    let skron_repo = rerere_multi_conflict_fixture();
    write_file(git_repo.path(), "a.txt", "resolved a\n");
    write_file(git_repo.path(), "b.md", "resolved b\n");
    write_file(skron_repo.path(), "a.txt", "resolved a\n");
    write_file(skron_repo.path(), "b.md", "resolved b\n");
    assert_eq!(
        command_all_output("git", git_repo.path(), &["rerere"]),
        command_all_output(skron_bin(), skron_repo.path(), &["rerere"])
    );
    ...
    assert_eq!(
        ...
        run_skron(skron_repo.path(), ["rerere", "diff"])
    );
}
...
fn rerere_reuses_recorded_resolution_and_autoupdates_index_like_stock_git() {
    ...
    let skron_conflict =
        fs::read_to_string(skron_repo.path().join("f.txt")).expect("skron conflict");
    write_file(git_repo.path(), "f.txt", "resolved\n");
    write_file(skron_repo.path(), "f.txt", "resolved\n");
    assert_eq!(
        command_all_output("git", git_repo.path(), &["rerere"]),
    ...
    write_file(skron_repo.path(), "f.txt", &skron_conflict);
    assert_eq!(
        command_all_output("git", git_repo.path(), &["rerere"]),
        command_all_output(skron_bin(), skron_repo.path(), &["rerere"])
    );
    assert_eq!(
        fs::read_to_string(skron_repo.path().join("f.txt")).expect("skron resolved"),
...
fn rerere_gc_expiry_matches_stock_git_for_resolved_and_unresolved_cache() {
    ...
    assert_eq!(list(skron_repo.path()), list(git_repo.path()));
}
