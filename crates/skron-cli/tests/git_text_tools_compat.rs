...
use common::{
    git, git_args, git_init, git_with_stdin, git_with_stdin_args, run_skron, run_skron_args,
    run_skron_with_stdin, run_skron_with_stdin_args,
};
...
fn check_mailmap_matches_stock_git_for_common_entries() {
    ...
    for identity in [
        ...
        assert_eq!(
            run_skron(repo.path(), ["check-mailmap", identity]),
    ...
    let input =
        "Alias Name <alias@example.com>\nOther <old@example.com>\nNone <none@example.com>\n";
    assert_eq!(
        run_skron_with_stdin(repo.path(), ["check-mailmap", "--stdin"], input),
...
fn column_matches_stock_git_for_common_modes() {
    ...
    for args in [
        ...
        assert_eq!(
            run_skron_with_stdin_args(repo.path(), args, input),
...
fn stripspace_matches_stock_git_for_common_modes() {
    ...
    let fixture = "\n  a  \n\n\n  b  \n\n";
    assert_eq!(
        run_skron_with_stdin(repo.path(), ["stripspace"], fixture),
        git_with_stdin(repo.path(), ["stripspace"], fixture)
    );
    assert_eq!(
        run_skron_with_stdin(repo.path(), ["stripspace", "-s"], fixture),
        git_with_stdin(repo.path(), ["stripspace", "-s"], fixture)
    );
    assert_eq!(
        run_skron_with_stdin(repo.path(), ["stripspace", "-c"], fixture),
