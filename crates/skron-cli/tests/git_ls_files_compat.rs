...
fn ls_files_modes_match_stock_git() {
    ...
    assert_eq!(
        command_output_any(skron_bin(), repo.path(), &["ls-files", "--format=%(bad)"]),
    ...
    for args in [
        ...
        assert_eq!(
            command_output_any(skron_bin(), repo.path(), args),
...
fn ls_files_resolve_undo_matches_stock_git() {
    ...
    let skron = command_output_any(skron_bin(), repo.path(), &args);
