...
use common::{
    ...
    git, git_args, git_failure_output, git_init, git_status, git_with_env, run_skron,
    run_skron_args, run_skron_failure_output, run_skron_status, run_skron_with_env, skron_bin,
    write_file,
};
...
fn update_ref_and_symbolic_ref_match_stock_git_state() {
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["show-ref", "--heads"]),
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["symbolic-ref", "HEAD"]),
        git(git_repo.path(), ["symbolic-ref", "HEAD"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["symbolic-ref", "--short", "HEAD"]),
        git(git_repo.path(), ["symbolic-ref", "--short", "HEAD"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["branch", "--show-current"]),
    ...
    assert_eq!(
        run_skron_status(skron_repo.path(), ["symbolic-ref", "-q", "HEAD"]),
...
fn refs_verify_matches_stock_git_for_healthy_repository() {
    ...
    for args in [
        ...
        assert_eq!(
            command_any_output(skron_bin(), repo.path(), args, "skron refs verify"),
...
fn update_ref_stdin_batch_transactions_match_stock_git() {
    ...
    let skron = command_any_output_with_stdin(
        skron_bin(),
        skron_repo.path(),
        &["update-ref", "--stdin"],
        &badly_quoted,
    ...
    assert_eq!(skron.0, git_output.0);
    assert_eq!(skron.1, git_output.1);
    assert_eq!(skron.2.lines().next(), git_output.2.lines().next());
    ...
    assert_eq!(skron.0, git.0);
    assert_eq!(skron.1, git.1);
    assert_eq!(skron.2.lines().next(), git.2.lines().next());
...
fn update_ref_stdin_batch_updates_match_stock_git() {
    ...
    for name in [
        ...
        run_skron(skron_repo.path(), ["update-ref", name, &head]);
    }
...
fn update_ref_reflog_updates_match_stock_git() {
    ...
    assert_eq!(
        fs::read_to_string(skron_repo.path().join(".git/logs/refs/heads/reflogged"))
            .expect("read skron branch reflog"),
    ...
    assert_eq!(
        fs::read_to_string(skron_repo.path().join(".git/logs/refs/custom/reflogged"))
            .expect("read skron custom reflog"),
...
fn packed_refs_are_resolved_updated_and_deleted_like_stock_git() {
    ...
    assert_eq!(git(skron_repo.path(), ["rev-parse", "HEAD"]), first);
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["branch", "--list"]),
        git(git_repo.path(), ["branch", "--list"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["branch", "--list", "packed-*"]),
        git(git_repo.path(), ["branch", "--list", "packed-*"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["tag", "--list"]),
...
fn update_ref_no_deref_modes_match_stock_git_head_storage() {
    ...
    run_skron(skron_repo.path(), ["update-ref", "HEAD", &first]);
    assert_eq!(
        fs::read_to_string(skron_repo.path().join(".git/HEAD")).expect("read skron HEAD"),
        fs::read_to_string(git_repo.path().join(".git/HEAD")).expect("read git HEAD")
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["rev-parse", "HEAD"]),
    ...
    let delete_skron_repo = committed_repo();
    let delete_branch = git(
        delete_git_repo.path(),
        ["rev-parse", "--abbrev-ref", "HEAD"],
    );
    assert_eq!(
        run_skron_status(delete_skron_repo.path(), ["update-ref", "-d", "HEAD"]),
        git_status(delete_git_repo.path(), ["update-ref", "-d", "HEAD"])
    );
    assert_eq!(
        fs::read_to_string(delete_skron_repo.path().join(".git/HEAD"))
            .expect("read skron symbolic HEAD"),
    ...
    let no_deref_delete_skron_repo = committed_repo();
    assert_eq!(
        run_skron_status(
            no_deref_delete_skron_repo.path(),
    ...
    assert_eq!(
        no_deref_delete_skron_repo.path().join(".git/HEAD").exists(),
...
fn update_ref_stdin_symref_commands_match_stock_git() {
    ...
    assert_eq!(
        fs::read_to_string(skron_repo.path().join(".git/refs/heads/sym"))
            .expect("read skron symref"),
    ...
    assert_eq!(
        fs::read_to_string(skron_repo.path().join(".git/refs/heads/sym"))
            .expect("read skron updated symref"),
    ...
    assert_eq!(
        skron_repo.path().join(".git/refs/heads/sym").exists(),
...
fn update_ref_invalid_refname_failures_match_stock_git() {
    ...
    for ref_name in [
        ...
        assert_eq!(skron.0, git.0, "status for {ref_name}");
        assert_eq!(skron.1, git.1, "stdout for {ref_name}");
        assert_eq!(
            skron.2.lines().next(),
            git.2.lines().next(),
            "stderr for {ref_name}"
...
fn branch_create_list_delete_and_rename_match_stock_git_state() {
    ...
    assert_eq!(
        run_skron(repo.path(), ["branch", "--show-current"]),
        git(repo.path(), ["branch", "--show-current"])
    );
    assert_eq!(
        run_skron(repo.path(), ["branch", "--show-current", "ignored"]),
        git(repo.path(), ["branch", "--show-current", "ignored"])
    );
    assert_eq!(
        run_skron(repo.path(), ["rev-parse", "--abbrev-ref", "HEAD"]),
        git(repo.path(), ["rev-parse", "--abbrev-ref", "HEAD"])
    );
    run_skron(repo.path(), ["branch", "feature"]);
    assert_eq!(
        run_skron(repo.path(), ["branch"]),
        git(repo.path(), ["branch"])
    );
    assert_eq!(
        run_skron(repo.path(), ["show-ref", "--heads"]),
        git(repo.path(), ["show-ref", "--heads"])
    );

    run_skron(repo.path(), ["branch", "-d", "feature"]);
    assert_eq!(
        run_skron(repo.path(), ["show-ref", "--heads"]),
        git(repo.path(), ["show-ref", "--heads"])
    );

    run_skron(repo.path(), ["branch", "force-delete"]);
    run_skron(repo.path(), ["branch", "-D", "force-delete"]);
    assert_eq!(
        run_skron(repo.path(), ["show-ref", "--heads"]),
        git(repo.path(), ["show-ref", "--heads"])
    );

    run_skron(repo.path(), ["branch", "rename-source"]);
    run_skron(
        repo.path(),
    ...
    run_skron(repo.path(), ["checkout", "rename-target"]);
    run_skron(repo.path(), ["branch", "-m", "rename-current"]);
    assert_eq!(
        git(repo.path(), ["symbolic-ref", "HEAD"]),
        "refs/heads/rename-current"
    );
    run_skron(repo.path(), ["branch", "force-source"]);
    run_skron(repo.path(), ["branch", "force-dest"]);
    run_skron(repo.path(), ["branch", "-M", "force-source", "force-dest"]);
    assert_eq!(
        run_skron(repo.path(), ["show-ref", "--heads"]),
        git(repo.path(), ["show-ref", "--heads"])
    );
...
fn branch_upstream_config_matches_stock_git() {
    ...
    run_skron(skron_repo.path(), ["branch", "-u", "origin/main"]);
    assert_eq!(
        git(skron_repo.path(), ["config", "--get", &remote_key]),
        git(git_repo.path(), ["config", "--get", &remote_key])
    );
    assert_eq!(
        git(skron_repo.path(), ["config", "--get", &merge_key]),
    ...
    run_skron(skron_repo.path(), ["branch", "--unset-upstream"]);
    assert_eq!(
        run_skron_status(skron_repo.path(), ["config", "--get", &remote_key]),
        git_status(git_repo.path(), ["config", "--get", &remote_key])
    );
    assert_eq!(
        run_skron_status(skron_repo.path(), ["config", "--get", &merge_key]),
...
fn tag_create_list_delete_and_annotated_objects_match_stock_git_state() {
    ...
    run_skron(repo.path(), ["tag", "v1.0.0"]);
    assert_eq!(run_skron(repo.path(), ["tag"]), git(repo.path(), ["tag"]));
    assert_eq!(
        run_skron(repo.path(), ["tag", "--list", "v1.*"]),
        git(repo.path(), ["tag", "--list", "v1.*"])
    );
    assert_eq!(
        run_skron(repo.path(), ["show-ref", "--tags"]),
        git(repo.path(), ["show-ref", "--tags"])
    );

    run_skron(repo.path(), ["tag", "-d", "v1.0.0"]);
    assert_eq!(run_skron(repo.path(), ["tag"]), git(repo.path(), ["tag"]));
    ...
    run_skron_with_env(skron_repo.path(), ["tag", "-a", "v1.0.0", "-m", "release"]);

    assert_eq!(
        run_skron(skron_repo.path(), ["cat-file", "-t", "v1.0.0"]),
        git(git_repo.path(), ["cat-file", "-t", "v1.0.0"])
    );
    assert_eq!(
        git(skron_repo.path(), ["cat-file", "-p", "refs/tags/v1.0.0"]),
        git(git_repo.path(), ["cat-file", "-p", "refs/tags/v1.0.0"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["show-ref", "--tags"]),
        git(git_repo.path(), ["show-ref", "--tags"])
    );
    assert_eq!(
        run_skron(skron_repo.path(), ["rev-parse", "--verify", "v1.0.0^{tag}"]),
    ...
    assert_eq!(
        run_skron(skron_repo.path(), ["rev-parse", "--verify", "v1.0.0^{}"]),
    ...
    assert_eq!(
        git(skron_repo.path(), ["cat-file", "-p", "refs/tags/v1.0.1"]),
