...
pub(crate) fn dispatch(command: runtime::Command) -> std::result::Result<(), runtime::CliError> {
    match command {
        runtime::Command::Credential { operation } => {
            runtime::credential_commands::credential(&operation)
        }
        runtime::Command::CredentialStore { file, action } => {
            runtime::credential_commands::credential_store(file, &action)
        }
        runtime::Command::CredentialCache {
            ...
            runtime::credential_commands::credential_cache(timeout, socket, daemon_internal, action)
        }
        command => {
