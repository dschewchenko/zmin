...
pub(crate) fn fast_import() -> Result<()> {
    let repo = find_repo()?;
    let store = LooseObjectStore::new(repo.objects_dir.clone(), GitHashAlgorithm::Sha1);
    let refs = RefStore::new(&repo.git_dir, GitHashAlgorithm::Sha1);
    let mut input = Vec::new();
    io::stdin().read_to_end(&mut input)?;
    FastImportParser::new(input, &store, &refs).parse()
}
...
impl<'a> FastImportParser<'a> {
    fn new(input: Vec<u8>, store: &'a LooseObjectStore, refs: &'a RefStore) -> Self {
        Self {
            input,
            cursor: 0,
            store,
    ...
    fn parse_commit(&mut self, ref_name: &str) -> Result<()> {
        let mark = self.expect_mark()?;
        let author = self.expect_signature("author ")?;
        let committer = self.expect_signature("committer ")?;
        let message = self.expect_data()?;
        let mut parent = None;
        let mut index = self.ref_indexes.remove(ref_name).unwrap_or_default();
    ...
    fn apply_fast_import_modify(&self, index: &mut GitIndex, rest: &str) -> Result<()> {
    ...
    fn expect_signature(&mut self, prefix: &str) -> Result<Signature> {
        ...
        signature_from_commit_bytes(raw.as_bytes())
    }
