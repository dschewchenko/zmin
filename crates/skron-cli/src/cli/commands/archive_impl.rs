...
fn archive_to_tar_bytes(
    ...
    let (tree_id, commit_id, source_mtime) = archive_tree_source(repo, &store, treeish)?;
    ...
    if path_filters.is_empty() {
        archive_tree_entries(&context, &tree_id, "", &mut out)?;
    } else {
...
fn archive_to_zip_bytes(
    ...
    let store = LooseObjectStore::new(repo.objects_dir.clone(), GitHashAlgorithm::Sha1);
    let (tree_id, _commit_id, source_mtime) = archive_tree_source(repo, &store, treeish)?;
    let tree_cache = TreeObjectCache::new(&store);
    let mtime = match options.mtime.as_deref() {
        Some(value) => parse_archive_mtime(value)?,
        None => source_mtime,
    ...
    if path_filters.is_empty() {
        archive_tree_entries_zip(&context, &tree_id, "", &mut zip)?;
    } else {
        for path in path_filters {
            if path.is_empty() {
                archive_tree_entries_zip(&context, &tree_id, "", &mut zip)?;
...
fn archive_tree_source(
    ...
    treeish: &str,
) -> Result<(ObjectId, Option<String>, u64)> {
    ...
    match object.kind {
        GitObjectKind::Commit => {
            ...
            Ok((
                ...
                signature.timestamp as u64,
            ))
        }
        GitObjectKind::Tree => Ok((object_id, None, 0)),
        GitObjectKind::Tag => {
            ...
            if target.kind == GitObjectKind::Commit {
                ...
                Ok((
                    commit.tree.clone(),
                    Some(tag.target.to_hex()),
                    signature.timestamp as u64,
                ))
            } else if target.kind == GitObjectKind::Tree {
                Ok((tag.target, None, 0))
            } else {
                Err(CliError::Fatal {
                    code: 128,
