...
fn bisect_start(args: &[String]) -> Result<()> {
    ...
    let start_id = refs.resolve("HEAD")?;
    fs::write(
...
fn revert_message(id: &ObjectId, commit: &skron_git_core::CommitObject) -> Vec<u8> {
...
pub(crate) fn rebase(
    ...
    if let Some(branch) = args.get(1) {
        worktree_commands::checkout_existing(false, branch)?;
    }
    let repo = find_repo()?;
    ...
    if let Some(todo) = interactive_todo {
        ...
        for commit in commits {
            sequencer_pick(false, false, None, vec![commit.to_hex()])?;
        }
    }
