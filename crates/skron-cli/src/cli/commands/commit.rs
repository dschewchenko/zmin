...
pub(crate) fn dispatch(command: runtime::Command) -> std::result::Result<(), runtime::CliError> {
    match command {
        runtime::Command::Commit {
            ...
            trailers,
            paths,
        } => runtime::commit_commands::commit_command(
            runtime::commit_commands::CommitCommandOptions {
                all,
                only,
                allow_empty,
                amend,
        ...
        runtime::Command::Citool {
            ...
            message_file,
            messages,
        } => runtime::commit_commands::citool_command(
            amend,
            nocommit,
            message_file.as_deref(),
            messages,
        ),
        runtime::Command::Gui { args } => runtime::commit_commands::gui_command(args),
        runtime::Command::WriteTree => runtime::commit_commands::write_tree_command_entry(),
        runtime::Command::CommitTree {
            tree,
            parents,
            messages,
        } => runtime::commit_commands::commit_tree_command(&tree, parents, messages),
        runtime::Command::Mktree {
            nul_terminated,
            missing,
            batch,
        } => runtime::commit_commands::mktree_command(nul_terminated, missing, batch),
