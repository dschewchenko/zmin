...
pub(crate) fn dispatch(command: runtime::Command) -> std::result::Result<(), runtime::CliError> {
    match command {
        runtime::Command::Maintenance {
            ...
            quiet,
            tasks,
        } => runtime::maintenance_commands::maintenance(
            runtime::maintenance_commands::MaintenanceOptions {
                operation: &operation,
                auto,
        ...
        runtime::Command::PrunePacked { dry_run, quiet } => {
            runtime::maintenance_commands::prune_packed_command(dry_run, quiet)
        }
        runtime::Command::Repack {
            ...
            threads,
            keep_pack,
        } => runtime::maintenance_commands::repack_command(
            all,
            all_and_loosen_unreachable,
            delete_redundant,
            quiet,
        ...
        runtime::Command::Gc {
            ...
            aggressive,
            quiet,
        } => runtime::maintenance_commands::gc_command(prune, no_prune, auto, aggressive, quiet),
        runtime::Command::Prune { args } => runtime::maintenance_commands::prune_command(args),
        command => {
