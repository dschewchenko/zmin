...
fn hook_run(
    ...
    let output = ProcessCommand::new(&hook_path)
        .args(args)
...
fn update_index(options: UpdateIndexCommandOptions) -> Result<()> {
    ...
    let paths = update_index_paths(&options, &repo)?;

    for cacheinfo in &options.cacheinfo {
        update_index_cacheinfo(&repo, &store, &mut index, cacheinfo, options.add)?;
    }
...
fn update_index_cacheinfo(
    ...
    let (mode, rest) = value.split_once(',').ok_or_else(|| CliError::Fatal {
        code: 129,
        message: "update-index --cacheinfo expects <mode>,<object>,<path>".into(),
    })?;
    let (id, path) = rest.split_once(',').ok_or_else(|| CliError::Fatal {
        code: 129,
        message: "update-index --cacheinfo expects <mode>,<object>,<path>".into(),
    })?;
    ...
    for remote in remotes {
        ...
        let Some(source_path) = local_repository_path_from_location(&url)? else {
            return Err(CliError::Fatal {
                code: 128,
                message: format!(
                    "backfill for promisor remote '{remote}' is only implemented for local and HTTP transports"
                ),
            });
        };
...
fn diagnose_log(repo: &GitRepo) -> Result<String> {
    ...
    report.push_str("SHA-1: skron-git-core\n");
    report.push_str("SHA-256: skron-git-core\n");
...
fn run_cvs_command(
    ...
    let mut command = ProcessCommand::new("cvs");
    if let Some(root) = options.cvsroot.as_deref() {
...
fn read_or_generate_cvsps_output(options: &CvsImportOptions) -> Result<String> {
    ...
    let mut command = ProcessCommand::new("cvsps");
    if let Some(root) = options.cvsroot.as_deref() {
        command.arg("-d").arg(root);
    }
...
fn archimport(args: Vec<String>) -> Result<()> {
    ...
    let temp_root = match &options.temp_dir {
        ...
        None => create_cli_temp_root("skron-archimport")?,
    };
...
fn run_arch_command(client: &str, args: &[&str], checkout_dir: &Path) -> Result<()> {
    let output = ProcessCommand::new(client)
        .args(args)
        .arg(checkout_dir)
        .output()
        .map_err(CliError::Io)?;
...
fn p4_print_file(depot_path: &str, revision: &str) -> Result<Vec<u8>> {
    ...
    let output = ProcessCommand::new("p4")
        .args(["print", "-q", &spec])
        .output()
        .map_err(CliError::Io)?;
    if output.status.success() {
...
fn svn_cat_file(url: &str, path: &str) -> Result<Vec<u8>> {
    ...
    let output = ProcessCommand::new("svn")
        .args(["cat", &full_url])
        .output()
        .map_err(CliError::Io)?;
    if output.status.success() {
...
fn instaweb_start(repo: &GitRepo, options: &InstawebCommandOptions) -> Result<()> {
    ...
    let child = match options.httpd.as_deref().unwrap_or("builtin") {
        "skron" | "builtin" => instaweb_spawn_builtin(repo, options)?,
