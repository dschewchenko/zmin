...
pub(crate) fn bundle(operation: &str, file: PathBuf, args: Vec<String>) -> Result<()> {
    match operation {
        "create" => bundle_create(file, args),
...
fn bundle_create(file: PathBuf, revs: Vec<String>) -> Result<()> {
    if revs.is_empty() {
        return Err(CliError::Fatal {
            code: 129,
    ...
    let rev_list = collect_rev_list_revs(&repo, &store, false, revs)?;
    let commits = collect_commits_with_exclusions(&repo, &store, &rev_list, None)?;
    let commit_cache = CommitObjectCache::new(&store);
    let initial_capacity =
...
fn parse_bundle_header(path: &std::path::Path) -> Result<ParsedBundleHeader> {
    ...
    if bundle_header_line(&line)? != "# v2 git bundle" {
        return Err(CliError::Fatal {
            code: 128,
...
fn index_bundle_pack(
    ...
    pack_offset: u64,
    store: Option<&LooseObjectStore>,
) -> Result<skron_git_core::IndexedPack> {
...
fn verify_pack_one(idx_path: &std::path::Path, verbose: bool, stat_only: bool) -> Result<()> {
    ...
    let verified = skron_git_core::verify_pack_file_matches_index(
        GitHashAlgorithm::Sha1,
        &pack_path,
        idx_path,
        verbose,
...
mod tests {
    use super::*;
    use skron_git_core::{GitObjectSink, InMemoryObjectStore};
