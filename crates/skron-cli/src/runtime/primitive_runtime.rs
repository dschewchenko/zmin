...
use skron_git_core::GitHashAlgorithm;

use super::{
    CliPatchRenderer, CliRewriteEngine, CliTransportAdapter, CliWorktreeAdapter, GitRepo,
    OwnedCliObjectStoreAdapter, OwnedCliRefsStoreAdapter,
};
...
impl CliPrimitiveRuntime {
    ...
    pub(crate) fn new_from_repo_with_mode(
        ...
        Self {
            ...
            refs_store: OwnedCliRefsStoreAdapter::from_path(&repo.git_dir, algorithm),
            transport: CliTransportAdapter,
...
mod tests {
    ...
    use skron_primitives::git_runtime::GitOid;
    ...
    fn cli_primitive_runtime_transport_upload_pack_is_mode_stable_for_fetch_path() {
        ...
        for mode in [
            ...
            let response = runtime
                .transport()
                .upload_pack_request(
                    &skron_primitives::git_runtime::TransferRequest {
                        remote: remote.clone(),
                        service: skron_primitives::git_runtime::GitTransportService::UploadPack,
                        refspecs: vec![],
                        atomic: false,
                        thin_pack: false,
                        depth: None,
        ...
        for mode in [
            ...
            let output = runtime
                ...
                .receive_pack_request(
                    &skron_primitives::git_runtime::TransferRequest {
                        remote: remote.clone(),
                        service: skron_primitives::git_runtime::GitTransportService::ReceivePack,
                        refspecs: vec![],
                        atomic: false,
                        thin_pack: false,
                        depth: None,
    ...
    fn init_local_repo_for_transport_tests() -> TransportFixture {
        ...
        let commit_id = {
            let object = skron_git_core::CommitBuilder::new(
                tree_id.clone(),
                signature.clone(),
                signature.clone(),
            )
    ...
    fn init_repo_for_primitive_surface_tests() -> PrimitiveSurfaceFixture {
        ...
        let parent_tree_content = skron_git_core::encode_tree(&[skron_git_core::TreeEntry {
            mode: skron_git_core::TreeMode::File,
        ...
        let parent_commit = skron_git_core::CommitBuilder::new(
            parent_tree.clone(),
            signature.clone(),
            signature.clone(),
        )
        ...
        let head_tree_content = skron_git_core::encode_tree(&[skron_git_core::TreeEntry {
            mode: skron_git_core::TreeMode::File,
            name: b"note.txt".to_vec(),
            id: head_blob,
        }])
        ...
        let head_commit =
            skron_git_core::CommitBuilder::new(head_tree.clone(), signature.clone(), signature)
                .parent(
    ...
    fn cli_primitive_runtime_rewrite_paths_are_mode_stable() {
        ...
        for mode in [
            ...
            let replacement_commit =
                skron_git_core::decode_commit(GitHashAlgorithm::Sha1, &replacement_bytes)
            ...
            let retree_commit =
                skron_git_core::decode_commit(GitHashAlgorithm::Sha1, &retree_bytes)
