...
pub(crate) fn upstream_counts(
    ...
    let local = collect_commits_cached(repo, &store, &commit_cache, &[String::from("HEAD")], None)?
        .into_iter()
        .map(|id| id.to_hex())
        .collect::<HashSet<_>>();
...
pub(crate) fn parse_git_date(value: &str) -> Result<(i64, String)> {
    ...
    let timestamp = parts
        .next()
        .ok_or_else(|| CliError::Message("git date is empty".into()))?
        .trim_start_matches('@')
        .parse::<i64>()
        .map_err(|err| CliError::Message(format!("git date timestamp is invalid: {err}")))?;
