...
use super::{CliError, GitRepo, Result, resolve_commitish_io};
...
pub(crate) fn print_rev_parse_object(
    ...
    if let Some(length) = short {
        ...
        if length == 0 || length > hex_len {
            return Err(CliError::Message(format!(
                "invalid --short length `{length}`"
            )));
        }
        println!("{}", id.short_hex(length));
    } else {
        println!("{id}");
    }
    Ok(())
...
pub(crate) fn resolve_objectish(repo: &GitRepo, objectish: &str) -> io::Result<ObjectId> {
    ...
    if let Some((base, peel)) = split_peel_suffix(objectish) {
        return resolve_typed_objectish(repo, &store, base, peel);
    }
    if let Some((base, path)) = objectish.split_once(':') {
        if base.is_empty() {
            return resolve_index_object_path(repo, path);
        }
    ...
    if objectish.contains('~') {
        return resolve_commitish_io(repo, &store, objectish);
    }
    resolve_plain_objectish(repo, &store, objectish)
}

fn resolve_index_object_path(repo: &GitRepo, raw_path: &str) -> io::Result<ObjectId> {
...
pub(crate) fn resolve_plain_objectish(
    ...
    if objectish == "HEAD" || objectish.starts_with("refs/") {
        return RefStore::new(&repo.git_dir, GitHashAlgorithm::Sha1).resolve(objectish);
    }
    if let Some(ref_name) = objectish.strip_prefix("heads/") {
        return RefStore::new(&repo.git_dir, GitHashAlgorithm::Sha1)
            .resolve(&format!("refs/heads/{ref_name}"));
    }
    if let Some(ref_name) = objectish.strip_prefix("tags/") {
        return RefStore::new(&repo.git_dir, GitHashAlgorithm::Sha1)
            .resolve(&format!("refs/tags/{ref_name}"));
    }
...
pub(crate) fn resolve_treeish(
    ...
    let mut id = if treeish.contains('~') {
...
pub(crate) fn resolve_named_ref(repo: &GitRepo, name: &str) -> io::Result<Option<ObjectId>> {
    ...
    for candidate in [
        ...
        match refs.read_ref(&candidate) {
            Ok(RefTarget::Direct(id)) => return Ok(Some(id)),
            Ok(RefTarget::Symbolic(target)) => return refs.resolve(&target).map(Some),
...
mod tests {
    ...
    impl GitObjectStore for CountingObjectStore {
        fn read_object(&self, _id: &ObjectId) -> io::Result<skron_git_core::LooseObject> {
