...
fn resolve_submodule_clone_url(parent_repository: &str, url: &str) -> String {
    ...
    let base = parent.parent().unwrap_or(parent.as_path());
    canonical_or_absolute(base.join(url)).display().to_string()
}
...
pub(crate) fn write_gitmodules_entry(repo: &GitRepo, url: &str, path: &str) -> Result<()> {
    ...
    let mut out = match fs::read_to_string(&gitmodules) {
        Ok(raw) => raw,
